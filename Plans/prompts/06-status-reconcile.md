# 06 — Status Reconcile (RETIRED)

> **RETIRED — superseded by slice 741 auto-regen (2026-06-12).**
> There is no longer any status reconcile step, manual or otherwise.
> `docs/issues/_STATUS.md` is **generated** by `scripts/gen-status.sh` (`just status`) and,
> as of **slice 741**, **auto-regenerated by CI** (the `Slice status · auto-regenerate` job in
> `.github/workflows/ci.yml`) on every push to `main` — it commits the regenerated file back
> with `[skip ci]`, so the committed tracker is always fresh within one main-push with **no PR
> and no human action**. Slice status is derived from `git log` + open PRs + branches +
> `docs/issues/_events.jsonl`. This file is kept only as a historical pointer; do not run it as
> a workflow. The sole remaining manual lever is the event overlay for states git can't prove:
> `just event <slice> <state> "<why>"`. See [`docs/issues/_GENERATOR.md`](../../docs/issues/_GENERATOR.md).
>
> The original 2026-06-10 "verification-only" content is retained below for historical context.
