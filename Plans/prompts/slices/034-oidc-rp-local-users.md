# Slice 034 — OIDC RP for SSO + local users for solo deployments

Copy-paste prompt for executing slice 034. See `Plans/prompts/04-per-slice-template.md` for the template that generated this file.

---

```
Build docs/issues/034-oidc-rp-local-users.md.

Branch: auth/034-oidc-rp-local-users (from main).
...
Open PR titled "feat(auth): OIDC RP for SSO + local users for solo deployments (#034)" with body: AC pass/fail · files changed · CI URL · open questions surfaced.

Use Algorithm mode. Initialize a PRD (id: 034-oidc-rp-local-users).
```
...
- Issue file: [`docs/issues/034-oidc-rp-local-users.md`](../../../docs/issues/034-oidc-rp-local-users.md)
- Cluster: `auth`
- Branch: `auth/034-oidc-rp-local-users`
- PRD id: `034-oidc-rp-local-users`
