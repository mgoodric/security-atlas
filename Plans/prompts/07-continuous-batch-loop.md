...
>   derived. As of **slice 741**, `_STATUS.md` is **auto-regenerated by CI** on every push to
>   `main`, so there is **nothing to run and nothing to commit** per iteration — the merge of
>   the last slice PR is the terminal action.
...
selection → claim (pushed feat/NNN branch) → parallel Engineers → merge order →
merge queue → final summary. As of slice 741 there is NO status reconcile PR:
`_STATUS.md` is auto-regenerated by CI on every push to `main`. The merge of the
last slice PR is the terminal action of the iteration.
...
The loop pauses (exits without ScheduleWakeup) on any of:
  ...
  E-5 · Branch protection rejects a slice PR's squash-merge for an unexpected
        structural reason (not a transient CI fail). NOTE: the slice-741
        auto-regenerate job's push to main is fail-soft by design — a
        rejected `_STATUS.md` bot push does NOT escalate (it prints the
        one-time bypass-config hint and exits 0); only a rejected SLICE-PR
        merge is an E-5.
...
If the WORK PHASE completed successfully (the last slice PR squash-merged in
Step 5 of 05; there is no reconcile PR — slice 741 auto-regenerates `_STATUS.md`
in CI) AND no escalation fired AND no guard fired:

  Call ScheduleWakeup with:
    - delaySeconds: 300
        (5 min — lets CI on the just-merged slice PR, INCLUDING the
        `Slice status · auto-regenerate` job's [skip ci] commit, fully
        drain before the next iteration's GUARD-2 reads `gh pr list` and
        GUARD-4 reads `origin/main`. The auto-regen bot commit is a normal
        commit on main; GUARD-4's drift check tolerates it because the next
        iteration fetches origin/main fresh before computing drift.)
...
- Each iteration: ~2-3hr wall-clock (≈ max slice estimate; the ~12 min status-PR overhead is gone — slice 741 auto-regenerates `_STATUS.md` in CI, no claim-stake or reconcile PR)
