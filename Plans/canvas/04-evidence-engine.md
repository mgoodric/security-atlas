...
### 4.6.7 Board-narrative AI-assist (OQ #14 resolved 2026-05-20)

The auto-drafted board narrative is the second AI-assist surface in the platform (after questionnaire-answer drafting) and is the **highest-risk surface in the entire platform**. The deep insight that load-bears the design: **board narratives are unique among GRC outputs because their audience is non-technical by definition and cannot verify the content** — board members read the narrative at face value. The hallucination cost is asymmetric vs. every other AI-assist surface (questionnaires reach auditors / prospects who can spot bad claims; mapping suggestions reach a domain expert; freshness explanations are narrow and verifiable). The constitutional response is a coupled bundle of seven design commitments, locked together so the worst-case LLM output remains acceptable.

The seven sub-decisions (full constitutional text in [`CLAUDE.md`](../../CLAUDE.md) "AI-assist boundary (hard)" → "Board-narrative AI-assist"; full rationale + rejected alternatives in [`docs/adr/0006-board-narrative-ai-assist.md`](../../docs/adr/0006-board-narrative-ai-assist.md)):

| #   | Sub-decision                                                                                                                                                                                                            |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | **Input shape: hybrid.** Deterministic pre-computation rollup + cited evidence excerpts. Not raw evidence (model wanders), not pure rollup (model fabricates context).                                                  |
| D2  | **Approval granularity: per-section.** Numbered sections; each approve/edit/reject independently. Not per-narrative (too coarse), not per-claim (too friction-heavy).                                                   |
| D3  | **Audit trail: full prompt + full response, every time.** System prompt + evidence inputs + draft + edits + final approved text. Storage is small (KB per section); forensically airtight.                              |
| D4  | **Hallucination guardrails: all four.** (a) Mandatory citations to evidence/control/risk IDs; (b) numeric verification against the deterministic pre-computation; (c) section-shape enforcement; (d) editor-mode UX.    |
| D5  | **Inference backend: Llama 3.1 8B Instruct default + cloud opt-in.** Local Ollama on commodity hardware (8-12GB GPU); cloud LLM per-tenant opt-in via slice 050's banner discipline. Refresh cadence every 6-12 months. |
| D6  | **Rejection / iteration flow: inline-edit default** with regenerate, regenerate-with-instruction, and write-from-scratch as explicit affordances.                                                                       |
| D7  | **Prompt-template versioning: snapshot with each generation.** Old reports stay immutable; no retroactive re-flagging.                                                                                                  |

**Tone discipline.** The system prompt enforces a measured, factual, slightly defensive voice. Marketing-voice phrases (`we are proud to report`, `industry-leading`, `world-class`, unprompted superlatives) are rejected at generation and at post-generation regex check. The canonical full list — banned phrases, banned framings, permitted phrases commonly mistaken as banned, and the living-document discipline for adding new entries — lives at [`docs/governance/board-narrative-tone-anti-patterns.md`](../../docs/governance/board-narrative-tone-anti-patterns.md). The file is the trust root for D4's tone aspect; modifications require maintainer review under branch protection.

**Implementation timing.** Board-narrative v0 is **v2+ work** (multi-slice build: prompt construction + numeric-verification library + section-shape enforcement + editor-mode operator UX + audit-log integration + prompt-template versioning). Slice 182 lands the foundation pre-commitments only (constitutional expansion in CLAUDE.md + this canvas reflection + the ADR + the tone-anti-pattern reference + the local-model refresh cadence). Schema-level extensions (`prompt_version`, `model_name`, `model_version`, `model_provider` columns on the eventual board-narrative-records table) ship WITH board-narrative v0, not in the foundation slice.

**Operational discipline (D5 refresh cadence).** The default local-model recommendation is reviewed every 6-12 months by the maintainer per [`docs/operator/maintenance-cadence.md`](../../docs/operator/maintenance-cadence.md) — when a new local model with comparable hardware requirements demonstrably outperforms Llama 3.1 8B Instruct on board-narrative-style drafting, the maintainer updates CLAUDE.md + this canvas section + the operator docs + the ADR's D5 entry, and records the refresh in `docs/audit-log/model-refresh-<YYYY>-<MM>.md`.
