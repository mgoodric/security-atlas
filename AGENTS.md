# AGENTS.md — security-atlas

> Read this first when starting any session in this repo.
>
> **This file is the canonical per-repo agent context** (open-engine
> `docs/AGENTS-MD-CONVENTION.md`, OE-642). `CLAUDE.md` at the repo root is a
> symlink to it — Claude Code reads it via the symlink; Codex and agy read
> `AGENTS.md` natively. Edit `AGENTS.md` only; never create diverging content
> in a harness-specific file. Engine autonomy policy (risk tiers, escalation)
...
`just` (justfile at root) is the task runner; `just` alone lists every recipe.
The full table is in `CONTRIBUTING.md` "Task surface". The load-bearing ones:

```bash
# build:
just build              # build-go (go build ./...) + build-frontend (npm workspaces)

# test (unit tiers — no external services needed):
just test               # test-go (go test ./...) + test-frontend (vitest in web/)

# integration (real Postgres w/ RLS — see harness below):
just db-up              # local postgres:16-alpine in Docker (container: security-atlas-pg)
just migrate-up         # bootstrap roles + apply migrations/sql/*.sql (needs $DATABASE_URL)
just test-integration   # go test -tags=integration -race ./internal/db/... (needs $DATABASE_URL_APP)

# lint / typecheck:
just lint               # lint-go (golangci-lint, pinned) + errleak + duphelper + frontend + python

# everything CI-shaped locally:
just ci                 # lint + test + build
```

### The integration harness (not a mocked DB)

Integration tests run against a **real Postgres with RLS enforced** — never a
mock. Two connection roles matter:

- `$DATABASE_URL` — the migrate/owner role (`atlas_migrate`); used by
  `just migrate-up`.
- `$DATABASE_URL_APP` — the **non-owner app role** that RLS actually binds on;
  used by the tests. Tests skip (with a stderr note) when it is unset, so a
  green run without `$DATABASE_URL_APP` proves nothing.

Local `just test-integration` covers `./internal/db/...`. CI runs the wider
suite — `go test -tags=integration -p 1 ./internal/...` with real Postgres +
NATS + MinIO — sharded across runner legs per `scripts/integration-shards.txt`
(slice 417; guard: `just check-integration-shard-coverage`). Green local
integration is necessary, not sufficient; the CI check `Go · integration
(Postgres RLS)` is the gate.
...
| **Frontend**                | **Next.js 16 App Router + shadcn/ui + Tailwind 4 + TanStack Query** (verified 2026-07-28 against `web/package.json`: `next@16.2.11`, `react@19.2.7`, `eslint@^9` per slice 078)                                                                                                                                                                                                                                                                                                                                                                                                                         | Server Components for data-heavy dashboards; shadcn/ui aligns with mockups                                                                                                                               |
...
| **Observability**           | OTEL native (traces + metrics), no-op unless `OTEL_EXPORTER_OTLP_ENDPOINT` is set; the `deploy/observability/` bundle ships OTel Collector + Prometheus + Tempo (Grafana + Loki are expected to already exist on the host)                                                                                                                                                                                                                                                                                                                                                                              | Production users route OTEL to their own stack                                                                                                                                                           |
...
│   ├── atlas-cli/                # CLI (`security-atlas-cli evidence push`, etc.)
...
| ~~Risk-methodology default lock (NIST 800-30 + 5x5 + dollar-banded vs FAIR)~~ | RESOLVED 2026-07-28 (OPENENGINE-596): **NIST 800-30 remains the default; FAIR is supported as a separate dollar-exposure methodology and is not normalized into the 5x5 rollup**. See [`docs/audit-log/596-fair-risk-methodology-decisions.md`](docs/audit-log/596-fair-risk-methodology-decisions.md). |
...
1. `just ci` exits 0 (lint + unit tests + build — no services needed).
2. If the change touches Postgres-backed code (`internal/db/dbx` importers,
   RLS, migrations, handlers): run the integration harness —
   `just db-up`, export `$DATABASE_URL` + `$DATABASE_URL_APP`,
   `just migrate-up`, then `just test-integration` — and confirm the tests
   actually RAN (they skip silently-ish when `$DATABASE_URL_APP` is unset).
   The full CI-shaped sweep is `go test -tags=integration -p 1 ./internal/...`.
3. If the change touches `web/`: `cd web && npm test`, and `npm run test:e2e`
   for user-flow changes.
4. New tenant-scoped tables carry RLS policies (`just audit-rls` must stay
   green); packages importing `internal/db/dbx` are enrolled per
   `just audit-integration-enrolment`.
5. The PR turns all four required checks green — that, not local output, is
...
- **Local integration ≠ CI integration.** `just test-integration` covers
  `./internal/db/...` only; CI runs `./internal/...` with NATS + MinIO too.
  A change outside `internal/db` can pass locally and fail the CI tier.
- **Version pins are load-bearing.** sqlc is pinned (`SQLC_VERSION` in the
  justfile) — regenerating `internal/db/dbx` with any other version produces
  spurious diffs; golangci-lint is pinned in lockstep with CI. Bump either
  only in a dedicated slice that updates both pin sites.
- **The coverage gate is a ratchet.** Never lift a floor in
  `cmd/scripts/coverage-thresholds.json` without the tests in the same PR,
  and never add the tests without the lift.
- **Integration flakes are hard fails.** The Go integration tier has no
  retry by design; investigate to root cause, don't re-run to green.
- **DCO identity:** the sign-off email must match the commit author
  (`Matt Goodrich <matt@mattgoodrich.com>`); never set a bot
  `user.name`/`user.email` override in this repo.
- **`ai_assisted`/`human_approved` guard:** new AI-assist surfaces adopt the
  shared `ai_assist_human_approver_guard` CHECK function; do not re-author
  the predicate inline (slice 498).
