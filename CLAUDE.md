...
| **Authorization Server**    | Internal OAuth 2.0 AS layer issuing JWT access tokens (RS256 or ES256) per RFC 9068 JWT Profile · RFC 8693 Token Exchange (tenant-switch verb) · RFC 7636 PKCE (browser) · RFC 8628 Device Authorization Grant (CLI) · RFC 7009 Revocation · RFC 7662 Introspection. JWT claims carry `atlas:current_tenant_id`, `atlas:available_tenants[]`, `atlas:roles{tenant→[role]}`, `atlas:super_admin`. | Resolves OQ #21 (Reading D, 2026-05-20). Spine: slices 187-192. ADR-0003. Composes with the OIDC RP — RP authenticates the human; AS mints the atlas JWT.    |
