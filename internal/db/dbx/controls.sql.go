...
WITH matched_controls AS (
    ...
    WHERE c.tenant_id = $1
      ...
      AND ($3::text IS NULL OR c.control_family = $3::text)
      AND (
            $4::uuid[] IS NULL
            OR c.scf_anchor_id = ANY($4::uuid[])
          )
),
ranked_controls AS (
    SELECT
...
type ListActiveControlsForPortfolioRow struct {
	...
	FailingEvidenceCount int64              `json:"failing_evidence_count"`
