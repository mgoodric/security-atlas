...
const parentChainIDs = `-- name: ParentChainIDs :many
...
type ParentChainIDsParams struct {
	TenantID pgtype.UUID `json:"tenant_id"`
	ID       pgtype.UUID `json:"id"`
}
...
func (q *Queries) ParentChainIDs(ctx context.Context, arg ParentChainIDsParams) ([]pgtype.UUID, error) {
	rows, err := q.db.Query(ctx, parentChainIDs, arg.TenantID, arg.ID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
