// Slice 412 — contract-test-tier rollout to the control-detail tail routes
...
//
// This is the slice-392 / slice-409 / slice-410 / slice-411 shared-recorder
// helper copied into this package because Go test files cannot cross a package
// boundary (the same reason the sibling copies exist in
// internal/api/controldetail, internal/api/auditperiods, internal/api/risks,
// internal/api/dashboard, …).
//
// Pattern (ADR-0007 option 1, slice-411 per-route Option-A seam):
//
//	provider test:  construct the real Handler over an injected fixed-row
//	                two-method controlEvaluator stub (no eval.Engine, no
//	                Postgres pool) -> drive the real State / Effectiveness
//	                handler -> canonicalize the body -> diff against the
...
//	                (web/app/api/controls/[id]/{state,effectiveness}/route.ts)
//	                are VERBATIM passthroughs (NextResponse.json(body) over
//	                res.json() unchanged), so the consumer assert is
//	                toEqual(golden) like the slice-411 control-detail tabs.
//
// Regenerate the goldens after an intentional shape change:
//
//	go test ./internal/api/controlstate/ -run TestContract -update
//
// Runs on the plain `go test ./...` unit surface (no DB): the State +
// Effectiveness paths read through the unexported two-method controlEvaluator
// seam (handlers.go), which the recorder satisfies with a fixed-row stub. That
// is what keeps the tier zero-new-gate (ADR-0007 (d): rides the Go-unit
...
// panic (mirrors slice 392/409/410/411's lazy lookup).
...
func assertContractGolden(t *testing.T, path, comment, endpoint string, recorded map[string]json.RawMessage) {
	...
	for name, gotBody := range recorded {
		...
		if !bytes.Equal(gotBody, wantCanon) {
			t.Errorf("variant %q wire shape drifted from golden:\n  handler: %s\n  golden:  %s\nrun `go test ./internal/api/controlstate/ -run TestContract -update` if the change is intentional",
