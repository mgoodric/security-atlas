// Slice 692 — contract-test-tier rollout to the manual-attestation read route
// served by this package:
//
//	GET /v1/controls/{id}/attest-form  -> control-attest-form.golden.json
//
// This is the slice-392 / slice-409 / slice-410 / slice-411 / slice-412 /
// slice-687 / slice-689 / slice-690 shared-recorder helper copied into this
// package because Go test files cannot cross a package boundary (the same
// reason the sibling copies exist in internal/api/controldetail,
// internal/api/audit, internal/api/auditnotes, internal/api/walkthroughs, …).
//
// Pattern (ADR-0007 option 1, slice-411 per-route read seam):
//
//	provider test:  construct the real AttestHandler over an injected fixed-row
//	                controlByIDReader stub (no pgx pool) -> route the GET through
//	                a chi mux so chi.URLParam(r, "id") resolves -> drive the real
//	                AttestForm handler -> canonicalize the body -> diff against
//	                the committed golden under web/lib/contracts/.
//	consumer test:  read the same golden -> assert the BFF passthrough holds
//	                against the recorded upstream truth. The BFF
//	                (web/app/api/controls/[id]/attest-form/route.ts) is a
//	                VERBATIM passthrough (getAttestForm returns res.json()
//	                unchanged; the route does NextResponse.json(form)), so the
//	                consumer assert is toEqual(golden).
//
// Regenerate the golden after an intentional shape change:
//
//	go test ./internal/api/controls/ -run TestContract -update
...
// (mirrors slice 392/409/410/411/412/687/689's lazy lookup).
...
func assertContractGolden(t *testing.T, path, comment, endpoint string, recorded map[string]json.RawMessage) {
	...
	for name, gotBody := range recorded {
		...
		if !bytes.Equal(gotBody, wantCanon) {
			t.Errorf("variant %q wire shape drifted from golden:\n  handler: %s\n  golden:  %s\nrun `go test ./internal/api/controls/ -run TestContract -update` if the change is intentional",
