// Slice 409 — contract-test-tier rollout to the freshness + drift panel
// routes (shared provider-side helper for this package).
//
// This is the slice-392 shared-recorder helper (internal/api/
// contractrecord_test.go), copied into this package because Go test files
// cannot cross a package boundary (the same reason slice 392 gave for the
// sibling copies in internal/api/me + internal/api/admindemo, decisions
// log D3).
//
// Pattern (ADR-0007 option 1, slice-409 Option A seam):
//
//	provider test:  construct the real Handler over injected fixed-row
//	                stubs (no Postgres pool) -> drive the real handler ->
//	                canonicalize the body -> diff against the committed
//	                golden under web/lib/contracts/<endpoint>.golden.json.
...
//
// Runs on the plain `go test ./...` unit surface (no DB): the handler
// depends on the unexported read seams (freshnessLister / driftReporter),
// which the recorder satisfies with fixed-row stubs. That is what keeps
// the tier zero-new-gate (ADR-0007 (d): rides the Go-unit surface, no
// fifth CI job; slice 409 P0-409-1: no recorder on the integration
// surface).
...
// flag set the surrounding `go test` invocation uses without a
// duplicate-flag panic (mirrors slice 392's lazy lookup).
...
// canonicalizeJSON re-marshals a body through a generic value so the
// golden is byte-stable regardless of struct/map field order.
...
func assertContractGolden(t *testing.T, path, comment, endpoint string, recorded map[string]json.RawMessage) {
	...
	for name, gotBody := range recorded {
		...
		if !bytes.Equal(gotBody, wantCanon) {
			t.Errorf("variant %q wire shape drifted from golden:\n  handler: %s\n  golden:  %s\nrun `go test ./internal/api/freshnessdrift/ -run TestContract -update` if the change is intentional",
