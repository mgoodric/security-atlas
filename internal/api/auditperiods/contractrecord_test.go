// Slice 411 — contract-test-tier rollout to the audit-workspace period-index
// route (GET /v1/audit-periods) served by this package (shared provider-side
// helper).
//
...
// same reason the sibling copies exist in internal/api/risks,
// internal/api/controldetail, internal/api/dashboard, …).
//
// Pattern (ADR-0007 option 1, slice-411 list-only Option A seam):
//
//	provider test:  construct the real Handler over an injected fixed-row
//	                list-only stub (no Postgres pool) -> drive the real List
//	                handler -> canonicalize the body -> diff against the
//	                committed golden under
//	                web/lib/contracts/audit-periods.golden.json.
//	consumer test:  read the same golden -> assert the BFF passthrough holds
//	                against the recorded upstream truth. The /api/audits BFF
//	                (web/app/api/audits/route.ts) is a VERBATIM passthrough (it
//	                forwards the upstream body text unchanged), so the consumer
//	                assert is toEqual(golden) like the slice-409 dashboard
//	                panels — NOT transform-aware like slice 410's risks BFF.
//
// Regenerate the golden after an intentional shape change:
//
//	go test ./internal/api/auditperiods/ -run TestContract -update
//
// Runs on the plain `go test ./...` unit surface (no DB): the List path reads
// through the unexported list-only periodLister seam (handlers.go), which the
// recorder satisfies with a fixed-row stub. That is what keeps the tier
...
func assertContractGolden(t *testing.T, path, comment, endpoint string, recorded map[string]json.RawMessage) {
	...
	for name, gotBody := range recorded {
		...
		if !bytes.Equal(gotBody, wantCanon) {
			t.Errorf("variant %q wire shape drifted from golden:\n  handler: %s\n  golden:  %s\nrun `go test ./internal/api/auditperiods/ -run TestContract -update` if the change is intentional",
