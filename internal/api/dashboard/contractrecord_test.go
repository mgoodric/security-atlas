// Slice 409 — contract-test-tier rollout to the dashboard panel routes
// (shared provider-side helper for this package).
//
// This is the slice-392 shared-recorder helper (internal/api/
// contractrecord_test.go), copied into the dashboard package because Go
// test files cannot cross a package boundary (the same reason slice 392
// gave for the sibling copies in internal/api/me + internal/api/admindemo,
// decisions log D3).
//
// Pattern (ADR-0007 option 1, slice-409 Option A seam):
//
//	provider test:  construct the real Handler over an injected fixed-row
//	                reader stub (no Postgres pool) -> drive the real handler
...
// depends on the unexported `reader` seam, which the recorder satisfies
// with a fixed-row stub. That is what keeps the tier zero-new-gate
// (ADR-0007 (d): rides the Go-unit surface, no fifth CI job; slice 409
// P0-409-1: no recorder on the integration surface).
...
func assertContractGolden(t *testing.T, path, comment, endpoint string, recorded map[string]json.RawMessage) {
	...
	for name, gotBody := range recorded {
		...
		if !bytes.Equal(gotBody, wantCanon) {
			t.Errorf("variant %q wire shape drifted from golden:\n  handler: %s\n  golden:  %s\nrun `go test ./internal/api/dashboard/ -run TestContract -update` if the change is intentional",
