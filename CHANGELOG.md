...
* **fix(db): deterministic exit for the metrics-scheduler inline-sweep integration test (integration-tier flake).** `TestRun_FiresInlineSweepAndExitsOnCancel` in `internal/metrics/scheduler/integration_test.go` used a single `context.WithTimeout(500ms)` as BOTH the inline pre-tick sweep's DB-work budget AND the `Run()` exit trigger. Under CI contention a cold-pool first sweep (list-tenants → begin-tx → apply-tenant-GUC → compute-evaluators → insert-observation → commit) can exceed 500ms, so the context cancelled mid-sweep, the observation insert was aborted, no row was written, and the `observations >= 1` assertion failed — surfacing identically on Go-touching dependabot PRs (#636, #637, #951, #953) and silently blocking each from validating. (The `psql ... DROP POLICY IF EXISTS tenant_read ... does not exist, skipping` NOTICE that appeared just above the failure in the CI log is a benign first-apply idempotency-guard message from the unrelated `20260518000000_audit_sink_failures.sql` migration; the full forward migration set applies cleanly and idempotently — verified by a 3× drop-recreate-reapply — and was misattributed as the failure site.) Fixed by decoupling the two concerns: the test now runs `Run()` under a cancel-only context (no artificial deadline, so the inline sweep always runs to completion), polls for the observation the inline sweep writes (generous 5s outer bound that returns as soon as the row appears), then calls `cancel()` to exercise the clean-exit path. The exit is now driven by an OBSERVED sweep completion rather than a wall-clock guess. No production code, schema, or migration change — test-only. Per slice 353 Q-16 (no-retry, investigate-every-flake): root-caused, not papered over.
