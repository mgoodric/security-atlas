...
| `global-search.spec.ts`      | Global ⌘K search, end to end against the REAL `/api/search` → `GET /v1/search` path (no route mock): shortcut opens, typed query returns controls + risks + evidence hits, Enter opens the control, ArrowDown + Enter opens the risk. Seeded by `fixtures/e2e/global-search.sql` (OE-398c). |
...
| `personnel-security.spec.ts` | `/personnel-security` — manual checklist create, item completion with evidence URI + notes, overdue-offboarding prominence (top sort + badge) on the list. Self-establishing: the UI create form seeds the data; no SQL fixture (OE-664).                                                   |
