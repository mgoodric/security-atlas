...
// /v1/evidence?control_id=<id>, the per-control evidence-ledger window the
// control-detail evidence card reads).
//
// PROVIDER: internal/api/controldetail/evidence_contract_test.go records the
// real Evidence handler's per-control bodies into control-evidence.golden.json.
// This CONSUMER half asserts the BFF (web/app/api/evidence/route.ts) against
// them. That BFF is a VERBATIM passthrough — it forwards the upstream body
// bytes + status unchanged — so the assert is toEqual(golden), like the
// slice-411 control-detail tabs.
//
// Load-bearing field assumptions (the per-control evidence envelope built by
// internal/api/controldetail/handler.go Evidence + evidenceWireFrom):
...
const golden: Golden = JSON.parse(
  readFileSync(join(__dirname, "control-evidence.golden.json"), "utf8"),
) as Golden;

const CONTROL_ID = "11111111-1111-4111-8111-111111111111";
const TARGET = `http://localhost/api/evidence?control_id=${CONTROL_ID}`;
const ALLOWED_RESULTS = new Set(["pass", "fail", "na", "inconclusive"]);

describe("contract: GET /api/evidence?control_id <-> atlas GET /v1/evidence?control_id", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  test("golden pins the documented endpoint", () => {
    expect(golden.endpoint).toBe("GET /v1/evidence?control_id={id}");
  ...
  for (const variantName of Object.keys(golden.variants)) {
    test(`BFF passes provider variant "${variantName}" through verbatim`, async () => {
      mockCookieGet.mockReturnValue({ value: "test-bearer-692" });
