// Slice 136 — vitest coverage for the /api/risks/export BFF route.
//
...
function makeReq(query: string): Request {
  return new Request(`http://test/api/risks/export${query}`);
}

describe("GET /api/risks/export", () => {
  beforeEach(() => {
    cookieStore.clear();
    vi.restoreAllMocks();
  ...
  test("forwards bearer + query string verbatim on happy path", async () => {
    ...
    const csvBody =
      "id,title,category\n11111111-1111-1111-1111-111111111111,test risk,operational\n";
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(csvBody, {
        status: 200,
        headers: {
          "Content-Type": "text/csv; charset=utf-8",
          "Content-Disposition": `attachment; filename="risk-register_20260519.csv"`,
    ...
    expect(requestedURL).toBe(`http://atlas:8080/v1/risks/export${query}`);
    ...
    expect(res.headers.get("Content-Disposition")).toBe(
      `attachment; filename="risk-register_20260519.csv"`,
    );
  ...
  test("passes through 403 from backend (no eligible role)", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          error: "role does not grant risk/program-read access",
        }),
  ...
  test("passes through 429 + Retry-After header from backend (concurrency cap)", async () => {
    ...
    // Slice 145 inherited: Retry-After is load-bearing for clients.
