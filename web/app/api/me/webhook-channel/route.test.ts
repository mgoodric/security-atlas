// Slice 584 — vitest coverage for the /api/me/webhook-channel BFF route.
//
...
//   * The body is a boolean opt-in only — no user-supplied webhook URL
//     ever crosses this route (P0-543-2 / SSRF).
...
describe("GET /api/me/webhook-channel", () => {
  beforeEach(() => {
    cookieStore.clear();
    vi.restoreAllMocks();
  ...
  test("targets the webhook-channel upstream path", async () => {
    ...
    await GET();
    expect(fetchSpy.mock.calls[0][0]).toContain("/v1/me/webhook-channel");
  });
});

describe("PUT /api/me/webhook-channel", () => {
  beforeEach(() => {
    cookieStore.clear();
    vi.restoreAllMocks();
    process.env.ATLAS_HTTP_URL = "http://atlas:8080";
  ...
  test("returns 401 when session cookie is absent", async () => {
    const req = new Request("http://localhost/api/me/webhook-channel", {
      method: "PUT",
  ...
  test("forwards {enabled} body verbatim with bearer + PUT method", async () => {
    ...
    expect(call[0]).toContain("/v1/me/webhook-channel");
