// Slice 174 — vitest coverage for the /api/anchors/export BFF route.
//
...
function makeReq(query: string): Request {
  return new Request(`http://test/api/anchors/export${query}`);
}

describe("GET /api/anchors/export", () => {
  beforeEach(() => {
    cookieStore.clear();
    vi.restoreAllMocks();
  ...
  test("forwards bearer + query string verbatim on happy path", async () => {
    ...
    const csvBody =
      "id,scf_id,family,title\n11111111-1111-1111-1111-111111111111,IAC-06,iam,MFA\n";
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(csvBody, {
        status: 200,
        headers: {
          "Content-Type": "text/csv; charset=utf-8",
          "Content-Disposition": `attachment; filename="anchors_20260520.csv"`,
    ...
    expect(requestedURL).toBe(`http://atlas:8080/v1/anchors/export${query}`);
    ...
    expect(res.headers.get("Content-Disposition")).toBe(
      `attachment; filename="anchors_20260520.csv"`,
    );
  ...
  test("passes through 429 + Retry-After header from backend", async () => {
    cookieStore.set(SESSION_COOKIE, "test-bearer-token");
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(
        JSON.stringify({
