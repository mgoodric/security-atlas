// Slice 137 — vitest coverage for the /api/controls/export BFF route.
//
...
function makeReq(query: string): Request {
  return new Request(`http://test/api/controls/export${query}`);
}

describe("GET /api/controls/export", () => {
  beforeEach(() => {
    cookieStore.clear();
    vi.restoreAllMocks();
  ...
  test("forwards bearer + query string verbatim on happy path", async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(csvBody, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="controls_20260519.csv"`,
    ...
    expect(requestedURL).toBe(`http://atlas:8080/v1/controls/export${query}`);
    ...
    expect(res.headers.get("Content-Disposition")).toBe(
      `attachment; filename="controls_20260519.csv"`,
    );
