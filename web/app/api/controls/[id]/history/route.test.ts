// Slice 253 — vitest coverage for the per-control history BFF proxy.
//
// Pattern mirrors `app/api/controls/[id]/policies/route.test.ts`. The
// history endpoint surfaces `control_evaluations` ledger rows
// (newest-first, keyset-paginated) — the right-rail audit-log card
// renders the default page only.
...
describe("GET /api/controls/[id]/history", () => {
  ...
  test("returns 401 when atlas_jwt cookie is absent", async () => {
    const req = new Request(
      `http://localhost/api/controls/${CONTROL_ID}/history`,
    );
    const res = await GET(
  ...
  test("forwards bearer to upstream and passes 200 through", async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          ...
          history: [
            {
              evaluated_at: "2026-05-23T08:00:00Z",
    ...
    expect(calledURL).toContain(`/v1/controls/${CONTROL_ID}/history`);
