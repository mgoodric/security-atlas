...
`docs/coverage-audit-2026-05.md` measured `internal/oscal` at **41.4% merged
coverage** (unit-only: 41.4%), below the 70% aspirational target the
...
**Notes:** OSCAL ingest/export marshalling; pure-data unit-testable

## What ships in this slice

1. **New unit tests** under `internal/oscal/*_test.go` covering the
   uncovered branches identified by the slice 279 audit.
2. **Floor ratchet** in `cmd/scripts/coverage-thresholds.json` from
   the current `39` to `floor(measured - 2pp)` where
...
- [ ] **AC-1.** New unit tests for `internal/oscal` move its merged coverage
      to ≥ 70%.
...
- [ ] **AC-3.** Each new test file's first comment block names the
      ...
      designed to cover.
- [ ] **AC-4.** `coverage-thresholds.json` ratchets the `internal/oscal` floor
...
go tool cover -func=merged.cov | grep 'internal/oscal'
```
