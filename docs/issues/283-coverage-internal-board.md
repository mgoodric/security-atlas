# 283 — Coverage lift — `internal/board` to 70%+
...
- **P0-283-1.** Does NOT raise the `internal/board` floor without writing
  the unit tests that hit the new bar.
- **P0-283-2.** Does NOT lower any existing floor — every change to
  `thresholds` is monotonically ↑.
- **P0-283-3.** Does NOT modify `_STATUS.md` from inside this
