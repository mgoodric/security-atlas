# 290 — Coverage lift — `internal/api/controldetail` to 70%+
...
`docs/coverage-audit-2026-05.md` measured `internal/api/controldetail` at **29.3% merged
coverage** (unit-only: 25.0%), below the 70% aspirational target the
...
**Notes:** control-detail HTTP handler logic; needs handler tests

## What ships in this slice

1. **New unit tests** under `internal/api/controldetail/*_test.go` covering the
   uncovered branches identified by the slice 279 audit.
2. **Floor ratchet** in `cmd/scripts/coverage-thresholds.json` from
   the current `25` to `floor(measured - 2pp)` where
...
- [ ] **AC-1.** New unit tests for `internal/api/controldetail` move its merged coverage
      to ≥ 70%.
...
- [ ] **AC-3.** Each new test file's first comment block names the
      ...
      designed to cover.
- [ ] **AC-4.** `coverage-thresholds.json` ratchets the `internal/api/controldetail` floor
...
- **P0-290-1.** Does NOT raise the `internal/api/controldetail` floor without writing
  the unit tests that hit the new bar.
- **P0-290-2.** Does NOT lower any existing floor — every change to
  `thresholds` is monotonically ↑.
- **P0-290-3.** Does NOT modify `_STATUS.md` from inside this
...
go tool cover -func=merged.cov | grep 'internal/api/controldetail'
```
