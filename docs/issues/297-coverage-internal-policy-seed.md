# 297 — Coverage lift — `internal/policy/seed` to 70%+
...
`docs/coverage-audit-2026-05.md` measured `internal/policy/seed` at **33.3% merged
coverage** (unit-only: 33.3%), below the 70% aspirational target the
...
**Notes:** policy-seed surface; partially excluded but the kept paths need unit lift

## What ships in this slice

1. **New unit tests** under `internal/policy/seed/*_test.go` covering the
   uncovered branches identified by the slice 279 audit.
2. **Floor ratchet** in `cmd/scripts/coverage-thresholds.json` from
   the current `31` to `floor(measured - 2pp)` where
...
- [ ] **AC-1.** New unit tests for `internal/policy/seed` move its merged coverage
      to ≥ 70%.
...
- [ ] **AC-3.** Each new test file's first comment block names the
      ...
      designed to cover.
- [ ] **AC-4.** `coverage-thresholds.json` ratchets the `internal/policy/seed` floor
...
- **P0-297-1.** Does NOT raise the `internal/policy/seed` floor without writing
...
go tool cover -func=merged.cov | grep 'internal/policy/seed'
```
