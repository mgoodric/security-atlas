# 300 — Coverage lift — `connectors/github/cmd/atlas-github` to 70%+
...
`docs/coverage-audit-2026-05.md` measured `connectors/github/cmd/atlas-github` at **15.1% merged
coverage** (unit-only: 15.1%), below the 70% aspirational target the
...
1. **New unit tests** under `connectors/github/cmd/atlas-github/*_test.go` covering the
   uncovered branches identified by the slice 279 audit.
2. **Floor ratchet** in `cmd/scripts/coverage-thresholds.json` from
   the current `13` to `floor(measured - 2pp)` where
...
- [ ] **AC-1.** New unit tests for `connectors/github/cmd/atlas-github` move its merged coverage
      to ≥ 70%.
...
- [ ] **AC-3.** Each new test file's first comment block names the
      ...
      designed to cover.
- [ ] **AC-4.** `coverage-thresholds.json` ratchets the `connectors/github/cmd/atlas-github` floor
...
- **P0-300-1.** Does NOT raise the `connectors/github/cmd/atlas-github` floor without writing
  the unit tests that hit the new bar.
- **P0-300-2.** Does NOT lower any existing floor — every change to
  `thresholds` is monotonically ↑.
- **P0-300-3.** Does NOT modify `_STATUS.md` from inside this
...
go tool cover -func=merged.cov | grep 'connectors/github/cmd/atlas-github'
```
