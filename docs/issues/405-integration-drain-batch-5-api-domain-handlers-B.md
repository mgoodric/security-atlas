# 405 — Integration-enrolment drain · batch 5 (api domain handlers B)

**Cluster:** infra
**Estimate:** 0.5-1d
**Type:** AFK
**Status:** `merged` (`600d39ff`, #924)
**Parent:** 390 (drain the 38-package integration-enrolment backlog)

> **Unblocked 2026-05-30** — dep #404 (drain batch 4) merged at `cbdbf50a`.

## Narrative

Drain batch 5 of slice 390. Enrols the next group of packages
...
Was `not-ready` until slice 404 (the prior drain batch) merged — batches run
in sequence so each PR's CI stays green and the failure-rate learned from
earlier batches informs the later ones. 404 is now merged, so this batch is `ready`.
...
- **#404** (prior drain batch) — must merge first; this batch flips to
  `ready` when 404 lands.

## Anti-criteria (P0)

- **P0-405-1.** Does NOT skip/delete a failing integration test to reach green.
- **P0-405-2.** Does NOT grow `KNOWN_UNENROLLED` — monotonic shrink only.
- **P0-405-3.** Enrols ONLY this batch's packages.
