# 282 — Coverage lift — `internal/eval` to 70%+
...
`docs/coverage-audit-2026-05.md` measured `internal/eval` at **67.2% merged
coverage** (unit-only: 31.4%), below the 70% aspirational target the
...
**Notes:** lift-target gap from slice 279; remaining surface is consumer.go (NATS subscriber); needs embedded NATS pattern or refactor

## What ships in this slice

1. **New unit tests** under `internal/eval/*_test.go` covering the
   uncovered branches identified by the slice 279 audit.
2. **Floor ratchet** in `cmd/scripts/coverage-thresholds.json` from
   the current `65` to `floor(measured - 2pp)` where
...
- [ ] **AC-1.** New unit tests for `internal/eval` move its merged coverage
      to ≥ 70%.
...
- [ ] **AC-3.** Each new test file's first comment block names the
      ...
      designed to cover.
- [ ] **AC-4.** `coverage-thresholds.json` ratchets the `internal/eval` floor
...
- **P0-282-1.** Does NOT raise the `internal/eval` floor without writing
  the unit tests that hit the new bar.
- **P0-282-2.** Does NOT lower any existing floor — every change to
  `thresholds` is monotonically ↑.
- **P0-282-3.** Does NOT modify `_STATUS.md` from inside this
...
go tool cover -func=merged.cov | grep 'internal/eval'
```
