# 283 — Coverage lift — `internal/scope` to 70%+
...
`docs/coverage-audit-2026-05.md` measured `internal/scope` at **35.3% merged
coverage** (unit-only: 35.0%), below the 70% aspirational target the
...
**Notes:** scope-cell predicate logic; pure-Go unit-testable; second-tier gap

## What ships in this slice

1. **New unit tests** under `internal/scope/*_test.go` covering the
   uncovered branches identified by the slice 279 audit.
2. **Floor ratchet** in `cmd/scripts/coverage-thresholds.json` from
   the current `33` to `floor(measured - 2pp)` where
...
- [ ] **AC-1.** New unit tests for `internal/scope` move its merged coverage
      to ≥ 70%.
...
- [ ] **AC-3.** Each new test file's first comment block names the
      ...
      designed to cover.
- [ ] **AC-4.** `coverage-thresholds.json` ratchets the `internal/scope` floor
...
- **P0-283-1.** Does NOT raise the `internal/scope` floor without writing
...
go tool cover -func=merged.cov | grep 'internal/scope'
```
