# 404 — Integration-enrolment drain · batch 4 (api domain handlers A)
...
> **Unblocked 2026-05-30** — dep #403 (drain batch 3) merged at `afa262ea`.

## Narrative

Drain batch 4 of slice 390. Enrols the next group of packages
...
Was `not-ready` until slice 403 (the prior drain batch) merged — batches run
in sequence so each PR's CI stays green and the failure-rate learned from
earlier batches informs the later ones. 403 is now merged, so this batch is `ready`.
...
- **#403** (prior drain batch) — must merge first; this batch flips to
  `ready` when 403 lands.

## Anti-criteria (P0)

- **P0-404-1.** Does NOT skip/delete a failing integration test to reach green.
- **P0-404-2.** Does NOT grow `KNOWN_UNENROLLED` — monotonic shrink only.
- **P0-404-3.** Enrols ONLY this batch's packages.
