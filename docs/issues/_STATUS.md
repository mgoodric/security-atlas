...
**Last reconciled:** 2026-05-20 (batch 85 claim-stake · 187 + 155 → in-progress)
...
| 155 | Questionnaire feature design + build (CAIQ/SIG/HECVAT response)                       | `in-review` | feature/155-questionnaire-tracer                        | gh#433 | 2026-05-20 | —          | **scope locked 2026-05-20 by maintainer to TRACER-BULLET: Excel import only + manual authoring + AnswerLibrary skeleton + PDF export.** Tracer-bullet shipped: 4-table migration · questionnaires/excel/library/pdf/store/api packages · 7 routes · 14 tests · 68/68 PRD ISCs complete · D1 excelize/v2 with 5MB cap · D2 AnswerLibrary updated_at DESC ranking · D3 header-row heuristic + canonical aliases · D4 chromedp PDF reuse · D5 unmappable rows `needs_mapping` (never reject upload). CSV / JSON / Word import + HECVAT bundled templates + vendor portal + AI-assist all DEFERRED as spillovers (7 candidates enumerated in decisions log for future filing). · filed 2026-05-18 from comprehensive front-end gap audit · backend/frontend · JUDGMENT · canvas §4.6 commits to v1 ship                                                                                                                                                                                                                                    |
...
| 187 | OAuth AS scaffolding — JWT signing + JWKS + OIDC discovery                            | `in-review` | backend/187-oauth-as-scaffolding                        | gh#432 | 2026-05-20 | —          | filed 2026-05-20 as foundation slice for the auth-substrate-v2 spine (OQ #21 Reading D resolution) · Backend/Auth · JUDGMENT · 2-3d · 20 ACs · 11 P0 anti-criteria · threat-model verdict: has-mitigations · ships keystore + tokensign packages + JWKS endpoint + OIDC discovery doc + JWT claim types + ADR-0003 · ES256 + go-jose/v4 + filesystem keystore @ mode 0600 + multi-key JWKS + 4 untouched 501-stub /oauth/\* routes · 20/20 ACs pass · 11/11 P0 anti-criteria honored · spine continues: 188 (`/oauth/token` + token-exchange) · 189 (PKCE + frontend OAuth) · 190 (JWT validation middleware + R2 + revocation) · 191 (SDK migration ×4 + CLI device-code) · 192 (multi-tenant switch via token-exchange; closes slice 141 success criterion)                                                                                                                                                                                                                                                                          |
