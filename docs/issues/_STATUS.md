...
**Last reconciled:** 2026-05-14 (batch 25 claim-stake — 030 + 065 + 067 → in-progress · 59/67 on main)
...
| 030 | OSCAL SSP + POA&M export pipeline                      | `in-progress` | audit/030-oscal-ssp-poam-export                      | —      | 2026-05-14 | —          | batch 25 · JUDGMENT · sole spine-toucher (`pyproject.toml`) · lands first Python (`oscal-bridge/`) + gRPC bridge + cosign · deps 008/012/017/018/026/028 merged · sole critical-path bottleneck                                                                                                                                                                                                                                                        |
...
| 065 | self-host bundle P0 fixes (slice 037 follow-up)        | `in-progress` | infra/065-self-host-bundle-p0-fixes                  | —      | 2026-05-14 | —          | batch 25 · AFK · P0 follow-up to slice 037 · 5 first-deploy bugs (audit-writer RLS GUC, bootstrap `depends_on` deadlock, unguarded `CREATE TYPE`, `ALTER ROLE` on shared Postgres, missing `pgcrypto`) · head migration `20260511000000_extensions.sql` · deps 037/033/034 merged                                                                                                                                                                      |
...
| 067 | Risk-hierarchy backend read endpoints                  | `in-progress` | risk/067-risk-hierarchy-backend-endpoints            | —      | 2026-05-14 | —          | batch 25 · AFK · no migration · fills slice 056's placeholders: `GET /v1/org_units?include_risk_counts=true`, `GET /v1/risks/theme-heatmap` (themes × org_units aggregation), per-cell contributing-risk filter, richer `/v1/decisions` filters · deps 052/053/054/055 merged · 040→066 pattern                                                                                                                                                        |
...
**Two slices ready** (038, 058) — 030/065/067 are in batch 25. Both remaining carry a `justfile` spine touch (038 possible, 058 definite), so they cannot co-batch. Each is a natural solo or paired-with-a-newly-unblocked-slice batch once batch 25 lands.

## In-flight (3 worktrees building)

Batch 25 — created after the claim-stake PR merges:

- **030** — `audit/030-oscal-ssp-poam-export` · `in-progress` since 2026-05-14 · JUDGMENT · sole spine-toucher (`pyproject.toml`)
- **065** — `infra/065-self-host-bundle-p0-fixes` · `in-progress` since 2026-05-14 · AFK · head migration only
- **067** — `risk/067-risk-hierarchy-backend-endpoints` · `in-progress` since 2026-05-14 · AFK · no migration
