...
**Last reconciled:** 2026-05-18 (out-of-band fix: slice 122 implementation — cascade unblock for #234/#259/#262/#264)
...
| 117 | StepSecurity Harden-Runner (audit mode)                                | `in-progress` | infra/117-stepsecurity-harden-runner                    | —      | 2026-05-17 | —          | batch 44 claim-stake · solo-pick · AFK · 0.5d · Apache 2.0 action · free Community Plan covers OSS public repos on GitHub-hosted runners · adds `step-security/harden-runner@v2` (SHA-pinned) as first step of every job in every `.github/workflows/*.yml` with `egress-policy: audit` + `disable-sudo: true` · CONTRIBUTING.md "Local CI parity" updated with audit-mode contributor note · companion slice 118 (block-mode promotion) filed by 117's engineer per AC-5, `not-ready`, 2-week audit-soak gate · no technical deps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 121 | atlas OTel SDK (traces + metrics + Go runtime telemetry)               | `ready`       | docs/106-atlas-otel-sdk                                 | gh#235 | —          | —          | renumber from 106 (slot collision — `docs/issues/106-evidence-list-backend-extension.md` shipped via PR #240 at `860c10a`) · infra (observability) · AFK · 2.5d · 24 ACs · OTel SDK init (TracerProvider + MeterProvider + propagators, env-var-configured, no-op when `OTEL_EXPORTER_OTLP_ENDPOINT` unset) + HTTP/DB/NATS spans + Go runtime metrics + opt-in `/metrics` fallback · companion to PR #234 (observability deploy bundle) · no technical deps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 122 | Seed-harness `api_keys` idempotency fix (slice 082 follow-up)          | `in-review`   | infra/122-seed-harness-api-keys-idempotency             | TBD    | 2026-05-18 | —          | out-of-band fix · maintainer requested cascade unblock for #234/#259/#262/#264 · root cause = parallel-worker race on DELETE-then-INSERT pattern (Playwright defaults to multi-worker) · fix: add `ON CONFLICT (token_hash) DO NOTHING` to the `ensureApiKey()` INSERT in `web/e2e/seed.ts` · all workers insert deterministic identical content so DO NOTHING is correct semantics · slice file cherry-picked from PR #259's branch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 124 | Unified audit-log aggregation API                                      | `ready`       | —                                                       | —      | —          | —          | filed 2026-05-17 via /idea-to-slice · backend (multi-tenancy) · AFK · 2d · 16 ACs · UNION ALL across 9 audit-log tables · `GET /v1/admin/audit-log/unified` · admin OR auditor OPA role gate · 90-day window + 1000-row cursor pagination · idx `(tenant_id, occurred_at DESC)` migration · STRIDE: P0-A4 (no BYPASSRLS) + P0-A5 (no tenant_id param) + AC-3 (tenancy.ApplyTenant) + AC-9 (per-table integration test) are the load-bearing tenant-isolation chain · no deps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 125 | Frontend `/audit-log` page                                             | `not-ready`   | —                                                       | —      | —          | —          | filed 2026-05-17 via /idea-to-slice · spillover from 124 · frontend · AFK · 1-2d · gated on 124 · consumes `GET /v1/admin/audit-log/unified` · filters (from/to/actor/kind) + cursor infinite-scroll + admin route guard + BFF route + Playwright e2e                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 126 | External audit-log sink (tamper-evident retention)                     | `not-ready`   | —                                                       | —      | —          | —          | filed 2026-05-17 via /idea-to-slice · spillover from 124 · infra (observability) · JUDGMENT · 1.5d · gated on 124 (+ 121 if option (c) picked) · engineer picks 1 of 4 sink mechanisms (JSONL/syslog/OTel-via-collector/S3-cosign) · maintainer's lean = OTel logs via Collector · async fan-out from every domain audit-log INSERT · backpressure to fallback table (no silent drops) · per-record integrity proof                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 119 | Fix recurring `port 3000 already in use` Playwright e2e flake          | `in-progress` | infra/119-playwright-port-3000-ci-race-fix              | —      | 2026-05-16 | —          | batch 43 claim-stake · solo-pick · AFK · 0.5d (diagnosis-heavy) · `Frontend · Playwright e2e` IS in `.github/branch-protection.json` required_status_checks → flake blocks STEP-7 auto-merge on every Dependabot PR + blocks PR #234 (observability bundle) per user request · diagnose root cause (likely `web/playwright.config.ts` `reuseExistingServer:true` + CI environment interaction) · validate via 3 consecutive clean runs + 1 canary Dependabot PR re-run (151/152/153/154/156/158/159 are all currently UNSTABLE on this exact failure) · unblocks slice 116 gating condition · no deps                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 120 | Audit and remove phantom (unused) dependencies across all manifests    | `in-progress` | quality/120-phantom-dependencies-audit                  | —      | 2026-05-17 | —          | batch 45 claim-stake · solo-pick · JUDGMENT · 1-2d · AFK · ships `scripts/audit-deps.sh` (USED / USED-VIA-CONFIG / USED-VIA-SCRIPT / PHANTOM classifier) + initial removal pass (one commit per ecosystem) + recurring-cadence mechanism (engineer's JUDGMENT among 4 options; maintainer's lean = PR-comment CI check on manifest changes) · 11 ACs · threat model with config-allowlist mitigation (P0-A4 load-bearing — don't nuke eslint/postcss plugins) · no deps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
