...
**Last reconciled:** 2026-05-22 (batch 105 claim-stake · 207 SOLO → in-progress + spec filed)
...
| 207 | Edge deploy channel: per-commit images + Watchtower-driven `atlas-edge` instance                  | `in-progress`                 | infra/207-edge-deploy-channel                           | —      | —                   | —          | batch 105 · SOLO · infra/deploy · AFK · 1d · 14 ACs · 7 P0 anti-criteria · extends `container-publish.yml` to build `:edge` + `:main-<sha7>` on push-to-main · NEW `deploy/docker/docker-compose.edge.yml` (separate postgres/keystore/minio prefix) · NEW `docs/operations/edge-deploy.md` runbook (Watchtower config + access control + migration safety + rollback) · enables fast iteration loop for the maintainer's `atlas-edge.home.gmoney.sh` instance; VersionFooter slice 072 already renders `v<version> · <short-commit>` so operator can identify running build · stable channel (`atlas.home.gmoney.sh` via release-please) untouched · slice 204's audit fleet becomes meaningfully runnable against edge once shipped                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
