...
| 065 | self-host bundle P0 fixes (slice 037 follow-up)        | `in-review` | infra/065-self-host-bundle-p0-fixes                  | gh#115 | 2026-05-14 | —          | batch 25 · AFK · 12/12 ACs · 7/7 P0 · fixes 6 bugs: audit-writer + `db_resolver.go` RLS-blind (both from slice 035), bootstrap `depends_on` deadlock, 18 `CREATE TYPE` guards + `schema_migrations` ledger, head migration `20260511000000_extensions.sql` (pgcrypto), non-superuser role bootstrap (`BYPASSRLS CREATEROLE`), bundled-mode initdb role chicken-and-egg · wired `internal/authz` tests into CI · `test-self-host-bundle` matrix job green both modes · 10 judgment calls in decisions log · AC-12 fixed in a fast-follow pass after both e2e modes initially failed |
