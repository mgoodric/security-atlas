...
**Last reconciled:** 2026-05-14 (batch 24 claim-stake — 056 + 066 + 031 → in-progress · 56/65 on main)
...
| 031 | Monthly board brief (templated, no LLM)                | `in-review` | board/031-monthly-board-brief                        | gh#108 | 2026-05-14 | —          | batch 24 · AFK · migration `_031` (board_briefs append-only pinned snapshot) · templated text/template narrative (no LLM) · PDF via existing chromedp path · POST/GET/list + .md + /pdf · 6/6 ACs · 3/3 P0 · deps 012/016/020 merged                                                                                                                                                                                                                   |
...
| 056 | Hierarchical risk dashboard view                       | `in-review` | frontend/056-hierarchical-risk-dashboard             | —      | 2026-05-14 | 2026-05-14 | batch 24 · AFK · `web/**`-only · three-panel view (org tree · theme heatmap · decision timeline) · deps 005/053/054/055 merged · AC-2/3/4/5/10 PARTIAL (backend-endpoint gaps + screenshot capture pend a live instance — see decisions log) · ship-gate CLEAR                                                                                                                                                                                         |
...
| 066 | Dashboard backend read endpoints                       | `in-review` | catalog/066-dashboard-backend-endpoints              | gh#109 | 2026-05-14 | —          | batch 24 · AFK · 8/8 ACs · 6/6 P0 · no migration · `internal/api/dashboard` · 4 read endpoints fill slice 040's placeholders (`GET /v1/frameworks/posture\|activity\|upcoming` + `?sort=residual,age` on `/v1/risks`) · `/v1/activity` reads slice-062 `admin_audit_log_v` evidence branch · additive `risk.ListSort` · 5 judgment calls in decisions log                                                                                              |
...
**Three slices ready** (030, 038, 058) — 031/056/066 are in batch 24. All three remaining carry a spine touch (030 `pyproject.toml`, 038 + 058 `justfile`) — at most one per batch, so the next batch is effectively N=1 unless new slices land. 030 (the OSCAL pipeline, 4-5d JUDGMENT) is the highest-value of the three and a natural focused solo batch.

## In-flight (3 worktrees building)

Batch 24 — created after the claim-stake PR merges:

- **056** — `frontend/056-hierarchical-risk-dashboard` · `in-progress` since 2026-05-14 · AFK · `web/**`-only
- **066** — `catalog/066-dashboard-backend-endpoints` · `in-progress` since 2026-05-14 · AFK · no migration
- **031** — `board/031-monthly-board-brief` · `in-progress` since 2026-05-14 · AFK · migration slot `_031`
