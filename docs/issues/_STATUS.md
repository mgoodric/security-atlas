...
**Last reconciled:** 2026-05-16 (batch 38 claim-stake · 103/106/107 → in-progress)
...
| 107 | `ready` → `in-progress` | branch `backend/107-policies-include-ack-rate` · AFK · 1d · spillover from 101 · `?include=ack_rate` extension mirroring slice 104 pattern                                             |
...
| 103 | /settings user-facing page (per slice 093 mockup)                   | `in-progress` | frontend/103-settings-page                              | —      | 2026-05-16 | —          | batch 38 · AFK · 2d · deps 093/005/034/035/051 all merged · 5 sections: profile + appearance + notifications + API tokens + active sessions · admin lives at /admin (design doc §4 non-negotiable) · API token plaintext-shown-once invariant is security-critical (P0-A2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| 106 | `GET /v1/evidence` extension (control_id optional + filters)        | `in-progress` | backend/106-evidence-list-backend-extension             | —      | 2026-05-16 | —          | batch 38 · backend · AFK · 1d · spillover from 099 · deps 013/016/033 merged · makes `control_id` query param optional + adds filters + surfaces `result` field on GET wire shape · also updates `web/lib/api.ts` `fetchEvidenceList` + `web/app/(authed)/evidence/page.tsx` to consume new shape                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 107 | `GET /v1/policies?include=ack_rate` extension                       | `in-progress` | backend/107-policies-include-ack-rate                   | —      | 2026-05-16 | —          | batch 38 · backend · AFK · 1d · spillover from 101 · deps 022/023/033 merged · adds `?include=ack_rate` query param mirroring slice 104 pattern · also updates `web/lib/api.ts` `fetchPoliciesList` + `web/app/(authed)/policies/page.tsx` to consume new shape                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |

## Ready set right now

| #   | Title                                              | Cluster       | Est (d) | Notes                                                                                                                  |
| --- | -------------------------------------------------- | ------------- | ------- | ---------------------------------------------------------------------------------------------------------------------- |
| 106 | `GET /v1/evidence` extension (control_id optional) | Backend / API | 1       | Spillover from 099. Make `control_id` optional + add `?kind=`/`?result=` filters + surface `result` on GET wire shape. |
| 107 | `GET /v1/policies?include=ack_rate` extension      | Backend / API | 1       | Spillover from 101. Mirrors slice 104 shape (per-row join, no fan-out).                                                |
...
**3 slices ready** (106, 107, 103 — ordered ascending by estimate). **Recommended next continuous-batch sweep:**

- **Batch 38**: 106 + 107 + 103 in parallel — two 1d backend slices + one 2d frontend, all file-disjoint (backend `internal/api/{evidence,policies}/` + frontend `web/app/(authed)/settings/`).
- After that batch lands, the slice-093 UI fill-in suite is **COMPLETE** (all 6 mockup-impl pages + 3 backend `?include=` joins on main) and the v2 backlog enters a true vacuum until 082/084/095 gates clear or new slices file.

Total wall-clock estimate: ~2 calendar days.
...
- PR pending — this batch-37 final-reconcile sweep. Touches `_STATUS.md` only. Awaits CI + maintainer review.
