...
**Last reconciled:** 2026-05-14 (batch 25 merged — 030 + 065 + 067 → merged · 62/68 on main)
...
| 032 | Quarterly board pack + investment-vs-coverage          | `ready`     | —                                                    | —      | —          | —          | **NEWLY UNBLOCKED** (batch 25) · deps 031 (batch 24) + 030 (batch 25) merged                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 068 | Schema-registry evidence_kind identifier fix           | `ready`     | —                                                    | —      | —          | —          | **NEW** (surfaced by slice 065's AC-12 e2e) · `DefaultSeed()` registers kinds with a `.v1` suffix; schema dirs + control bundles + evidence-push use bare names + separate semver · fresh-deploy phase-6 control-bundle upload fails for every bare-named evidence_kind · pre-existing slice-014/010 bug · cluster `evidence-pipeline` · deps 014/010 merged                                                                                                                                                                                                          |

## Ready set right now

| #   | Title                                        | Cluster           | Est (d) | Notes                                                                                                                    |
| --- | -------------------------------------------- | ----------------- | ------- | ------------------------------------------------------------------------------------------------------------------------ |
| 032 | Quarterly board pack + investment-coverage   | board             | 2-3     | **newly unblocked** (batch 25) · deps 030 + 031 merged · likely `web/**` + board backend                                 |
...
| 068 | Schema-registry evidence_kind identifier fix | evidence-pipeline | 1-1.5   | **new** (from slice 065's AC-12) · `DefaultSeed` `.v1`-suffix mismatch · `internal/api/schemaregistry/` · no spine touch |

**Four slices ready** (032, 038, 058, 068). 068 (`internal/api/schemaregistry/`) and 032 (board reporting) are conflict-light; 038 + 058 each carry a `justfile` spine touch and cannot co-batch with each other. A conflict-free N=3 next batch: 068 + 032 + one of {038, 058}. 068 is the highest-leverage — it unbreaks fresh-deploy control-bundle upload (broken on `main` today) and would green slice 065's AC-12 self-host e2e job.

## In-flight (0 worktrees building)

Batch 25 fully merged — no worktrees building.
