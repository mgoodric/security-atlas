...
| 040 | Program dashboard view                                 | `in-review` | frontend/040-program-dashboard-view                  | gh#101 | 2026-05-14 | —          | batch 23 · AFK · `web/**`-only · 3/7 ACs full (AC-1/4/7) + freshness panel bound · AC-2/3/5/6 PARTIAL — 4 backend endpoint gaps inventoried (per-framework posture, activity archive, `sort=residual,age`, unified upcoming-rollup) · 6 panels + shared `dashboardProxy` BFF helper + `PanelCard` · decisions log committed · ship-gate CLEAR                                                                                                          |
...
| 064 | Control-detail backend read endpoints                  | `in-review` | controls/064-control-detail-backend-endpoints        | gh#102 | 2026-05-14 | —          | batch 23 · AFK · 8/8 ACs · 6/6 P0 · no migration · `internal/api/controldetail` · 4 read endpoints fill slice 041's placeholders (`GET /v1/evidence?control_id=`, `/controls/{id}/policies\|risks\|history`) · reuses slice-012 control→evidence resolution · keyset pagination · 7 judgment calls in decisions log                                                                                                                                    |
