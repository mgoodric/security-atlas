...
**Last reconciled:** 2026-05-21 (batch 91 claim-stake · 192 + 195 → in-progress)
...
| 141 | Multi-tenant login + tenant picker + persistent header switcher                                   | `not-ready`   | —                                                       | —      | —          | —          | **PARKED 2026-05-20 by OQ #21 Reading D resolution** — gated on slice 192 (multi-tenant switch via OAuth token-exchange). Original bespoke-session design retired; the data-model + bootstrap + R2 + switch all move into the auth-substrate-v2 spine (slices 187-192). Maintainer rationale: "no users yet; don't implement just to replace." Slice 192 fulfills 141's success criterion via `grant_type=urn:ietf:params:oauth:grant-type:token-exchange` and JWT-claim-based R2 middleware. Original 141 spec is preserved as historical context. · slice doc at `docs/issues/141-multi-tenant-login-and-switcher.md` retains original spec text · escalation context at `~/.claude/MEMORY/STATE/continuous-batch-escalation.md`                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| 192 | Multi-tenant switch via OAuth token-exchange + frontend tenant switcher (closes slice 141)        | `in-progress` | backend/192-multi-tenant-switch                         | —      | 2026-05-21 | —          | filed 2026-05-21 · auth-substrate-v2 spine slot 6 (FINAL) · Backend/Auth + Frontend · JUDGMENT · 2-3d · 23 ACs · 11 P0 anti-criteria · CLOSES slice 141 vCISO criterion (switch tenants mid-session without re-auth) · ships `<TenantSwitcher>` component + `GET /v1/me/tenants` + login picker for ≥2 tenants + bootstrap branch (first-install creates Default Tenant + grants super_admin) + membership-removed UX banner · HIDDEN when tenant_count==1 (canvas §11 #13) · honest eventual-eviction semantics documented · gate 191 merged at 8f0d265 (batch 90) · batch 91 parallel-pair with 195                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| 195 | Java SDK OAuth client_credentials helper                                                          | `in-review`   | sdk/195-java-oauth-client                               | gh#457 | 2026-05-21 | —          | filed 2026-05-21 · spillover from slice 191 · SDK · AFK · 1-2d · `sdk/java/` doesn't exist today; this slice creates it + translates the slice-191 Go/Python/TS OAuth client_credentials pattern to Java · batch 91 parallel-pair with 192                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
