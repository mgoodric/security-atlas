...
**Last reconciled:** 2026-05-14 (batch 26 claim-stake — 032 + 038 + 068 → in-progress · 62/68 on main)
...
| 032 | Quarterly board pack + investment-vs-coverage          | `in-progress` | board/032-quarterly-board-pack                          | —      | 2026-05-14 | —          | batch 26 · AFK · extends slice 031 board package · quarterly pack + investment-vs-coverage · migration slot `_032` (`board_packs`) · templated narrative (no LLM) · per-section approve UI · deps 030/031 merged                                                                                                                                                                                                                                                                                                                                                      |
...
| 038 | Helm chart for K8s                                     | `in-review`   | infra/038-helm-chart                                    | gh#124 | 2026-05-14 | —          | batch 26 · AFK · `deploy/helm/**` (Chart + values + values-production + 17 templates) + `.github/workflows/ci.yml` (helm lint+template job, slice-061 stub pattern) · pre-install migration hook reuses slice-065 bootstrap · 7/7 ACs · 8 judgment calls in decisions log                                                                                                                                                                                                                                                                                             |
...
| 057 | README screenshots + animated GIFs of core flows       | `not-ready`   | —                                                       | —      | —          | —          | waits on frontend views (040–043) · 040/041/042 merged, 043 still not-ready                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 068 | Schema-registry evidence_kind identifier fix           | `in-review`   | evidence-pipeline/068-schema-registry-evidence-kind-fix | gh#125 | 2026-05-14 | —          | batch 26 · AFK · grill corrected the fix direction (issue doc was inverted) — canonical convention per `EVIDENCE_SDK.md` §4.5 is `.v1`-suffixed kind + separate semver; the drift was the SOC 2 control bundles using bare names · aligns 26 `controls/soc2/*/control.yaml` evidence_kind refs to `.v1` · registry/connectors/SDK already correct, untouched · adds `internal/control/evidence_kind_drift_test.go` drift-guard · fixes fresh-deploy phase-6 control-bundle upload · greens slice 065's AC-12 e2e · no migration · deps 014/010 merged                 |
...
**One slice ready** (058) — 032/038/068 are in batch 26. 058 carries a `justfile` spine touch; it's a clean solo batch, or pairs with whatever batch 26's reconcile newly unblocks. Note: slice 069 (verification suite) has a `docs/issues/069-*.md` doc but no `_STATUS.md` row yet — register it in a reconcile when its deps are confirmed.

## In-flight (3 worktrees building)

Batch 26 — created after the claim-stake PR merges:

- **032** — `board/032-quarterly-board-pack` · `in-progress` since 2026-05-14 · AFK · migration slot `_032`
- **038** — `infra/038-helm-chart` · `in-progress` since 2026-05-14 · AFK · `deploy/helm/**`, no migration
- **068** — `evidence-pipeline/068-schema-registry-evidence-kind-fix` · `in-progress` since 2026-05-14 · AFK · no migration
