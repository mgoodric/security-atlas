...
**Last reconciled:** 2026-05-14 (batch 26 merged — 032 + 038 + 068 → merged · 65/69 on main)
...
| 043 | Board pack preview/export view                         | `in-review`   | frontend/043-board-pack-preview-view                    | gh#131 | 2026-05-15 | —          | batch 27 · AFK · `web/**` board-pack view per `Plans/mockups/board-pack.html` · no migration · no spine · 11 components in `web/components/board-pack/` · 2 BFF passthrough routes (binary-safe MD + PDF) · Templated v1 badge (no LLM in v1) · approver gate via slice-060 is_admin probe · published pack read-only · `web/package.json` untouched (conflict-safety w/ 069) · typecheck/lint/build/pre-commit/go-test all green · commit dc0d422                                                                                                                                                                                                 |
...
| 058 | User docs scaffold + 5 core pages                      | `in-review`   | infra/058-user-docs-scaffold                            | gh#133 | 2026-05-14 | —          | batch 27 · JUDGMENT · 10/10 ACs (AC-10 PARTIAL — slice-057 screenshot TODO placeholders, pre-cleared) · `docs-site/` mkdocs Material scaffold + 5 core pages (index/install/framework-setup/first-audit/board-reporting) · `justfile` `docs-serve`+`docs-build` recipes via `uv tool run` (no monorepo workspace pollution) · `.github/workflows/docs-publish.yml` (PR strict build + tag-only Pages deploy, least-privilege `GITHUB_TOKEN`) · PR template "Docs impact" section · global ship-gate `DOCS-01` advisory · Plans/canvas/11 item 20 + CLAUDE.md row resolves docs-generator OQ · 11 judgment calls in decisions log · commit a846cc0  |
...
| 069 | Verification suite (Playwright + vitest + Go coverage) | `in-review`   | infra/069-verification-suite                            | gh#132 | 2026-05-15 | 2026-05-15 | batch 27 · AFK · 22/22 ACs (AC-5 partial — first-run e2e green pending seed-data harness; documented in PR) · 9/9 P0 · no migration · 14 vitest tests + 37 enumerated Playwright tests + Go coverage gate (66 packages floored at measured-minus-2pp) · 2 new required CI checks (10→12) · 2 judgment calls (D10 per-package floors over 60% blanket; D11 created CLAUDE.md testing-discipline section)                                                                                                                                                                                                                                            |

## Ready set right now

| #   | Title | Cluster | Est (d) | Notes                                                                                                                                              |
| --- | ----- | ------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| —   | —     | —       | —       | Ready set is empty — all three ready slices (043, 058, 069) claimed by batch 27. Next ready slices unlock when batch 27 merges (043 unblocks 057). |

**Ready set empty.** Batch 27 claimed the entire ready set (043 + 058 + 069). Slice 057 (README screenshots) unblocks once 043 merges — it is the only `not-ready` row and 043 is its last unmerged dep.

## In-flight (3 worktrees building — batch 27)

| #   | Branch                                 | Worktree                | Type     |
| --- | -------------------------------------- | ----------------------- | -------- |
| 043 | `frontend/043-board-pack-preview-view` | `../security-atlas-043` | AFK      |
| 058 | `infra/058-user-docs-scaffold`         | `../security-atlas-058` | JUDGMENT |
| 069 | `infra/069-verification-suite`         | `../security-atlas-069` | AFK      |
