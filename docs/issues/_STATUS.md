...
| 043 | Board pack preview/export view                         | `ready`     | —                                                       | —      | —          | —          | **NEWLY UNBLOCKED** (batch 26) · deps 005 + 032 merged                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| 069 | Verification suite (Playwright + vitest + Go coverage) | `ready`     | —                                                       | —      | —          | —          | doc landed via gh#120; registered to `_STATUS.md` in the batch-26 reconcile · deps 037/040/041/060 all merged · Playwright e2e + vitest + Go coverage gate over the `web/e2e/*.spec.ts` inventory already shipped by prior slices                                                                                                                                                                                                                                                                                                                                                                                                                  |

## Ready set right now

| #   | Title                                      | Cluster  | Est (d) | Notes                                                                                                                  |
| --- | ------------------------------------------ | -------- | ------- | ---------------------------------------------------------------------------------------------------------------------- |
| 043 | Board pack preview/export view             | frontend | 2-2.5   | **newly unblocked** (batch 26) · deps 005 + 032 merged · `web/**` board-pack view                                      |
| 058 | User docs scaffold + 5 core pages          | docs     | 3       | deps 005 + 050 merged · JUDGMENT-type (docs authorship) · `justfile` spine touch                                       |
| 069 | Verification suite (Playwright + coverage) | infra    | 2       | doc registered in batch-26 reconcile · deps 037/040/041/060 merged · touches `.github/workflows/` + `web/` test config |

**Three slices ready** (043, 058, 069). Conflict-light next batch: 043 (`web/**`) + 069 (`.github/workflows/` + test config) are disjoint and conflict-free; 058 carries a `justfile` spine touch (pairs with either, but at most one spine-toucher per batch). A clean N=3 next batch: 043 + 058 + 069.

## In-flight (0 worktrees building)

Batch 26 fully merged — no worktrees building.
