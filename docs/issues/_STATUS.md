...
**Last reconciled:** 2026-05-14 (batch 23 merged — 040 + 055 + 064 → merged · 56/65 on main)
...
| 056 | Hierarchical risk dashboard view                       | `ready`     | —                                                    | —      | —          | —          | **NEWLY UNBLOCKED** (batch 23) · deps 005/053/054/055 all merged                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 066 | Dashboard backend read endpoints                       | `ready`     | —                                                    | —      | —          | —          | **NEW** (from slice 040 decisions log) · fills slice 040's 4 placeholders: per-framework posture (`GET /v1/frameworks/posture`), activity-stream archive (`GET /v1/activity`), `sort=residual,age` on `GET /v1/risks`, unified upcoming-rollup · deps 008/012/015/016/019/021/023/024/028 merged · 041→064 pattern                                                                                                                                     |
...
| 056 | Hierarchical risk dashboard view  | frontend | 2.5     | **newly unblocked** (batch 23) · deps 005/053/054/055 merged · `web/**`-only               |
| 058 | User docs scaffold + 5 core pages | docs     | 3       | deps 005 + 050 merged · JUDGMENT-type (docs authorship) · `justfile` spine touch           |
| 066 | Dashboard backend read endpoints  | controls | 2-2.5   | **new** (from slice 040 decisions log) · fills slice 040's 4 endpoint placeholders         |

**Six slices ready** (030, 031, 038, 056, 058, 066). Spine-touch conflicts: 030 (`pyproject.toml`), 038 + 058 (`justfile`) — at most one of these three per batch. 031 + 066 are backend/read slices (httpserver.go mount-append + sqlc — known-safe coexistence); 056 is `web/**`-only (zero overlap). A conflict-free N=3 next batch: 056 + 066 + one of {031, 038, 058}.

## In-flight (0 worktrees building)

Batch 23 fully merged — no worktrees building.
