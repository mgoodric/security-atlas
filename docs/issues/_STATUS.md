...
**Last reconciled:** 2026-05-16 (slice 120 filed `ready` — phantom-dependency audit)
...
| 119 | Fix recurring `port 3000 already in use` Playwright e2e flake          | `ready`     | —                                                       | —      | —          | —          | surfaced 2026-05-16 in dep-review session · infra (CI hardening) · AFK · 0.5d (diagnosis-heavy) · `Frontend · Playwright e2e` IS in `.github/branch-protection.json` required_status_checks → flake blocks STEP-7 auto-merge on every Dependabot PR · diagnose root cause (likely `web/playwright.config.ts` `reuseExistingServer:true` + CI environment interaction) · validate via 3 consecutive clean runs + 1 canary Dependabot PR re-run (151/152/153/154/156/158/159 are all currently UNSTABLE on this exact failure) · unblocks slice 116 gating condition · no deps                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
