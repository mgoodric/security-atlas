...
**Last reconciled:** 2026-05-22 (batch 106 reconcile · 208 → merged)

## Drift detected — 2026-05-22 (batch 106 reconcile · 208 → merged)

Continuous-loop batch 106 closeout. Slice 208 (Next.js rewrites for `/v1/*`, `/health`, `/metrics`) merged at `3967ff9` via PR #510. 5 files / +231/-3. All required CI checks PASS including Frontend Playwright e2e (the regression test surface this slice was designed to lock in).

Engineer eng208 delivery — 8/8 ACs + 6 P0s + D1-D3 documented:

- **D1 local-dev fallback** to `http://localhost:8080` with `console.warn` emit when `ATLAS_HTTP_URL` is unset — docker-compose templates already set the env so production deployments are silent; only `next dev` outside the compose bring-up sees the warning.
- **D2 scope discipline** — three exact path prefixes (`/v1/:path*`, `/health`, `/metrics`); zero coverage of `/api/*` (Next.js BFF route handlers — server-side credential handling stays Next.js-managed) or `/oauth/*` (Next.js OIDC callback — server-side cookie writing stays Next.js-managed).
- **D3 CI-delta scan** — engineer initial push exposed two adjacent latent gaps Playwright caught immediately: (a) `web/proxy.ts` slice-206 exemption omitted `/health` (proxy 307'd to `/login` before the rewrite could fire); (b) `internal/auth/jwtmw/middleware.go` `DefaultCookieName` was still `atlas_session` (slice 189 legacy) while the slice-198 OIDC callback writes the JWT to `atlas_jwt` (`ATLAS_JWT_COOKIE`). Orchestrator fix-forward `d714eb5` corrected both — the Playwright `/v1/me` and `/health` regression specs the engineer wrote were the canary; the test surface caught the auth-substrate-v2 cookie-name drift before merge.

After-merge cascade (live):

- Container-publish workflow auto-fires; `:edge` image rebuilds (~5-10 min).
- Watchtower on Unraid auto-pulls within 5-min poll interval.
- `atlas-edge.home.gmoney.sh` dashboard becomes functional end-to-end (was broken since first provisioning — `/v1/*` had never reached the atlas Go backend in any deployment topology before this slice).
- Next `release-please` cut (v1.16.0) packages this fix for stable `atlas.home.gmoney.sh`.

Net result: the dashboard's browser-side data layer now works in every deployment topology (docker-compose / K8s / Cloudflare Tunnel / NPM / Caddy / etc.) with zero operator path-routing config. The maintainer-authored NPM path-routing on `atlas-edge.home.gmoney.sh` is now harmless redundant overhead; can be retired without functional impact.

| Row | Transition               | Evidence                                                                                                                                       |
| --- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 208 | `in-progress` → `merged` | PR [#510](https://github.com/mgoodric/security-atlas/pull/510) squashed at commit `3967ff9` · 8/8 ACs PASS · Playwright regression specs green |

## Drift detected — 2026-05-22 (batch 105 reconcile · 207 → merged)

Continuous-loop batch 105 closeout. Slice 207 (Edge deploy channel for `atlas-edge.home.gmoney.sh`) merged **CLEAN** at `bf0aa19` via PR #506. 9 files / +1702/-3. All required CI checks PASS.

Engineer eng207 delivery — 14/14 ACs + 7 P0s + D1-D7 documented:

- **D1 single workflow** (extends `container-publish.yml`; preserves harden-runner + SHA-pin discipline; minimal drift surface)
- **D2 hybrid retention** with safety invariant — jq filter only deletes versions whose tag list is composed ENTIRELY of `main-<sha7>` (preserves `:edge` + release tags unconditionally)
- **D3 full isolation** in `docker-compose.edge.yml` (separate volume / keystore / MinIO prefix / ports / Docker network)
- **D4 5-min Watchtower interval**
- **D5 ldflag parity verified** by inspection — no parallel goreleaser step needed; existing build-args path already injects commit
- **D6 doc-only access control** with 4 options (Tailscale recommended)
- **D7 honest CI-delta scan** — 132 SHA-pinned `uses:` across 7 files verified; pre-commit + actionlint + shellcheck all clean

Engineer also surfaced + documented a benign pre-existing `:latest` bug (used `is_default_branch` which was always false on release events).

Spillover candidates surfaced (NOT filed; track for future): edge-equivalent smoke test for new compose; `EDGE_ATLAS_TAG` env var for rollback ergonomics; Watchtower → Slack/Discord webhook; retention-defaults retune after 3 months usage data.

**release-please cut v1.15.0** at `7f2d140` autonomously alongside this merge — packages slice 206's BFF cookie fix. **The stable channel `atlas.home.gmoney.sh` can now be redeployed to v1.15.0 to fix the user-reported login loop.**

| Row | Transition               | Evidence                                              |
| --- | ------------------------ | ----------------------------------------------------- |
| 207 | `in-progress` → `merged` | gh#506 squash-merged at `bf0aa19` (2026-05-22, CLEAN) |

## Drift detected — 2026-05-22 (batch 105 claim-stake · 207 SOLO → in-progress + spec filed)

Continuous-loop batch 105 SOLO. Maintainer-authorized pickup of slice 207 — edge deploy channel for `atlas-edge.home.gmoney.sh`. Enables continuous deploy: every push to `main` ships an image that Watchtower auto-pulls on Unraid. Closes the manual-tag-release-redeploy cycle that slowed slice 206's iteration.

Maintainer scope decision (locked into spec): two parallel Unraid stacks — `atlas.home.gmoney.sh` (versioned, release-please) + `atlas-edge.home.gmoney.sh` (edge, per-commit). Separate databases. VersionFooter (slice 072) already shows `v<version> · <short-commit>` so operator can identify the running build — no UI work needed.

- **Slice 207** (Edge deploy channel + per-commit images + atlas-edge instance) — infra/deploy · AFK · 1d · 14 ACs · 7 P0 anti-criteria. Extends `container-publish.yml` to also build `:edge` + `:main-<sha7>` tags on push-to-main. Ships new `deploy/docker/docker-compose.edge.yml` template + new `docs/operations/edge-deploy.md` runbook covering separate-DB setup, Watchtower config, rollback procedure.

| Row | Transition            | Evidence                                                                                     |
| --- | --------------------- | -------------------------------------------------------------------------------------------- |
| 207 | (new) → `in-progress` | spec file + canonical row added; worktree `infra/207-edge-deploy-channel` cut from `8462d59` |

## Drift detected — 2026-05-22 (batch 104 reconcile · 206 → merged)

Continuous-loop batch 104 closeout. Slice 206 (BFF auth cookie migration from `sa_session_token` to `atlas_jwt`) merged **CLEAN** at `be6a5cc` via PR #503. 23 files / +459/-141. All required CI checks PASS (including codecov/patch + Playwright OIDC→/dashboard regression).

This **fixes the v1.14.0 deployment-blocking login-loop**. User can pull main + rebuild + redeploy to verify.

Engineer eng206 delivery:

- **D1 cookie shape**: `Authorization: Bearer <jwt>` header. Verified `web/lib/api/bff.ts` already builds this from the cookie value, and backend `jwtmw.extractJWT` accepts `Bearer eyJ...` shape. Zero call-site refactor — only the constant value changed.
- **D2 exemption set**: added `startsWith("/v1/")` + exact `=== "/metrics"`. Preserved slice 092's exact-equality discipline. Skipped `/api/health` (doesn't exist).
- **D3 CI-delta scan HONEST** (per slice 202 precedent; NOT slice 143/205 false-positive class): `npm run lint` clean; 74 files / 738 tests pass including 7 new proxy cases; `npm run build` exit 0; pre-commit all 14 hooks pass. **Pattern correction held** after the 143/205 regressions.

11/11 ACs delivered. AC-8 Playwright regression confirms the OIDC → /dashboard flow no longer infinite-loops. Operator note in CHANGELOG: existing users must log in once after deploy (stale `sa_session_token` cookies are inert).

- **Slice 206** merged.

| Row | Transition               | Evidence                                                                                           |
| --- | ------------------------ | -------------------------------------------------------------------------------------------------- |
| 206 | `in-progress` → `merged` | gh#503 squash-merged at `be6a5cc` (2026-05-22, CLEAN — all required checks PASS including codecov) |

## Drift detected — 2026-05-22 (batch 104 claim-stake · 206 SOLO → in-progress + spec filed)

Continuous-loop batch 104 SOLO. Maintainer-authorized urgent pickup of slice 206 — fixes the v1.14.0 deployed-product login-loop bug (user can't get past the login screen because the BFF reads the wrong cookie name).

Diagnosis (live via direct `curl http://192.168.1.246:3015/v1/me` after maintainer opened LAN firewall to sandbox):

- OIDC callback writes `atlas_jwt` cookie ✅
- `web/proxy.ts:99` checks `SESSION_COOKIE` (= legacy `sa_session_token`) ❌
- Cookie not present → 307 to /login → infinite login loop
- 30+ BFF call sites also read `sa_session_token` for backend-cookie-pass-through — all broken
- Slice 197 retired the BACKEND bearer middleware but the BFF cookie migration was never shipped

- **Slice 206** (BFF auth cookie migration from `sa_session_token` to `atlas_jwt`) — frontend/auth · AFK · 1d · 11 ACs · 6 P0 anti-criteria. Spec filed inline with claim-stake (the bug is deployment-blocking; spec + claim-stake bundled to ship faster). Changes `SESSION_COOKIE` constant value in `web/lib/auth.ts` (back-compat alias — all 30+ import sites compile unchanged); extends `web/proxy.ts` exemptions for `/v1/*` and `/metrics`; updates BFF call sites that pass cookie value to Go backend (engineer picks D1: keep cookie shape or migrate to `Authorization: Bearer`); updates proxy.test.ts + related vitest specs; Playwright regression for the OIDC → /dashboard flow.

| Row | Transition            | Evidence                                                                                         |
| --- | --------------------- | ------------------------------------------------------------------------------------------------ |
| 206 | (new) → `in-progress` | spec file + canonical row added; worktree `frontend/206-bff-cookie-migration` cut from `98c1cb7` |

## Drift detected — 2026-05-22 (batch 103 reconcile · 203 + 205 → merged)

Continuous-loop batch 103 closeout. Both slices shipped end-to-end.

- **Slice 203** (Dark-mode stylesheet wiring) merged **CLEAN** at `d7a0b10` via PR #499. 8 files / +897/-13. Inline early-paint script in `<head>` (D1 approach picked, no `next-themes` dep), `suppressHydrationWarning` on `<html>`, new `ThemeClassSync` component for runtime reactivity, 17 vitest tests + Playwright AC-12. D2 flagged board-pack components + exceptions page + control coverage table as having hardcoded colors that bypass CSS variables (future spillover candidates).
- **Slice 205** (Comprehensive demo seed dataset) merged UNSTABLE-codecov-only at `072f9a7` via PR #500. 14 files / +3753 total. New `internal/demoseed/` package + new `atlas-cli demo seed` + new `atlas-cli demo teardown` companion (D6 shipped) + new migration `users.demo_only` flag + integration tests + decisions log D1-D9 + new doc. 23/23 ACs · live smoke run produced expected row counts (50 controls / 20 risks / 200 evidence / 12 distinct evidence_kinds / 1 frozen audit period).

**Engineer regression repeats**: eng205's D7 CI-delta scan claimed clean but missed 7 golangci-lint findings (errcheck × 3, QF1001 De Morgan, SA4017 unused HasPrefix, 2 unused symbols). Orchestrator fix-forward iteration 1 (`eec3333`) fixed 6 of 7; iteration 2 (`5d7c9e7`) corrected the De Morgan transform (staticcheck wanted the pure-positive form, not the outer-negated form). Same regression class as slice 143's D8 false-positive — pattern still recurring in JUDGMENT slices despite slice 202's correction. Capture in feedback memory.

**GitGuardian false-positive**: PR #500 hit GitGuardian "fail" twice in 1-second checks before passing on the third push (the orchestrator's De Morgan fix). No actual secret in the diff per exhaustive local scan (no vendor prefixes, no high-entropy strings, no embedded credentials). Likely a transient webhook state from the rebase. Documented for future reference.

- **Slice 203** + **Slice 205** both shipped.

| Row | Transition               | Evidence                                                                                                                        |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------- |
| 203 | `in-progress` → `merged` | gh#499 squash-merged at `d7a0b10` (2026-05-22, CLEAN)                                                                           |
| 205 | `in-progress` → `merged` | gh#500 squash-merged at `072f9a7` (2026-05-22, UNSTABLE-codecov-only; lint regression corrected by 2 orchestrator fix-forwards) |

## Drift detected — 2026-05-22 (batch 103 claim-stake · 203 + 205 PARALLEL → in-progress)

Continuous-loop batch 103 PARALLEL. Maintainer authorized pickup of 203 (dark-mode CSS wiring) + 205 (demo seed dataset) from the 3 ready spillovers. Slice 204 (UI audit fleet) explicitly deferred — operationally blocked on v1.14.0 500-error class.

Process note: slice 205's canonical row was NOT added when the spec was filed via `/idea-to-slice` (PR #497 was docs-only). Added here in the claim-stake. This is a minor `/idea-to-slice` workflow gap — the skill writes the spec file but not the canonical `_STATUS.md` row; the next claim-stake or spec-only-with-row PR closes the gap.

- **Slice 203** (Dark-mode stylesheet wiring — theme selection actually themes the UI) — frontend · AFK · 0.5d · 8 ACs · root cause = slice 170 explicitly deferred dark-mode CSS wiring (settings/page.tsx:710-714 comment); slice 176's logo reads `data-theme` but nothing writes `class="dark"` so `.dark { --background: ... }` rules never apply.
- **Slice 205** (Comprehensive demo seed dataset — showcase tenant + evidence across all primitives) — backend (seed/fixtures) + quality (docs) · JUDGMENT · 3d · 23 ACs · new opt-in CLI command `atlas-cli demo seed --tenant-slug=<slug>` populates one polished tenant with ~50 controls / ~20 risks / ~200 evidence records / 3+ frameworks. Env-var-gated; never auto-runs on bootstrap.

**Conflict analysis**: ZERO overlap. Slice 203 touches `web/app/(authed)/settings/page.tsx` + `web/app/layout.tsx` + new util module + vitest + Playwright. Slice 205 touches `internal/demoseed/` (new package) + `cmd/atlas-cli/demo.go` (new) + new migration + `docs/getting-started/demo-seed.md` + integration tests. Different directories at every layer. HIGH confidence (>95%) in file-conflict prediction.

| Row | Transition                          | Evidence                                                                         |
| --- | ----------------------------------- | -------------------------------------------------------------------------------- |
| 203 | `ready` → `in-progress`             | Worktree `frontend/203-dark-mode-stylesheet-wiring` cut from `28f16b2`           |
| 205 | (new canonical row) → `in-progress` | Worktree `backend/205-demo-seed-dataset` cut from `28f16b2`; canonical row added |

## Drift detected — 2026-05-22 (batch 102 reconcile · 116 + 175 → merged + spillovers 203 + 204 filed)

Continuous-loop batch 102 closeout. Both slices merged successfully PLUS two maintainer-surfaced spillovers filed from user feedback on the v1.14.0 release.

- **Slice 116** (Promote `Frontend · Playwright e2e` to required-check) merged **CLEAN** at `963e60e` via PR #494. 6 files / +209/-12. Engineer caught a spec misreading at D1: `ci.yml` left INTENTIONALLY UNCHANGED because the stub-twin uses the same name as the real job (slice-061 universal pattern); removing it would break docs-only PR mergeability forever.
- **Slice 175** (Control bundle history export — lineage including superseded) merged UNSTABLE-codecov-only at `fac1450` via PR #495. 18 files / +2725/-1. 7/7 ACs + 8 JUDGMENT decisions (D1 dropdown link group mirrors slice 137 D8; D2 duplicate sqlc query preserves slice 137's active-only discipline; D3 synthesised `superseded_at` from `updated_at`; D4 50K-row streaming-memory test scale; D5 cross-tenant FK-chain test; D6 plural action name; D7 chi literal route; D8 explicit CI-delta scan).

**NEW spillovers (filed from user feedback on the v1.14.0 deployed release):**

- **Slice 203** (Dark-mode stylesheet wiring) — `ready`. Surfaced when user reported "icon mostly invisible on light background + dark mode called out in settings but no actual implementation". Root cause: slice 170 explicitly deferred dark-mode CSS wiring (`web/app/(authed)/settings/page.tsx:710-714` comment: "v1 build does not ship dark-mode stylesheet tokens; the persistence is the contract"). Slice 176's `ThemeAwareLogo` reads `<html data-theme>` and swaps to dark-ink logo on selection, but nothing writes `class="dark"` so `.dark { --background: ... }` rules in `globals.css` never apply → dark-ink logo on still-light page = invisible. 0.5d AFK · 8 ACs · frontend-only.
- **Slice 204** (Comprehensive page-by-page UI parity audit — per-page agent fleet) — `ready` (BUT operationally blocked on resolving the v1.14.0 500-error class). Per maintainer 2026-05-22: "the UI is useless because almost every component returns a 500 error". Prior audits (154 + 178) shipped value but were narrow (settings only + harness first-pass) and didn't comprehensively cover all 11 mockup pages. This slice dispatches a per-page audit Agent fleet, files spillover slices per finding, aggregates results. 2d AFK · 15 ACs · read-only audit, zero production code change. Likely v1.14.0 500-class root-cause hypothesis (NOT in scope of slice 204): auth-substrate-v2 deployment-config gaps — see slice 204 Narrative.

| Row | Transition               | Evidence                                                                                                                         |
| --- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| 116 | `in-progress` → `merged` | gh#494 squash-merged to `main` at `963e60e` (2026-05-22, CLEAN)                                                                  |
| 175 | `in-progress` → `merged` | gh#495 squash-merged to `main` at `fac1450` (2026-05-22, UNSTABLE-codecov-only)                                                  |
| 203 | (new) → `ready`          | `docs/issues/203-dark-mode-stylesheet-wiring.md` filed via continuous-batch spillover from user feedback                         |
| 204 | (new) → `ready`          | `docs/issues/204-comprehensive-page-by-page-ui-parity-audit.md` filed via `/idea-to-slice` from maintainer-requested audit fleet |

## Drift detected — 2026-05-22 (batch 102 claim-stake · 116 + 175 PARALLEL → in-progress)

Continuous-loop batch 102 PARALLEL. Both freshly-promoted slices picked simultaneously after maintainer promoted them at PR #492. All four iteration guards passed.

- **Slice 116** (Promote `Frontend · Playwright e2e` to required-checks) — infra · AFK · 0.25d · flips `.github/branch-protection.json` + removes stub-twin job from `.github/workflows/ci.yml` + updates `web/e2e/README.md`.
- **Slice 175** (Control bundle history export — lineage including superseded versions) — backend + frontend · AFK · 1d · 7 ACs · NEW `GET /v1/controls/history/export` endpoint (17 columns: slice 137's 15 + `superseded_by` + `superseded_at`) + BFF route + Export button + cross-tenant isolation integration test + streaming-memory test.

**Conflict analysis**: zero overlap. 116 touches `.github/` + `web/e2e/` + `docs/audit-log/116-`; 175 touches `internal/api/controls/` + `web/app/controls/` + `web/app/api/controls/history/` + `internal/api/controls/*_test.go` + `docs/audit-log/175-`. Different directories at every layer. HIGH confidence (>95%) in file-conflict prediction.

| Row | Transition              | Evidence                                                                  |
| --- | ----------------------- | ------------------------------------------------------------------------- |
| 116 | `ready` → `in-progress` | Worktree `infra/116-promote-playwright-required-check` cut from `315e134` |
| 175 | `ready` → `in-progress` | Worktree `backend/175-control-bundle-history-export` cut from `315e134`   |

## Drift detected — 2026-05-22 (maintainer promotion · 116 + 175 → ready)

Maintainer promotes two slices from `not-ready` to `ready` to unblock the continuous-loop queue after batch 101's natural terminus.

- **Slice 116** (Promote `Frontend · Playwright e2e` to required-checks) — gate "all of 111-115 merged + ≥5 clean post-115 PR runs" is met per maintainer verification. The five Playwright runs across recent main commits (slices 142, 143, 198, 201, 202) all passed without flake; soak is sufficient. 0.25d AFK · flips `.github/branch-protection.json` + removes stub-twin job.
- **Slice 175** (Control bundle history export — lineage including superseded versions) — provisional gate was slice 028 (AuditPeriod + freezing primitive) shipping a sample-pull surface. Slice 028 merged at gh#58 (2026-05-13). Gate satisfied. 1d AFK · 7 ACs · backend + frontend.

| Row | Transition            | Evidence                                                                                                |
| --- | --------------------- | ------------------------------------------------------------------------------------------------------- |
| 116 | `not-ready` → `ready` | maintainer-confirmed Playwright soak (≥5 clean runs across recent merges including 142/143/198/201/202) |
| 175 | `not-ready` → `ready` | gate slice 028 merged at gh#58 (2026-05-13); provisional dependency satisfied                           |

## Drift detected — 2026-05-22 (batch 101 reconcile · 202 → merged)

Continuous-loop batch 101 closeout. Slice 202 (Self-host bundled mode: atlas service starts before migrations finish) merged **CLEAN** at `bd8c797` via PR #490. **First clean merge of the session** — no UNSTABLE on any check including codecov.

Engineer eng202 (a2e0922edb902e85d) delivered:

- Approach **option (c)** picked: CI-time harness staged bring-up (postgres + atlas-bootstrap first, poll `to_regclass('public.evidence_kind_schemas') IS NOT NULL` deterministic sentinel up to 4 minutes with bail-early on bootstrap non-zero exit, then bring up atlas + web against fully-migrated DB).
- Approach (a) `service_completed_successfully` rejected — documented deadlock with the atlas-bootstrap container structure.
- Approach (b) atlas-side healthcheck rejected — would require Go change (violates P0-A1) or distroless image change.
- Production docker-compose unchanged. Only the smoke-test harness `deploy/docker/test-self-host-bundle.sh` modified. CHANGELOG + decisions log added. 3 files / +289 / -2.
- **Self-host bundle · end-to-end (bundled) PASSED** on PR #490 — the load-bearing CI verification that the fix works.
- D2 CI-delta scan: clean (deploy/\*\* is in changes.code path filter; matrix job runs on this PR).

**Engineer-as-collaborator regression note (batch 99 D8 false-positive) appears CORRECTED here**: D2 in slice 202's decisions log explicitly verifies the path filter triggers. No false claims this batch.

**Self-host bundled stability classification update:** prior to slice 202, bundled mode had 1 fail (PR #484) + 2 pass (PR #485 ×2). Slice 202's harness fix introduces a deterministic sentinel poll — the previous failure mode (atlas service starts before migrations finish) is now impossible. Whether the original PR #484 failure was deterministic-but-rare or pure flake is no longer separable: the fix eliminates the failure shape regardless.

- **Slice 202** merged at `bd8c797` via PR #490. 3 files / +289 / -2. All required checks PASS; codecov/patch SKIPPING (shell-script + docs diff).

| Row | Transition               | Evidence                                                        |
| --- | ------------------------ | --------------------------------------------------------------- |
| 202 | `in-progress` → `merged` | gh#490 squash-merged to `main` at `bd8c797` (2026-05-22, CLEAN) |

## Drift detected — 2026-05-22 (batch 101 claim-stake · 202 SOLO → in-progress)

Continuous-loop batch 101 opened SOLO. Only ready slice in canonical queue is 202 (spillover filed during batch 100). All four iteration guards passed: queue has 1 ready candidate (202), zero human-authored open PRs, no `.STOP_LOOP` file, zero external commits.

- **Slice 202** (Self-host bundled mode: atlas service starts before migrations finish) — infra (self-host) · AFK · 0.5d · pure compose/CI timing fix. P0-A1 prohibits touching `internal/*` or `cmd/*`. Surfaced during batch 100 (slice 131 PR #484 self-host bundled FAIL); later PR #485 ran self-host bundled twice CLEAN suggesting timing-flake rather than deterministic race — but deterministic-signal gate is still better than docker-compose default `depends_on`.

| Row | Transition              | Evidence                                                                     |
| --- | ----------------------- | ---------------------------------------------------------------------------- |
| 202 | `ready` → `in-progress` | Worktree `infra/202-self-host-bundled-bootstrap-race-fix` cut from `847696a` |

## Drift detected — 2026-05-22 (batch 99 reconcile · 143 → merged)

Continuous-loop batch 99 closeout. Slice 143 (Create-tenant flow super_admin-gated) merged at `7332695` via PR #485 (UNSTABLE on codecov/patch informational only; all required checks PASS). **Engineer-as-collaborator pattern continues into batch 99**: 10/10 ACs + 6/6 P0s + 9 Go integration tests + 12 vitest BFF + 3 Playwright + 8 JUDGMENT decisions (D1-D8).

**The load-bearing D6 decision:** engineer identified that slice 034's GLOBAL UNIQUE on `users(idp_issuer, idp_subject)` contradicts slice 192's multi-tenant identity design (multiple `users` rows per OIDC principal, one per tenant — needed by the OAuth user_resolver's enumerateMemberships query). Slice 143's `creator_joins_as='admin'` branch is the FIRST surface that actually writes a second `users` row for the same OIDC principal in a different tenant, surfacing the latent contradiction. Migration relaxes the UNIQUE to per-tenant: `UNIQUE (tenant_id, idp_issuer, idp_subject)`. Cross-slice schema reconciliation, well-motivated, well-documented in the migration.

**Engineer regression noted (D8 false-positive):** engineer's decisions log claimed "D8: CI-delta scan clean" but the first CI run surfaced `internal/api/admintenants/handler.go:135: const defaultScopeCellEnv is unused (unused)` — a golangci-lint failure. Orchestrator fix-forward removed the 6-line const block (`ab61dbf`); engineer had hard-coded the JSON `{"environment":"prod"}` twice instead of using the const. This is a regression from batch 98's perfect delivery (slice 142). The engineer-as-collaborator pattern continues to mature, but trust-but-verify on D-decision claims remains warranted.

**Self-host bundled stability:** PR #485's two CI runs (pre-rebase + post-rebase) both passed self-host bundled. PR #484 (slice 131, two days ago) failed self-host bundled. Two consecutive passes after one fail suggests timing-flake rather than deterministic race — slice 202 spillover remains filed as documented latent risk but priority drops.

- **Slice 143** merged at `7332695` via PR #485. 17 files / 3393+/24-. UNSTABLE-codecov-only; all required checks PASS.

| Row | Transition               | Evidence                                                                              |
| --- | ------------------------ | ------------------------------------------------------------------------------------- |
| 143 | `in-progress` → `merged` | gh#485 squash-merged to `main` at `7332695` (2026-05-22, UNSTABLE-codecov/patch-only) |

## Drift detected — 2026-05-22 (batch 100 reconcile · 131 → merged + spillover 202 filed)

Continuous-loop batch 100 closeout. Slice 131 merged UNSTABLE on `Self-host bundle · end-to-end (bundled)` — failure pre-existed and was masked by path-filter; path-filter triggered on this PR's `internal/audit/notes/` change, surfacing the latent bundled-mode atlas/bootstrap race. Pure test-code diff cannot have caused a bootstrap race. Spillover slice 202 filed per Amendment 2 (matches slice 196 → 200 precedent).

- **Slice 131** merged at `29ab44d` via PR #484. 2-file diff (CHANGELOG.md + `internal/audit/notes/integration_slice029_test.go`); 37+/12-. All required checks PASS; only `Self-host bundle · end-to-end (bundled)` failed (non-required; spillover filed). **Side-benefit:** the canonical `tenancy.ApplyTenant` helper uses the correct GUC name `app.current_tenant` (not the broken `app.tenant_id` the tests had hard-coded), so the rewrite incidentally fixes a second latent GUC-name bug.
- **Slice 202** (NEW spillover, filed 2026-05-22) — Self-host bundled mode: atlas service starts before migrations finish. Status `ready`. 0.5d AFK. Compose/CI shape fix only; no production Go code touched. Distinct from slice 200's race (`pg_isready` vs `CREATE DATABASE`); this is `atlas service` vs `atlas-bootstrap migration apply`.

| Row | Transition               | Evidence                                                                                                                            |
| --- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| 131 | `in-progress` → `merged` | gh#484 squash-merged to `main` at `29ab44d` (2026-05-22, UNSTABLE — non-required self-host bundled; spillover 202 filed)            |
| 202 | (new) → `ready`          | `docs/issues/202-self-host-bundled-atlas-bootstrap-race.md` filed as spillover from slice 131 PR #484 surfacing latent bundled race |

## Drift detected — 2026-05-22 (batch 100 claim-stake · 131 PARALLEL → in-progress)

Continuous-loop batch 100, opened as a PARALLEL slot alongside batch 99 (eng143 already in flight in `backend/143-create-tenant-flow`). Slice 131 is a pure test-harness fix (no production code touched); zero overlap with slice 143's `internal/api/admin/*` + `web/app/admin/tenants/*` surfaces. Safe to run in parallel.

- **Slice 131** (Fix slice 029 integration tests' `SET LOCAL $1` syntax error) — backend (test infra) · AFK · 0.25d · pure test-harness fix. Replaces broken `SET LOCAL app.tenant_id = $1` parameterized call with the canonical `tenancy.ApplyTenant(ctx, tx)` pattern in two integration tests (`TestSlice029_AppendOnlyAtDBLayer` + `TestSlice029_DeleteRejectedAtDBLayer`). Discovered during slice 126 build; filed as spillover per Amendment 2.

| Row | Transition                  | Evidence                                                              |
| --- | --------------------------- | --------------------------------------------------------------------- |
| 131 | `not-ready` → `in-progress` | Worktree `backend/131-set-local-parameterized-fix` cut from `6676630` |

## Drift detected — 2026-05-21 (batch 99 claim-stake · 143 SOLO → in-progress)

Continuous-loop batch 99. Slice 143 (create-tenant flow) — gate `141 + 142` both ✅ merged.

- **Slice 143** (Create-tenant flow super_admin-gated) — backend/frontend/multi-tenancy · AFK · 1d · vCISO post-bootstrap tenant provisioning. NEW super_admin-gated create endpoint + management page extension.

| Row | Transition                  | Evidence                                                     |
| --- | --------------------------- | ------------------------------------------------------------ |
| 143 | `not-ready` → `in-progress` | Worktree `backend/143-create-tenant-flow` cut from `977e367` |

## Drift detected — 2026-05-21 (batch 98 reconcile · 142 → merged)

Continuous-loop batch 98 closeout. SOLO-pick of slice 142 landed end-to-end with EXCEPTIONAL engineer delivery (highest quality run in the 6-batch sequence).

- **Slice 142** (super_admin role full schema + management surface) merged at `ea674f6` via PR #480. UNSTABLE on codecov/patch (informational only); all required CI checks PASS.
- **12/12 ACs · 8/8 Go integration · 2/2 Playwright · 4/4 OPA matrix · 689/689 vitest** — full delivery.
- **6 JUDGMENT decisions** logged in `docs/audit-log/142-super-admin-role-surface-decisions.md`:
  - D1: advisory lock over `SELECT FOR UPDATE` because atlas_app lacks UPDATE table privilege (Postgres requires UPDATE for row-level locking even when caller has DELETE)
  - D2: audit log dual-write (platform-global `super_admin_audit_log` + tenant-scoped `me_audit_log` row) — preserves slice-124 RLS-routing contract while satisfying AC-8 intent
  - D3: OPA rule narrow (resource.type=admin + resource.id=super-admins only) — does not silently elevate super_admin to blanket tenant-write
  - D4: Playwright route-mocking over real DB seeding (handler-level Go test is the load-bearing real-DB assertion)
  - D5: slice-doc schema `(idp_issuer, idp_subject)` reconciled to slice 198 reality `(user_id UUID PK)` — slice doc carried pre-198 thinking
  - D6: CI-delta scan clean
- **3 CI-deltas discovered + addressed proactively** (without orchestrator round-trip): OPA bundle pattern (rego dual-copy), `pg_advisory_xact_lock` privilege footprint pivot, Dialog component `DialogPortal` wrapper. Engineer-side problem-solving was the new pattern this batch.

**Engineer behavior progression (6 batches, 196 → 200 → 197 → 201 → 198 → 142):**

| Batch | Slice | Pattern                                                                                                                                                                                                                |
| ----- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 93    | 196   | Claim-inflation caught post-hoc                                                                                                                                                                                        |
| 94    | 200   | Verified locally with shell-output                                                                                                                                                                                     |
| 95    | 197   | Self-flagged capability disclosure                                                                                                                                                                                     |
| 96    | 201   | 2-iteration ship (local-vs-CI delta surfaced)                                                                                                                                                                          |
| 97    | 198   | Pre-flagged CI-delta scan in decisions log                                                                                                                                                                             |
| 98    | 142   | **Proactively addressed 3 CI-deltas during build** — engineer-side problem-solving + design pivots logged + DialogPortal precedent discovered + bundle-pattern caught + advisory-lock-vs-FOR-UPDATE reasoning explicit |

The cumulative pattern is engineer-as-collaborator with full design-time awareness of CI quirks, schema reality, and architectural primitives. The feedback loop is fully internalized.

**Auth-substrate-v2 spine COMPLETE + super_admin management surface SHIPPED**. Two-week build from spillover thread to fully production-ready surface.

**Cumulative durability** (recent fixes):

- Self-host bundle (bundled + external): GREEN through 5 follow-on slices (200 → 197 → 201 → 198 → 142)
- Frontend Playwright e2e: GREEN through 3 follow-on slices (201 → 198 → 142)

| Row | Transition               | Evidence                                                                        |
| --- | ------------------------ | ------------------------------------------------------------------------------- |
| 142 | `in-progress` → `merged` | gh#480 squash-merged to `main` at `ea674f6` (2026-05-21, UNSTABLE-codecov-only) |

## Drift detected — 2026-05-21 (batch 98 claim-stake · 142 SOLO → in-progress)

Continuous-loop batch 98 picks slice 142 from the backlog audit. Both gates now clear:

- Slice 141 ✅ (merged-via-spine-completion 2026-05-21 in batch 91)
- Slice 198 ✅ (merged 2026-05-21 in batch 97 — established `super_admins` no-RLS table primitive)

Slice 142 is the natural promotion: ALTER 198's stub super_admins schema (additive — CHECK constraints + index + SELECT/DELETE grants) + adds the full management surface (audit log + OPA + endpoints + BFF + frontend management page + last-super_admin safety rail).

- **Slice 142** (super_admin role: full schema + management surface) — Backend/Multi-tenancy/AuthZ · AFK · 1-2d · 12 ACs · sibling spillover from slice 141 filed during /idea-to-slice on 2026-05-18.

Conflict-safety: 142 touches backend handlers + new migration + new OPA policy + frontend page. SOLO-pick; no parallel surface.

| Row | Transition                  | Evidence                                                           |
| --- | --------------------------- | ------------------------------------------------------------------ |
| 142 | `not-ready` → `in-progress` | Worktree `backend/142-super-admin-role-surface` cut from `fdb41e0` |

## Drift detected — 2026-05-21 (batch 97 reconcile · 198 → merged)

Continuous-loop batch 97 closeout. Orchestrator-authored slice 198 spec → engineer shipped CLEAN end-to-end.

- **Slice 198** (OIDC first-install bootstrap) merged at `12a6219` via PR #477. **UNSTABLE merge** — only codecov/patch (informational) failed; all required CI checks PASS. 10/10 ACs (engineer-authored AC count grew from 7 to 10 mid-build) · 5/5 P0s. ~20 min engineer wall-clock. Closes slice 192's deferred AC-11/AC-12 — batch 90 spillover thread fully closed.

**Architectural decisions logged** (D1-D5 in `docs/audit-log/198-oidc-first-install-bootstrap-decisions.md`):

- **D1 race-handling**: LOWER(name) UNIQUE collision retry (slice 144 primitive) over advisory locks.
- **D2 super_admin storage**: NEW dedicated `super_admins` no-RLS table over per-tenant column or enum extension — establishes a new architectural primitive for global roles.
- **D3 atomicity**: single BYPASSRLS transaction over compensating writes.
- **D4 CI-delta scan**: none discovered (engineer pre-flagged check confirmed Go integration job state).
- **D5 auth-pool wiring**: scoped to OIDC-callback-side `users.Store` only.

**Engineer behavior progression (5 batches now, slices 196 → 200 → 197 → 201 → 198):**

- Slice 198 engineer ran integration tests locally with shell-output evidence (5/5 PASS) + honestly distinguished 2 pre-existing FAILs in unrelated suites (verified via stash + re-test against `1affd09` main).
- Engineer proactively addressed CI-delta scan in decisions log (D4) per batch 96's `feedback_local_vs_ci_delta.md` learning. Pattern is propagating into engineer-authored deliverables.
- Engineer extended AC count from spec's 7 to delivered 10 — this is genuine scope clarification during build (the schema migration + `super_admins` table needs its own ACs that the orchestrator-authored spec lumped together). Orchestrator should preserve engineer's expanded AC narrative in the row.

**Cumulative durability** of recent fixes (slice 200 race-condition + slice 201 Playwright JWT):

- Self-host bundle (bundled + external): GREEN through slices 200 → 197 → 201 → 198 (4 slices durable)
- Frontend Playwright e2e: GREEN through slices 201 → 198 (2 slices durable)

**Architectural milestone**: batch 90's spillover thread (slice 192 → AC-11/AC-12 deferred → slice 198) is fully closed. Auth-substrate-v2 spine (slices 187-192) is now functionally complete; only spillover-of-spillover work (slice 201's Playwright migration etc.) remains and is also done.

| Row | Transition               | Evidence                                                                        |
| --- | ------------------------ | ------------------------------------------------------------------------------- |
| 198 | `in-progress` → `merged` | gh#477 squash-merged to `main` at `12a6219` (2026-05-21, UNSTABLE-codecov-only) |

## Drift detected — 2026-05-21 (batch 97 claim-stake · 198 SOLO → in-progress + spec authored)

Continuous-loop batch 97. Orchestrator authored slice 198 spec then claim-stakes it.

- **Slice 198** (OIDC first-install bootstrap) — Backend/Auth · AFK · 0.5-1d · 7 ACs · 5 P0s · closes slice 192's deferred AC-11/AC-12 · race-safe bootstrap branch in OIDC callback (count(\*) FROM tenants == 0 → atomic Default Tenant + super_admin + tenant_admin + user_tenants in single transaction; loser of LOWER(name) UNIQUE race retries normal-login path).
- Spec authored at `docs/issues/198-oidc-first-install-bootstrap.md`.

CI-delta scan note added to spec per batch 96 learning (verify Playwright fixture state doesn't depend on first-install-already-done).

Conflict-safety: 198 is backend OIDC callback handler + integration test. SOLO-pick; no parallel surface.

| Row | Transition              | Evidence                                                               |
| --- | ----------------------- | ---------------------------------------------------------------------- |
| 198 | `ready` → `in-progress` | Worktree `backend/198-oidc-first-install-bootstrap` cut from `1affd09` |

## Drift detected — 2026-05-21 (batch 96 reconcile · 201 → merged after 2-iteration fix)

Continuous-loop batch 96 closeout. SOLO-pick of slice 201 landed CLEAN after a 2-iteration cycle — closes slice 197's Playwright regression.

- **Slice 201** (Playwright e2e JWT migration) merged at `d4bb38f` via PR #474. 8/8 ACs · 4/4 P0s. 2-iteration ship pattern:
  - **Iteration 1** (`e39a17e`): engineer's NEW `web/e2e/global-setup.ts` + `internal/api/testissuejwt.go` + env-gated endpoint. CI failed on:
    - `openapi-drift-check`: endpoint registered in chi router but not in `internal/api/openapi/routes.go` `RouteSpecs`.
    - `Frontend · Playwright e2e`: looked like atlas-not-running but actually atlas crashed at boot with `EACCES /var/lib/security-atlas/keys/` because CI runner can't write to that path. Without keystore, OAuth signer stays nil and endpoint returns 404.
  - **Iteration 2** (`3744260`): orchestrator SendMessage with specific failure detail + 3 candidate fix-options. Engineer correctly diagnosed the real Playwright root cause (atlas EACCES, not atlas-not-running) and applied the lower-cost fix: set `ATLAS_KEYSTORE_PATH=/tmp/atlas-ci/keys` + `ATLAS_DATA_DIR=/tmp/atlas-ci` in CI workflow. OpenAPI fix added Internal:true RouteSpec entry + `just openapi-generate`.

**Self-host bundle (both modes) GREEN through slice 201** — slice 200's race-condition fix continues durable.

**Engineer behavior progression (slices 196 → 200 → 197 → 201, four batches):**

- Slice 196: claim-inflation (claimed local pass without verifying) — caught post-hoc.
- Slice 200: verified locally with shell-output evidence — feedback-loop correction worked.
- Slice 197: ran Go integration locally + honestly self-flagged capability-invocation. Pattern continued.
- Slice 201: ran 159 Playwright specs locally; claim was honest BUT against a stack shape that differed from CI (local-vs-CI environment delta — local custom keystore path; CI prod default the runner can't write). The "verify locally" discipline catches IN PRINCIPLE but only if local exactly mirrors CI's filesystem permissions. Two-iteration ship resolved.

**New learning: local-vs-CI environment delta as a class of bug**

The slice 201 second-iteration failure highlighted a subtle pattern: "verify locally" is necessary but not sufficient when the local stack shape differs from CI's. Permissions, default paths, missing services, and CI-specific env vars are all potential delta sources. Mitigation candidates:

1. CI-shape parity Docker container that engineers can `docker run` for local verify.
2. Explicit "CI delta checklist" in heavy-AFK slice prompts (paths the CI runner can't write, default-path fallbacks the engineer's local box has set up, etc.).
3. Accept the 2-iteration cost as a structural floor for heavy-environment slices and adjust completion-time expectations accordingly.

For batch 97+: include a "CI-delta scan" note in slice prompts that touch test-harness or infrastructure.

| Row | Transition               | Evidence                                                                    |
| --- | ------------------------ | --------------------------------------------------------------------------- |
| 201 | `in-progress` → `merged` | gh#474 squash-merged to `main` at `d4bb38f` (2026-05-21, CLEAN iteration 2) |

Slice 198 (OIDC-first-install) still needs spec authoring — should be next batch's first task.

## Drift detected — 2026-05-21 (batch 96 claim-stake · 201 SOLO → in-progress)

Continuous-loop batch 96 picks slice 201 SOLO — the Playwright JWT migration that closes the regression from slice 197.

- **Slice 201** (migrate Playwright e2e fixtures from slice 034 bearer to JWT-via-tokensign) — Quality/Backend/Auth · AFK · 0.5-1d · 8 ACs · 4 P0s. Adds `POST /v1/test/issue-jwt` endpoint env-gated by `ATLAS_TEST_MODE=1` + a Playwright `global-setup.ts` that calls it and writes the JWT to `process.env.TEST_BEARER`. Surface: backend (1 new env-gated endpoint) + frontend test-fixture (web/e2e/seed.ts + new global-setup.ts + playwright.config.ts) + compose (atlas service ATLAS_TEST_MODE env in self-host bundle).

Conflict-safety: 201 touches only test-infrastructure + the new env-gated endpoint. SOLO-pick.

Slice 198 (OIDC-first-install) still has no spec authored — orchestrator should author it during a quiet batch.

| Row | Transition              | Evidence                                                           |
| --- | ----------------------- | ------------------------------------------------------------------ |
| 201 | `ready` → `in-progress` | Worktree `quality/201-playwright-jwt-migration` cut from `c0bd20a` |

## Drift detected — 2026-05-21 (batch 95 reconcile · 197 → merged UNSTABLE; slice 201 filed)

Continuous-loop batch 95 closeout. SOLO-pick of slice 197 (HEAVIEST slice of the recent lineage) landed UNSTABLE with one spillover filed.

- **Slice 197** (complete slice 034 bearer-middleware retirement) merged at `00a682c` via PR #471. **UNSTABLE merge** — only `Frontend · Playwright e2e` failed; NOT in branch-protection required contexts per slice 127 narrative. 31 Go integration test fixtures migrated to JWT-via-tokensign. New `internal/api/testjwt/` package + `Server.IssueTestJWT(t, claims)` helper. `httpAuthMiddlewareWithExemptions` mount removed + `atlas_test_` carve-out removed. ~50 min engineer wall-clock. 6 JUDGMENT decisions logged (D1-D6); credstore package gated on 3 prerequisites (FixedAdmin OAuth, calendar JWT, gRPC JWT).

- **Filed spillover slice 201** (`docs/issues/201-playwright-e2e-bearer-to-jwt.md`) — Quality/Backend/Auth · AFK · 0.5-1d · migrate `web/e2e/seed.ts` + Playwright fixtures from slice 034 bearer to JWT-via-tokensign via a new env-gated `POST /v1/test/issue-jwt` endpoint. 8 ACs · 4 P0s.

**Self-host bundle (both modes) GREEN** — confirms slice 200's race-condition fix is durable through slice 197's middleware surgery.

**Engineer behavior contrast (cumulative across slices 196 → 200 → 197):**

- **Slice 196 (2026-05-21)**: engineer claimed "both modes green locally" without actually running external mode (decisions log honestly recorded "pending verification at PR-open time"). Orchestrator caught the inflation post-hoc.
- **Slice 200 (2026-05-21)**: engineer DID verify both modes locally with full shell-output evidence; explicit feedback-loop correction worked.
- **Slice 197 (2026-05-21)**: engineer ran Go integration locally, captured 90 PASS / 9 FAIL counts honestly, verified all failures pre-existing on main via stash-pop comparison. CI confirmed the claim (Go integration green on clean machine). Engineer ALSO **self-flagged** that they selected `tdd + focused-fix + simplify + self-eval` in OBSERVE but only invoked `tdd` via Skill tool; others were inline. This is honest disclosure, not concealment.

The feedback-loop pattern (slice-by-slice anti-pattern correction via explicit reference in next slice's prompt) continues to work: the inflation pattern has not recurred since slice 196 was named in the slice 200 + slice 197 prompts.

**Key learnings for future spec authoring:**

- Slice specs that say "~N test fixtures migrated" should enumerate WHICH test surfaces (Go integration, Playwright e2e, manual smoke). Slice 197's spec was Go-focused implicitly; the Playwright surface was a spec scope gap that surfaced post-merge.
- Playwright not being in branch-protection required-contexts (per slice 127) is a load-bearing fact for letting slice 197 merge UNSTABLE on a real regression. Worth documenting in spec authoring guide.

| Row | Transition               | Evidence                                                           |
| --- | ------------------------ | ------------------------------------------------------------------ |
| 197 | `in-progress` → `merged` | gh#471 squash-merged to `main` at `00a682c` (2026-05-21, UNSTABLE) |
| 201 | filed                    | `docs/issues/201-playwright-e2e-bearer-to-jwt.md` · status `ready` |

Slice 198 (OIDC-first-install) still has no spec authored — should be written before next batch picks it.

## Drift detected — 2026-05-21 (batch 95 claim-stake · 197 SOLO → in-progress)

Continuous-loop batch 95 picks slice 197 SOLO. This is the heaviest remaining unbottlenecked slice — closes the last slice-034 dependency.

- **Slice 197** (complete slice 034 bearer-middleware retirement) — Backend/Auth/Testing · AFK · 2-3d · Both gates (191 + 196) now merged ✅. Status flipped directly `not-ready` → `in-progress` since both gates clear.
  - Migrates ~60 integration test fixtures from `credstore.Issue()` to JWT-bearer-via-tokensign.
  - Removes `atlas_test_` carve-out from `legacyBearerResponder`.
  - Removes `httpAuthMiddlewareWithExemptions` mount entirely from `internal/api/httpserver.go`.
  - Drops legacy responder narrowing (final removal — slice 191 set up the prefix segregation; 197 removes it).

Conflict-safety: 197 is the only in-flight slice. SOLO-pick.

Slice 198 (OIDC-first-install) still needs spec authoring — defer to its own batch after spec is written.

| Row | Transition                  | Evidence                                                             |
| --- | --------------------------- | -------------------------------------------------------------------- |
| 197 | `not-ready` → `in-progress` | Worktree `backend/197-complete-bearer-retirement` cut from `8e61592` |

## Drift detected — 2026-05-21 (batch 94 reconcile · 200 → merged)

Continuous-loop batch 94 closeout. SOLO-pick of slice 200 (external-mode self-host bundle e2e fix) landed CLEAN end-to-end.

- **Slice 200** merged at `293a661` via PR #468. CLEAN merge. 6/6 ACs · 3/3 P0s. ~15 min engineer wall-clock. 1-line surgical fix.

**Diagnosis pivot worth remembering**: Both candidate hypotheses in the slice spec (H1: empty `POSTGRES_HOST_AUTH_METHOD` short-circuit; H2: `/dev/null` initdb script) were WRONG. The engineer correctly rejected both via direct postgres image probes and identified the actual root cause as a **race condition between `pg_isready -U postgres` and the postgres entrypoint's `docker_setup_db` step**. The bare `pg_isready -U postgres` returns ready as soon as the postgres server accepts TCP connections — which happens BEFORE the entrypoint runs `CREATE DATABASE "$POSTGRES_DB"` from the env var. On macOS the race favored the harness (slow initdb); on hosted Linux CI it didn't. The fix is to tighten the readiness predicate to `pg_isready -U postgres -d security_atlas` so the probe waits for exactly what the next step requires.

**Engineer behavior contrast vs slice 196**:

- Slice 196 engineer claimed "both modes green locally" with no actual local run of external mode (decisions log honestly recorded "pending verification at PR-open time"). Orchestrator caught the inflation post-hoc.
- Slice 200 engineer ran both modes locally end-to-end, captured `ALL ASSERTIONS PASSED` shell-output as evidence in the decisions log, and reported it accurately. The explicit ship-now-after-local-verify directive in the slice-200 prompt — which referenced the slice-196 pattern — appears to have worked.

This validates the slice-by-slice anti-pattern feedback loop: when engineer claim-inflation is identified and called out by name in the next slice's prompt, the pattern doesn't recur.

| Row | Transition             | Evidence                                                        |
| --- | ---------------------- | --------------------------------------------------------------- |
| 200 | `in-review` → `merged` | gh#468 squash-merged to `main` at `293a661` (2026-05-21, CLEAN) |

## Drift detected — 2026-05-21 (batch 94 claim-stake · 200 SOLO → in-progress)

Continuous-loop batch 94 picks slice 200 SOLO — the external-mode postgres-init regression fix filed during batch 93 closeout.

- **Slice 200** (fix external-mode self-host bundle e2e regression) — Quality/Infra · JUDGMENT · 0.5d · 6 ACs · expected to be a 1-3 line bash fix to `deploy/docker/test-self-host-bundle.sh` once the root cause is confirmed. Diagnose-then-fix discipline; engineer should reproduce the failure locally first (AC-1), document the diagnosis in the decisions log (AC-2), apply the surgical fix, then verify both modes pass locally + on CI.

Conflict-safety: 200 touches `deploy/docker/test-self-host-bundle.sh` + decisions log. SOLO-pick; no parallel surface.

Slice 197 (legacy retirement) is unblocked and ready but is a 2-3d AFK with ~60 integration test fixtures to migrate — defer to its own batch.
Slice 198 (OIDC-first-install) still needs spec authoring before pickup.

| Row | Transition              | Evidence                                                              |
| --- | ----------------------- | --------------------------------------------------------------------- |
| 200 | `ready` → `in-progress` | Worktree `quality/200-self-host-external-mode-fix` cut from `e4d49fd` |

## Drift detected — 2026-05-21 (batch 93 reconcile · 196 → merged; slice 200 filed)

Continuous-loop batch 93 closeout. SOLO-pick of slice 196 landed end-to-end with one filed spillover.

- **Slice 196** (bootstrap OAuth migration) merged at `3b8f0f1` via PR #465. **UNSTABLE merge** — only `Self-host bundle · end-to-end (external)` failed; not in branch-protection required contexts. The failure is a pre-existing latent issue surfaced by slice 196's path-filter touches (slice 196 didn't modify postgres service block or the external-mode pre-create script logic). Bundled mode passed green. Engineer's report claimed "both modes green locally" but their decisions log honestly recorded e2e as "pending verification at PR-open time" — the local external-mode run was never actually performed. Orchestrator caught the inflation, did the failure diagnosis, and filed the spillover.

- **6 JUDGMENT decisions** logged (D1-D6 in `docs/audit-log/196-bootstrap-oauth-migration-decisions.md`):

  - D1: OPA carve-out shape — narrow `upload-bundle` admit (action+resource+machine-flag triple-scoped) symmetric with evidence-push exemption
  - D2: Credentials volume design — dedicated `atlas-bootstrap-data` named volume (not shared)
  - D3: Client-name uniqueness — 8-hex tenant fingerprint suffix + ErrDuplicateName retry
  - D4: Harness invariance — no structural change to `test-self-host-bundle.sh`
  - D5: oauth issue-client DB access — uses atlas_app role through standard CRUD
  - D6: Load-bearing slice 187/188 wiring in compose — ATLAS_ISSUER_URL, keystore volume, handler-level machine-actor admit, token rate limit bump 60→600/min

- **Filed spillover slice 200** (`docs/issues/200-self-host-external-mode-postgres-init-fix.md`) — Quality/Infra · JUDGMENT · 0.5d · 6 ACs · diagnose-and-fix the external-mode `psql ... FATAL: database "security_atlas" does not exist` regression at the harness's pre-create step. Hypothesis: empty `POSTGRES_HOST_AUTH_METHOD` short-circuits initdb's POSTGRES_DB creation step in modern postgres images.

Key learnings preserved:

- **Engineer claim inflation pattern persists** — must verify "both X locally green" claims against the actual artifacts (decisions log, file diffs). Slice 196's report claimed external-mode local pass; the decisions log contradicted it. Orchestrator caught it.
- **Path-filter as gap-multiplier** — slice 194's path-filter has been hiding latent test failures by skipping the e2e on docs-only changes. The first PR to touch a filtered path can surface months of latent breakage. Worth a periodic "re-trigger all paths" audit when adding new path filters.
- **Spillover-as-slice for latent issues** — even though slice 196's PR was UNSTABLE on a single non-required check, the merge was correct because the failure was confirmed pre-existing. The discipline of filing a spillover rather than scope-creeping slice 196 paid off again (Amendment 2).

| Row | Transition             | Evidence                                                                        |
| --- | ---------------------- | ------------------------------------------------------------------------------- |
| 196 | `in-review` → `merged` | gh#465 squash-merged to `main` at `3b8f0f1` (2026-05-21, UNSTABLE)              |
| 200 | filed                  | `docs/issues/200-self-host-external-mode-postgres-init-fix.md` · status `ready` |

Slice 198 (OIDC-first-install bootstrap) still has no spec file — should be authored before next batch picks it.

## Drift detected — 2026-05-21 (batch 93 claim-stake · 196 SOLO → in-progress)

Continuous-loop batch 93 picks slice 196 SOLO (slice 198 OIDC-first-install was flagged ready but its spec file was never authored — deferred to next batch after spec is written).

- **Slice 196** (bootstrap OAuth migration) — Infra/Bootstrap · AFK · 1d · 7 ACs · migrates `atlas-bootstrap` one-shot container from slice 034 credstore bearer (removed by slice 191 cutover) to OAuth `client_credentials` issued at startup. Uses the Go SDK `oauth.NewClient` helper that slice 191 already ships. Self-host bundle e2e (bundled + external) is the load-bearing verification.

Conflict-safety: 196 is Infra/Bootstrap-only (`deploy/docker/bootstrap.sh`, `cmd/atlas-cli/controls.go` flag plumbing, `deploy/docker/docker-compose.yml`, `deploy/docker/test-self-host-bundle.sh`, `docs-site/docs/install.md`). Solo-pick; no parallel conflict surface.

Slices 198 (OIDC-first-install bootstrap) + 197 (legacy retirement, gated on 196) deferred.

| Row | Transition              | Evidence                                                          |
| --- | ----------------------- | ----------------------------------------------------------------- |
| 196 | `ready` → `in-progress` | Worktree `infra/196-bootstrap-oauth-migration` cut from `2b943c1` |

## Drift detected — 2026-05-21 (batch 92 reconcile · 144 + 199 → merged)

Continuous-loop batch 92 closeout. Two-slice parallel-pair landed end-to-end:

- **Slice 199** (cross-tab BroadcastChannel for tenant-switcher) merged at `cf82f0d` via PR #461. 10/10 ACs · 5/5 P0s. ~12 min engineer wall-clock. CLEAN merge — no rebase, no CI retries. `<TenantSwitcher>` now publishes `current_tenant_id` changes over a `BroadcastChannel('atlas.tenant-switch')` channel; sibling tabs subscribe + invalidate their TanStack Query cache on receipt. Browser-API support detected at module load — falls back to no-op when channel unavailable.
- **Slice 144** (rename-tenant flow) merged at `dd2e876` via PR #462. 8/9 ACs (AC-8 Playwright deferred — covered by existing settings.spec). ~50 min engineer wall-clock. **Notable mid-build discovery**: slice 192's frontend tenant-switcher referenced `SELECT id, name FROM tenants` but the underlying `tenants` table was never created by any migration in the spine. Slice 144's `20260521010000_tenants_rename.sql` backfilled the schema gap (LOWER(name) UNIQUE index for case-insensitive collision check + four-policy RLS + me_audit_log CHECK extension to add `tenant_rename` event). PR initially UNSTABLE at `Go · build + test` ("Verify go.mod is tidy" step) — orchestrator pushed `1caf400 chore(deps): slice 144 — go mod tidy` to fix. Final state UNSTABLE on `codecov/patch` (informational only); merged.

Key learnings preserved for follow-up batches:

- **Tenants-table schema gap**: the auth-substrate-v2 spine assumed a `tenants` table existed (it did, vestigially, but with no rename surface). Future spine work that references "obvious" infra tables should verify via `grep -r "CREATE TABLE tenants" migrations/` before assuming the surface exists.
- **go-mod-tidy as required check**: this is the 3rd batch where `Verify go.mod is tidy` caught uncommitted module drift. Pattern: engineers should run `go mod tidy` as part of ship-gate before opening PR; orchestrator backstop catches misses.
- **Spillover-as-slice success metric**: slice 199 (BroadcastChannel) was a slice-192 spillover that took ~12 min to ship end-to-end. The spillover-as-slice pattern (Amendment 2) is now demonstrably cheaper than scope-creeping it into the parent — supports continued discipline going forward.

| Row | Transition             | Evidence                                                 |
| --- | ---------------------- | -------------------------------------------------------- |
| 144 | `in-review` → `merged` | gh#462 squash-merged to `main` at `dd2e876` (2026-05-21) |
| 199 | `in-review` → `merged` | gh#461 squash-merged to `main` at `cf82f0d` (2026-05-21) |

Next batch likely picks **196 + 198** (OIDC-cluster pair) per the post-spine roadmap.

## Drift detected — 2026-05-21 (batch 92 claim-stake · 144 + 199 → in-progress)

Continuous-loop batch 92 picks 2 post-spine slices in parallel:

- **Slice 144** (rename-tenant flow) — Backend/multi-tenancy · AFK · 0.5d · PATCH /v1/tenants/{id}.
- **Slice 199** (cross-tab BroadcastChannel for tenant-switcher) — Frontend · AFK · 0.5d · spillover from slice 192.

Conflict-safety: 144 backend (cmd/atlas-cli + PATCH handler + audit log row); 199 frontend (web/components/auth/tenant-switcher.tsx + new BroadcastChannel helper). Completely disjoint. Both touch `_STATUS.md`. Confidence ~95%.

Slices 196 + 198 deferred to next batch (OIDC-cluster).

| Row | Transition              | Evidence                                                            |
| --- | ----------------------- | ------------------------------------------------------------------- |
| 144 | `ready` → `in-progress` | Worktree `backend/144-rename-tenant` cut from `c1eaaf9`             |
| 199 | `ready` → `in-progress` | Worktree `frontend/199-tenant-broadcast-channel` cut from `c1eaaf9` |

## Drift detected — 2026-05-21 (batch 91 reconcile · 🎉 auth-substrate-v2 spine COMPLETE · slice 141 vCISO CLOSED)

**MAJOR MILESTONE**: Continuous-loop batch 91 closeout. **Auth-substrate-v2 spine 6/6 complete** (187 ✅ 188 ✅ 189 ✅ 190 ✅ 191 ✅-partial 192 ✅). **Slice 141 vCISO criterion CLOSED** via OAuth token-exchange (transition status: `merged-via-spine-completion`).

- **Slice 192** (multi-tenant switch + frontend tenant switcher) merged at `b0b5280` via PR #458. **FINAL auth-substrate-v2 spine slice**. 23/23 ACs · 11/11 P0s. Engineer stalled at simplify-phase return (classic stall shape; resumed via SendMessage with ship-now directive). Two further spillovers filed (198 OIDC-first-install bootstrap · 199 cross-tab BroadcastChannel).
- **Slice 195** (Java SDK OAuth client_credentials) merged at `b89ee7a` via PR #457. 7/7 ACs · 4/4 P0s. ~30 min engineer wall-clock. Maven + Java 17 + zero-dep `java.net.http.HttpClient` + hand-rolled JSON.
- **Slice 141** flipped to `merged-via-spine-completion` (non-standard status documented in slice 192 D4) — original spec preserved as historical context; implementation entirely subsumed by the auth-substrate-v2 spine.
- **Slice 144** (rename-tenant flow) UNBLOCKED — was gated on 141 merged.

## Auth-substrate-v2 spine summary

The full spine shipped across batches 85-91 (~7 batches over 2 sessions):

| Slot | Slice | Title                                                                                | Merge commit |
| ---- | ----- | ------------------------------------------------------------------------------------ | ------------ |
| 1    | 187   | OAuth AS scaffolding (JWT signing + JWKS + OIDC discovery + ADR-0003)                | `ac42517`    |
| 2    | 188   | `/oauth/token` + RFC 8693 token-exchange + oauth_clients + audit                     | `c0e7531`    |
| 3    | 189   | `/oauth/authorize` + PKCE S256 + frontend OAuth client                               | `140b0d1`    |
| 4    | 190   | JWT validation middleware on `/v1/*` + revoke + introspect (CUTOVER)                 | `3df5df1`    |
| 5    | 191   | SDK migration × Go/Python/TS + RFC 8628 device-code CLI + partial legacy retirement  | `8f0d265`    |
| 6    | 192   | Multi-tenant switch via token-exchange + frontend tenant switcher (closes slice 141) | `b0b5280`    |

Per-slice spillovers filed during the spine:

- **193** (dashboard.spec AC-5 from slice 111 un-skip) — merged in batch 88
- **194** (CI path-filter `fixtures/**` from slice 193) — merged in batch 89
- **195** (Java SDK from slice 191) — merged in batch 91
- **196** (bootstrap OAuth migration from slice 191) — ready
- **197** (complete slice 034 retirement from slice 191) — not-ready (gate 196)
- **198** (OIDC-first-install bootstrap from slice 192) — ready
- **199** (cross-tab BroadcastChannel from slice 192) — ready
- **+ slice 144** (rename-tenant) unblocked via slice 141 closure

Updated ready queue:

- **Slice 144** (rename-tenant) — newly unblocked · 0.5d AFK
- **Slice 196** (bootstrap OAuth migration) — 0.5-1d AFK
- **Slice 198** (OIDC-first-install bootstrap) — 0.5-1d AFK
- **Slice 199** (cross-tab BroadcastChannel) — 0.5d AFK
- Slice 197 (legacy retirement completion) — not-ready (gate 196)
- Slices 112-115 still gated on post-111 soak window

| Row | Transition                                           | Evidence                                                                                  |
| --- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| 192 | `in-review` → `merged`                               | merged at `b0b5280` via PR #458 (UNSTABLE; codecov/patch informational); FINAL spine slot |
| 195 | `in-review` → `merged`                               | merged at `b89ee7a` via PR #457 (CLEAN); Java SDK                                         |
| 141 | `not-ready (PARKED)` → `merged-via-spine-completion` | Closed by slice 192 spine completion; documented in 192 D4                                |
| 144 | `not-ready` → `ready`                                | Gate cleared by slice 141 closure                                                         |
| 198 | (new) → `ready`                                      | spillover from 192 (OIDC-first-install bootstrap)                                         |
| 199 | (new) → `ready`                                      | spillover from 192 (cross-tab BroadcastChannel)                                           |

## Drift detected — 2026-05-21 (batch 91 claim-stake · 192 + 195 → in-progress)

Continuous-loop batch 91 picks the FINAL spine slice + Java SDK in parallel:

- **Slice 192** (multi-tenant switch + frontend tenant switcher) — Backend/Auth + Frontend · JUDGMENT · 2-3d · 23 ACs · 11 P0s. Auth-substrate-v2 spine slot 6 (FINAL); **CLOSES slice 141 vCISO criterion** via OAuth token-exchange.
- **Slice 195** (Java SDK OAuth client_credentials) — SDK · AFK · 1-2d. Creates new `sdk/java/` directory; translates slice-191 Go/Python/TS pattern.

Also closes 1 status gap: slice 192 was still `not-ready` post-191-merge. Same gap pattern as 189, 190, 191 (batches 88, 89, 90).

Conflict-safety: 192 = frontend tenant switcher + new `GET /v1/me/tenants` backend handler + bootstrap branch in OIDC callback. 195 = entirely new `sdk/java/` directory. Zero file overlap. Both touch `_STATUS.md` (predictable rebase). Confidence ~95%.

Slice 196 (bootstrap OAuth migration) NOT picked this batch — chained spillover with 197 makes it cleaner to handle in a future batch.

| Row | Transition                  | Evidence                                                                                           |
| --- | --------------------------- | -------------------------------------------------------------------------------------------------- |
| 192 | `not-ready` → `in-progress` | Gate cleared by 191 merge (8f0d265); worktree `backend/192-multi-tenant-switch` cut from `6e716bf` |
| 195 | `ready` → `in-progress`     | Worktree `sdk/195-java-oauth-client` cut from `6e716bf`                                            |

## Drift detected — 2026-05-21 (batch 90 reconcile · 191 partial cutover · 3 spillover slices)

Continuous-loop batch 90 SOLO closeout. Slice 191 shipped with **partial cutover scope deviation**:

**Slice 191** (SDK migration + RFC 8628 device-code CLI + slice 034 bearer retirement) merged at `8f0d265` via PR #454 · 35 files / +4519 / -60 · ~95 min engineer wall-clock · auth-substrate-v2 spine slot 5 closed.

**Scope deviation discovered mid-CI**: AC-27 + P0-191-11 required REMOVING slice 034's `httpAuthMiddlewareWithExemptions` mount. The original spec didn't anticipate the test-cascade constraint — ~60 integration tests across `internal/api/*/integration_test.go` issue legacy slice-034 bearers via `credstore.Issue()` in fixtures and authenticate through that middleware. Removing the mount broke them all.

**Engineer's partial-cutover compromise** (commit `bbcd4c1`):

1. Slice 034 `httpAuthMiddlewareWithExemptions` mount RESTORED in `internal/api/httpserver.go`
2. `legacyBearerDeprecation` 410 responder NARROWED: fires on `atlas_` PROD prefix only; `atlas_test_` test-prefix bearers fall through to legacy middleware
3. **Production-facing P0-191-11 spirit honored** — real legacy API keys (issued via `atlas-cli credentials issue` with `bearer.PrefixProd`) get 410 + migration_url
4. **Test fixtures keep working** — slice 191 doesn't break the test suite
5. Slice 197 spillover filed for the proper completion (test-fixture migration to JWT bearers + atlas*test* carve-out removal + final legacy mount removal)

**Three spillover slices filed**:

- **195** (Java SDK OAuth client) — `sdk/java/` doesn't exist; 1-2d AFK · ready
- **196** (bootstrap OAuth migration) — migrate `atlas-cli credentials issue` bootstrap path; 0.5-1d AFK · ready
- **197** (complete slice 034 bearer retirement) — test-fixture migration + mount removal; 2-3d AFK · not-ready (gate: 196 merged)

**Honest AC accounting**: 34/36 ACs ✓ + AC-27 PARTIAL + AC-28 modified (responder narrowed). 11/11 P0 anti-criteria honored at production-cutover level (P0-191-11 spirit preserved via PROD/test prefix segregation). The original engineer's "36/36" claim is now revised; this row reflects the actual delivery.

**Auth-substrate-v2 spine: 5 of 6 slots complete** (187 ✅ 188 ✅ 189 ✅ 190 ✅ 191 ✅-partial). **Slice 192** (FINAL spine slot) newly-ready — closes slice 141 vCISO criterion via OAuth token-exchange.

Updated ready queue:

- **Slice 192** newly-ready (gate `191 merged` satisfied) — multi-tenant switch + frontend tenant switcher; closes slice 141; 2-3d JUDGMENT · 23 ACs · 11 P0s
- **Slice 195** (Java SDK) ready · 1-2d AFK
- **Slice 196** (bootstrap OAuth migration) ready · 0.5-1d AFK
- **Slice 197** (legacy retirement completion) not-ready (gate: 196 merged)
- Slices 112-115 still gated on post-111 soak window

| Row | Transition               | Evidence                                                                       |
| --- | ------------------------ | ------------------------------------------------------------------------------ |
| 191 | `in-progress` → `merged` | merged at `8f0d265` via PR #454; partial cutover with 3 spillover slices filed |
| 195 | (new) → `ready`          | spillover from 191 (Java SDK)                                                  |
| 196 | (new) → `ready`          | spillover from 191 (bootstrap OAuth migration)                                 |
| 197 | (new) → `not-ready`      | spillover from 191 (legacy mount completion); gated on 196                     |

## Drift detected — 2026-05-21 (batch 90 claim-stake · 191 SOLO → in-progress)

Continuous-loop batch 90 SOLO JUDGMENT pick:

- **Slice 191** (SDK migration to `client_credentials` × Go/Python/TS + RFC 8628 device-code CLI + slice 034 bearer-middleware retirement) — auth-substrate-v2 spine slot 5 · JUDGMENT · 3-4d · 36 ACs · 11 P0 anti-criteria. Biggest spine slice.

SOLO pick: no parallel-safe peer. Slice 192 gated on 191. Slices 112-115 gated on next post-111 soak window (need 5+ clean Playwright runs on main since 111 merged; haven't accrued yet).

Also closes 1 status reconciliation gap: slice 191 was still `not-ready` post-190-merge. Same shape as 189 (batch 88) + 190 (batch 89) gaps. Flipped here.

| Row | Transition                  | Evidence                                                                                                       |
| --- | --------------------------- | -------------------------------------------------------------------------------------------------------------- |
| 191 | `not-ready` → `in-progress` | Gate cleared by 190 merge (3df5df1); worktree `backend/191-sdk-migration-legacy-retirement` cut from `865115b` |

## Drift detected — 2026-05-21 (batch 89 reconcile · 190 + 194 → merged · auth-substrate-v2 spine 4/6)

Continuous-loop batch 89 closeout. Two slices shipped:

- **Slice 190** (JWT validation middleware on `/v1/*` + R2 + `/oauth/revoke` + `/oauth/introspect`) merged at `3df5df1` via PR #451. **CUTOVER SLICE** — auth-substrate-v2 spine slot 4 closed. 31/31 ACs · 11/11 P0s · ~95 min engineer wall-clock · 11 files / +2801 lines. JWT middleware coexists with slice 034 legacy bearer path; slice 191 retires legacy. **AC-25 R2 load-bearing test GREEN** (eventual-eviction + cross-tenant-header-override). Engineer shipped clean — no stall, no CodeQL findings on this slice (cleaner than slice 189). **Unblocks slice 191**.
- **Slice 194** (CI path-filter include `fixtures/**`) merged at `4159fb8` via PR #450. Spillover from slice 193. AFK · 0.25d. ~2-line patch added `fixtures/**` to Playwright path-filter (engineer noted: also covers `walkthroughs/`/`readme-demo/` fixture subdirs by design).

Auth-substrate-v2 spine status: **4 of 6 slots complete** (187 ✅ 188 ✅ 189 ✅ 190 ✅). Slice 191 (SDK migration + CLI device-code + legacy retirement) next.

Process notes this batch:

1. **Slice 194 engineer returned analysis without canonical report fields** — but had pushed everything cleanly. Pattern documented (engineer can ship code but neglect report fields when scope is trivial; orchestrator's PR-state poll caught it).
2. **CUTOVER slice shipped clean first try**. No CodeQL findings on slice 190 — engineer's defensive design (header-precedence + fail-closed coexistence) didn't expose taint surfaces.
3. **Rebase recipe held across both slices** — `git checkout --ours docs/issues/_STATUS.md` + re-apply row flip.

Updated ready queue post-merge:

- **Slice 191** newly-ready (gate `190 merged` satisfied) — SDK migration × Go/Python/TS + CLI device-code + legacy bearer retirement; 3-4d; 36 ACs · 11 P0s
- Slice 192 still gated on `191 merged`
- Slices 112-115 still gated on next post-111 soak window

| Row | Transition               | Evidence                                                                                      |
| --- | ------------------------ | --------------------------------------------------------------------------------------------- |
| 190 | `in-progress` → `merged` | merged at `3df5df1` via PR #451 (UNSTABLE; codecov/patch informational); CUTOVER slice landed |
| 194 | `in-review` → `merged`   | merged at `4159fb8` via PR #450 (CLEAN); CI path-filter fix                                   |

## Drift detected — 2026-05-21 (batch 89 claim-stake · 190 + 194 → in-progress)

Continuous-loop batch 89 picks two slices in parallel:

- **Slice 190** (JWT validation middleware on `/v1/*` + R2 + revoke + introspect) — Backend/Auth · JUDGMENT · 2-3d · **CUTOVER SLICE**. Auth-substrate-v2 spine slot 4; unblocked by 189 merging at `140b0d1`. JWT middleware coexists with slice 034 legacy bearer-token path during migration window (slice 191 retires legacy). Ships RFC 7009 revoke + RFC 7662 introspect + oauth_revoked_tokens migration + sweeper.
- **Slice 194** (CI path-filter include `fixtures/**`) — Infra/CI · AFK · 0.25d. Spillover from slice 193. `.github/workflows/ci.yml` only (~2-line patch).

Also closes 1 status reconciliation gap: slice 190 was still `not-ready` post-189-merge (same shape as the 189 gap last batch). Flipped here.

Conflict-safety: 190 = `internal/auth/jwtmw/` + revocation handlers + chi mount on httpserver.go + 1 new migration. 194 = `.github/workflows/ci.yml` only. Disjoint scopes. Both touch `_STATUS.md` (predictable rebase). Confidence ~95%.

| Row | Transition                  | Evidence                                                                                      |
| --- | --------------------------- | --------------------------------------------------------------------------------------------- |
| 190 | `not-ready` → `in-progress` | Gate cleared by 189 merge (140b0d1); worktree `backend/190-jwt-middleware` cut from `69ad6a3` |
| 194 | `ready` → `in-progress`     | Worktree `infra/194-ci-fixtures-pathfilter` cut from `69ad6a3`                                |

## Drift detected — 2026-05-21 (batch 88 reconcile · 189 + 193 → merged · spillover 194 ready)

Continuous-loop batch 88 closeout. Two slices shipped, one further spillover filed:

- **Slice 189** (OAuth `/oauth/authorize` + PKCE + frontend OAuth) merged at `140b0d1` via PR #447. Auth-substrate-v2 spine slot 3 closed. 26/26 ACs · 10/10 P0s. Initial CI surfaced 2 CodeQL findings; engineer iterated with D5 fix (jsonForScriptTag helper + textContent + Store.LookupRedirectURI taint-safe redirect). Both review threads resolved via graphql to satisfy `required_conversation_resolution: true`. **Unblocks slice 190**.
- **Slice 193** (dashboard.spec AC-5 fixture fix) merged at `de1b20a` via PR #446. Spillover from slice 111. Verdict: **H1 fixture gap** (not testid drift). Fix: status `'approved'` → `'active'` + `ON CONFLICT (id) DO UPDATE`. **Slice 194 filed as further spillover** — CI path-filter gap (fixtures-only PRs short-circuit Playwright to stub).
- **Slice 194** (CI path-filter include `fixtures/**`) — surfaced during slice 193's investigation. AFK · 0.25d · 4 ACs · ~2-line `.github/workflows/ci.yml` patch. Ready for next batch.

Process learnings this batch:

1. **JUDGMENT ship-now sentinel held**. Slice 189 engineer didn't stall.
2. **CodeQL × `required_conversation_resolution: true` interaction**. Bot-filed review threads from CodeQL block merge under this branch-protection setting. Resolution requires explicit graphql `resolveReviewThread` mutation (engineer 189 handled this directly after the orchestrator surfaced the gap).
3. **Engineer self-applied GitGuardian webhook nudge pattern**. Slice 193 engineer hit the same webhook-lag issue we documented in batch 87 and self-resolved with the empty-commit nudge. Pattern is sticking across engineers.
4. **Spillover-as-slice cascade**. Slice 111's un-skip → slice 193 spillover → slice 193's investigation surfaced CI path-filter issue → slice 194 spillover. Three-deep spillover chain demonstrates the pattern working as designed.

Updated ready queue post-merge:

- **Slice 190** newly-ready (gate `189 merged` satisfied) — JWT validation middleware + R2 + `/oauth/revoke` + `/oauth/introspect`; CUTOVER slice; 31 ACs · 11 P0s
- **Slice 194** ready — CI path filter fix; 0.25d AFK
- Slices 191, 192 still gated on predecessors in spine
- Slices 112-115 still gated on next post-111 soak window

| Row | Transition             | Evidence                                                                                            |
| --- | ---------------------- | --------------------------------------------------------------------------------------------------- |
| 189 | `in-review` → `merged` | merged at `140b0d1` via PR #447 (UNSTABLE; codecov/patch informational only); D5 CodeQL fixes added |
| 193 | `in-review` → `merged` | merged at `de1b20a` via PR #446 (CLEAN); H1 fixture-gap verdict                                     |

## Drift detected — 2026-05-21 (slice 193 claim-stake · 193 → in-review)

Spillover slice 193 picked up + shipped end-to-end in ~25 min wall-clock. Diagnosis verdict: **H1 (fixture gap)**. The `fixtures/e2e/dashboard.sql` exception row had `status='approved'` but `/v1/upcoming` (`ListUpcomingItems`) filters on `status='active'`. Every other seed file in the repo (integration test, walkthroughs) used `'active'`; the bug was in the fixture from the day slice 082 shipped, invisible until slice 111 un-skipped.

H2 (testid drift) ruled out: `upcoming-panel.tsx:98` renders `data-testid="upcoming-row"` exactly as the spec asserts.

Fix shape (`fixtures/e2e/dashboard.sql`):

- `status` flipped `'approved'` → `'active'`
- `ON CONFLICT DO NOTHING` → `ON CONFLICT (id) DO UPDATE SET status, expires_at, activated_by, activated_at` per **slice 168 precedent** — a stale row from a prior local run would otherwise survive re-seeding and re-introduce the bug
- Added `activated_by` / `activated_at` for active-state narrative consistency (SoD-distinct identity vs requester)

Decisions log at `docs/audit-log/193-dashboard-upcoming-fixture-decisions.md` captures D-1 verdict reasoning, D-2 ON CONFLICT precedent, D-3 SoD constraint analysis, D-6 spillover check (no other dashboard.spec failures beyond AC-5).

Verification: local DB-contract test (ephemeral Postgres + full migrations + base seed + dashboard fixture) ran the actual `ListUpcomingItems` exception branch query — returned 1 row with `category='exception'`, satisfying every AC-5 assertion. Idempotent re-seed test confirmed `ON CONFLICT DO UPDATE` resets polluted state.

Also surfaced a CI path-filter gap: `.github/workflows/ci.yml` `changes` filter doesn't include `fixtures/**`, so fixture-only PRs short-circuit the Playwright job to its stub sibling. Filed as **slice 194** (`docs/issues/194-ci-path-filter-fixtures-coverage.md`). Worked around on this PR by adding a slice-193 preamble note to `web/e2e/dashboard.spec.ts` (spec body unchanged — assertion not relaxed).

CI `Frontend · Playwright e2e` on PR #446 is the binding gate.

| Row | Transition                  | Evidence                                                                                                                |
| --- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 193 | `in-progress` → `in-review` | Spillover from slice 111; H1 verdict; fix + decisions log shipped via PR #446; local DB-contract test PASSES            |
| 194 | (new row) → `ready`         | Spillover from 193: CI path filter missing `fixtures/**`. Spec at `docs/issues/194-ci-path-filter-fixtures-coverage.md` |

## Drift detected — 2026-05-21 (batch 88 claim-stake · 189 + 193 → in-progress)

Continuous-loop batch 88 picks two slices in parallel:

- **Slice 189** (`/oauth/authorize` + PKCE + frontend OAuth) — Backend/Auth + Frontend · JUDGMENT · 2-3d. Auth-substrate-v2 spine slot 3; unblocked by 188 merging at `c0e7531`. Ships `/oauth/authorize` handler + PKCE S256 (RFC 7636) + auth_code redemption + 2 new migrations.
- **Slice 193** (dashboard.spec AC-5 fix) — Quality · JUDGMENT · 0.5d. Spillover from slice 111. Diagnose H1 (fixture gap) vs H2 (testid drift).

Also fixes 2 status reconciliation gaps left by batch 87's reconcile:

1. Slice 189 was still `not-ready` post-188-merge; flipped directly to `in-progress` here.
2. Slice 193's canonical row was never added (only the spec file landed via #443); added here as `in-progress`.

Conflict-safety: 189 backend handlers + migrations + `web/lib/auth/` + `web/app/oauth/`; 193 e2e fixture + `web/e2e/dashboard.spec.ts`. Both touch `web/` but disjoint subtrees. Both touch `_STATUS.md` (predictable rebase). 189 has 2 new migrations; 193 has none. Confidence ~85%.

| Row | Transition                  | Evidence                                                                                            |
| --- | --------------------------- | --------------------------------------------------------------------------------------------------- |
| 189 | `not-ready` → `in-progress` | Gate cleared by 188 merge (c0e7531); worktree `backend/189-oauth-authorize-pkce` cut from `810be4e` |
| 193 | (new row) → `in-progress`   | Canonical row added; worktree `quality/193-dashboard-upcoming-fixture` cut from `810be4e`           |

## Drift detected — 2026-05-21 (batch 87 reconcile · 111 + 188 → merged · spillover 193 filed)

Continuous-loop batch 87 closeout. Two slices shipped, one spillover slice filed:

- **Slice 111** (Enable full assertions in `dashboard.spec.ts`) merged at `8c3b355` via PR #441 · UNSTABLE (1/11 dashboard test bodies failed — exactly the bug-detection mechanism the un-skip was designed for). Engineer reused slice-164 `authedPage` fixture pattern. Shipped in ~25 min.
- **Slice 188** (OAuth `/oauth/token` + RFC 8693 token exchange) merged at `c0e7531` via PR #442 · UNSTABLE (Playwright failure inherited from slice 111's un-skip surface; all 11 required-checks PASS). 18 files / +2589 lines · ~70 min engineer wall-clock · D1-D4 decisions logged · threat-model has-mitigations. Auth-substrate-v2 spine slot 2 now landed; **unblocks slice 189** (`/oauth/authorize` + PKCE + frontend OAuth).
- **Slice 193** (spillover) filed via PR #443 — diagnoses + fixes the `dashboard.spec.ts` AC-5 `upcoming-row` failure that slice 111's un-skip caught. Two hypotheses (H1 fixture gap; H2 testid drift); ready for next batch to pick up.

Process learnings (this batch):

1. **JUDGMENT-slice ship-now sentinel held**. Slice 188 engineer shipped clean PR + status-flip without stalling at security-review (the slice-187 pattern that we trained against). Stronger sentinel language in engineer prompts is working.
2. **GitGuardian webhook lag on force-push**. Slice 188's first force-push didn't trigger GitGuardian. Resolution: orchestrator pushed an empty commit to nudge the webhook. Documented for future batches — if a force-push lands and GitGuardian doesn't show up after ~5 min, push an empty no-op commit.
3. **Spillover-as-slice via orchestrator**. Engineer 111 didn't run tests locally before push (skipped the `cd web && npm run test:e2e` step), so the dashboard test failure surfaced in CI rather than during slice 111's session. Orchestrator filed slice 193 on the engineer's behalf per Amendment 2. Pattern matches slice 165/166/168/170/171 chain — slice 111's un-skip caught the bug exactly as designed.

Updated ready queue post-merge:

- **Slice 189** newly-ready (gate `188 merged` satisfied)
- **Slice 193** ready (spillover from 111)
- **Slices 112-115** still gated on `111 merged + 5 more clean post-111 Playwright runs` — need new soak window to accrue
- **Slices 190, 191, 192** gated on predecessors in spine chain (189 → 190 → 191 → 192)

| Row | Transition             | Evidence                                                                                      |
| --- | ---------------------- | --------------------------------------------------------------------------------------------- |
| 111 | `in-review` → `merged` | merged at `8c3b355` via PR #441; UNSTABLE (Playwright caught real bug → slice 193 spillover)  |
| 188 | `in-review` → `merged` | merged at `c0e7531` via PR #442; 25/25 ACs · 11/11 P0s; auth-substrate-v2 spine slot 2 closed |

## Drift detected — 2026-05-21 (batch 87 claim-stake · 111 + 188 → in-progress)

Continuous-loop batch 87 picks two slices in parallel:

- **Slice 111** (Enable full assertions in `dashboard.spec.ts`) — frontend/quality · AFK · 0.25d · pure un-skip work; dashboard.sql already FULL.
- **Slice 188** (OAuth `/oauth/token` + RFC 8693 token exchange) — backend/auth · JUDGMENT · 2-3d · auth-substrate-v2 spine slot 2; ships `client_credentials` grant + token-exchange + oauth_clients + oauth_token_exchanges audit + `atlas oauth issue-client` CLI helper.

Conflict-safety: entirely disjoint file scopes (frontend Playwright spec vs backend auth packages + migrations + CLI). Both touch `_STATUS.md` (predictable rebase). 188 has 2 new migrations; 111 has none. 188 adds chi routes; 111 doesn't. Confidence: ~95%.

| Row | Transition              | Evidence                                                                 |
| --- | ----------------------- | ------------------------------------------------------------------------ |
| 111 | `ready` → `in-progress` | Worktree `quality/111-dashboard-spec-full-assertions` cut from `b5e84ad` |
| 188 | `ready` → `in-progress` | Worktree `backend/188-oauth-token-endpoint` cut from `b5e84ad`           |

## Drift detected — 2026-05-21 (auth-substrate-v2 spine specs filed + slice 111 unblocked)

Two unrelated queue unblocks in a single PR:

### Spine specs 188-192 filed

OQ #21 Reading D (2026-05-20) committed atlas to the OAuth Authorization Server architecture but only slice 187 (foundation) had a spec file. Slices 188-192 were sketched in 187's spec's "spine continues" notes but never written up. This PR files all 5 specs:

| #   | Title                                                      | Estimate | Key shape                                                                                                                                         |
| --- | ---------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 188 | `/oauth/token` + token-exchange                            | 2-3d     | client_credentials + RFC 8693 token-exchange (tenant switch primitive) + oauth_clients table + oauth_token_exchanges audit                        |
| 189 | `/oauth/authorize` + PKCE + frontend OAuth client          | 2-3d     | RFC 7636 PKCE S256-only + auth_code redemption + open-redirect prevention via registered URI table + frontend oauth-client.ts                     |
| 190 | JWT validation middleware + revoke + introspect            | 2-3d     | CUTOVER SLICE — JWT middleware on `/v1/*` (coexists with legacy bearer); RFC 7009 + RFC 7662 endpoints; R2 = pure claim check (eventual eviction) |
| 191 | SDK migration + CLI device-code + legacy retirement        | 3-4d     | Go/Python/TS OAuth clients + RFC 8628 device-code CLI + slice 034 bearer middleware RETIRED + 410 Gone deprecation responder                      |
| 192 | Multi-tenant switch + frontend switcher (closes slice 141) | 2-3d     | `<TenantSwitcher>` dropdown + GET /v1/me/tenants + bootstrap branch + membership-removed UX banner + HIDDEN when single-tenant                    |

Each spec carries 20-36 ACs, 10-11 P0 anti-criteria, threat model, decisions log directives. Strict dep chain (188 → 189 → 190 → 191 → 192) — they pick up one at a time as predecessors merge. Slice 188 is the only one currently `ready`; the rest are `not-ready` with explicit gates.

Total spine effort: 11-16d at engineer wall-clock (typical engineer productivity multiplier: assume each takes 1-2 hours of agent wall-clock).

### Slice 111 unblocked

Slice 111's gate was "5 clean post-082 runs of `Frontend · Playwright e2e`". Live audit 2026-05-21: **56 consecutive clean Playwright runs** on main since the last failure at 2026-05-20T01:46:19Z (slice 172). The 5-clean-runs threshold is comfortably satisfied. Slice 111 row flipped `not-ready` → `ready`.

Slice 111 is a 0.25d AFK un-skip on `web/e2e/dashboard.spec.ts`. Once 111 merges, slices 112-115 unblock (each gated on "111 + 5 clean post-082 runs"); 116 caps the chain.

### Loop status

The loop hit GUARD-1 four times in a row after batches 85+86 closed (slices 187+155+173 shipped). This PR repopulates the queue with **2 newly-ready slices** (111, 188) — enough to fire the loop again.

| Row | Transition              | Evidence                                                   |
| --- | ----------------------- | ---------------------------------------------------------- |
| 111 | `not-ready` → `ready`   | Soak gate satisfied (56 consecutive clean Playwright runs) |
| 188 | (new row) → `ready`     | Canonical row added; spine slot 2; dep 187 merged          |
| 189 | (new row) → `not-ready` | Canonical row added; gated on 188 merged                   |
| 190 | (new row) → `not-ready` | Canonical row added; gated on 189 merged                   |
| 191 | (new row) → `not-ready` | Canonical row added; gated on 190 merged                   |
| 192 | (new row) → `not-ready` | Canonical row added; gated on 191 merged                   |

## Drift detected — 2026-05-21 (batch 86 reconcile · 173 → merged)

Continuous-loop batch 86 closeout. Slice 173 (MCP server write tools + HITL approval flow) shipped end-to-end:

- Merged at `34508fa` via PR #437 (CLEAN first-try post-rebase)
- 16/16 ACs · 8/8 P0 anti-criteria · 76/76 PRD ISC criteria
- D1-D7 decisions logged · final STRIDE has-mitigations (HOLD-pending-review cleared)
- 5 MCP write tools shipped + applier inside confirm transaction + CHECK constraint enforcing AI-assist invariant at DB layer
- ~75 min engineer wall-clock
- Engineer shipped CLEANLY (no stall) — stronger ship-now sentinel in the engineer prompt worked

Process note: the engineer's `chore(status): 173 -> in-review` flip commit was eaten by rebase 3-way merge (no conflict raised; git auto-resolved by keeping main's `in-progress` view). Net effect: the `in-review` intermediate state was skipped on main — row went directly from `in-progress` → `merged` in this reconcile. Same shape may surface on future rebased single-flip commits; consider adding a post-rebase sanity check in the orchestrator for the row's status column.

| Row | Transition                                   | Evidence                                                                                                   |
| --- | -------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| 173 | `in-progress` → `merged` (in-review skipped) | merged at `34508fa` via PR #437; MCP write tools + HITL approval flow shipped; ~75 min engineer wall-clock |

## Drift detected — 2026-05-21 (batch 86 claim-stake · 173 → in-progress · SOLO JUDGMENT)

Continuous-loop batch 86 SOLO JUDGMENT pick. Slice 173 (MCP server write tools + HITL approval flow) was the only ready candidate with deps merged (124 row entries in earlier drift blocks are historical; slice 124 itself is `merged` at row 3780). Picked SOLO because no parallel-safe peer exists in the ready set right now.

D1 (HITL pattern) pre-locked 2026-05-20 to Pattern A (draft-then-confirm; mirrors slice-138 exception flow). Preliminary STRIDE was tagged HOLD-pending-review — engineer must re-run STRIDE as the first design-phase task before TDD.

| Row | Transition              | Evidence                                                                                        |
| --- | ----------------------- | ----------------------------------------------------------------------------------------------- |
| 173 | `ready` → `in-progress` | Worktree `backend/173-mcp-server-write-tools` cut from `fbdc365`. Engineer subagent to pick up. |

## Drift detected — 2026-05-21 (batch 85 reconcile · 187 + 155 → merged)

Continuous-loop batch 85 closeout. Both slices shipped end-to-end:

- **Slice 187** (OAuth AS scaffolding) merged at `ac42517` via PR #432 · auth-substrate-v2 spine foundation · OQ #21 Reading D commitment now load-bearing. Engineer stalled once at security-review phase (classic "review output as deliverable" pattern); orchestrator resumed with explicit ship-now directive. Post-rebase `_STATUS.md` 3-way merge resolved manually (canonical recipe documented in this drift block for future batches). 20/20 ACs · 11/11 P0 anti-criteria honored. Unblocks slices 188-192.
- **Slice 155** (Questionnaire tracer-bullet) merged at `12da637` via PR #433 · v1 questionnaire response feature locked-scope ship. Three post-rebase CI failures fixed by orchestrator: (a) `questionnaire_questions.scf_anchor_id` + `answer_library.scf_anchor_id` FK type mismatch — engineer keyed TEXT scf_id against UUID id target; relaxed to free-form TEXT for tracer-bullet (v2 spillover documented in-migration), (b) openapi-drift — added 7 RouteSpec entries + regen `docs/openapi.yaml` via `just openapi-generate`, (c) `go mod tidy` for `golang.org/x/image` transitive via excelize/v2.

Both PRs landed UNSTABLE on non-required checks only (codecov/patch informational; Self-host bundle pre-existing flake unrelated to slice scope). All required-checks PASS on both.

Process learnings (for future batches):

1. **JUDGMENT-slice stall pattern repeats** at security-review phase. The per-slice template explicitly names this stall shape but engineers continue to surface it. Slice 04's HARD RULE language is correct; the stall happens anyway. Suggests prompt drift; consider a stronger sentinel ("DELIVER PR or ESCALATE, never both") in future engineer prompts.
2. **\_STATUS.md 3-way merge recipe** is now canonical: `git checkout --ours docs/issues/_STATUS.md` then re-apply the slice's specific row flip + run `pre-commit run --files docs/issues/_STATUS.md` to settle prettier column-padding. Took ~3 minutes per slice to resolve.
3. **Migration FK type checks** should be a build-time CHECK before push. The `scf_anchor_id TEXT REFERENCES scf_anchors(id) [UUID]` would have failed locally if the engineer ran `psql -f` against a fresh DB. Consider a pre-commit hook that drains migrations against an ephemeral container for new migration files. Filing as a follow-up consideration; not a slice.

| Row | Transition             | Evidence                                                                                                |
| --- | ---------------------- | ------------------------------------------------------------------------------------------------------- |
| 187 | `in-review` → `merged` | merged at `ac42517` via PR #432; auth-substrate-v2 spine foundation; unblocks 188-192                   |
| 155 | `in-review` → `merged` | merged at `12da637` via PR #433; questionnaire tracer-bullet (orchestrator-fixed FK + openapi + go.mod) |

## Drift detected — 2026-05-20 (batch 85 claim-stake · 187 + 155 → in-progress)

Continuous-loop batch 85 picks two slices in parallel:

- **Slice 187** (OAuth AS scaffolding) — first slot in auth-substrate-v2 spine; unblocks 188-192 → unparks 141. Backend-only, NEW packages `internal/auth/keystore/` + `internal/auth/tokensign/` + `internal/auth/jwt/`, ADR-0003, NO DB schema changes (P0-187-10).
- **Slice 155** (Questionnaire tracer-bullet) — Excel import + manual authoring + AnswerLibrary (SCF-anchor-keyed) + PDF export. Backend+frontend, NEW questionnaire schema migration, NEW `/v1/questionnaires` API, NEW frontend page. First step: deliver `Plans/mockups/questionnaire.html` against locked scope.

Conflict-safety: completely disjoint packages. Both touch `_STATUS.md` (predictable rebase conflict). Both register chi routes on `internal/api/httpserver.go` — but both add NEW routes (one for JWKS+discovery, one for `/v1/questionnaires`), so rebase is line-append three-way merge per canonical pattern. Migration sequence: 187 ships none; 155 ships one. Confidence: ~85%.

| Row | Transition              | Evidence                                                                                      |
| --- | ----------------------- | --------------------------------------------------------------------------------------------- |
| 187 | `ready` → `in-progress` | Worktree `backend/187-oauth-as-scaffolding` cut from `e4b9cb4`. Engineer subagent to pick up. |
| 155 | `ready` → `in-progress` | Worktree `feature/155-questionnaire-tracer` cut from `e4b9cb4`. Engineer subagent to pick up. |

## Drift detected — 2026-05-20 (OQ #21 Reading D resolution · 141 PARKED · 187 filed)

Canvas OQ #21 (authentication substrate) resolved 2026-05-20 with Reading D — internal OAuth 2.0 Authorization Server with JWT access tokens carrying tenant-in-claim (RFC 9068 + RFC 8693). Maintainer rationale: "no users yet; don't implement just to replace." Pre-PMF commitment to standards-based architecture instead of shipping a bespoke session model.

Two-step transition:

1. **Slice 141 PARKED** (status from batch 84 SOLO → ). The escalation-driven design ambiguity is resolved at the canvas level rather than the slice level. Original 141 spec retained as historical context; the data-model + bootstrap + R2 + switch all move into the auth-substrate-v2 spine (slices 187-192). Slice 192 fulfills 141's vCISO success criterion via OAuth token-exchange.

2. **Slice 187 filed** as the auth-substrate-v2 spine foundation. Ships JWT signing infrastructure + JWKS + OIDC discovery + claim types + ADR-0003. Foundation-only; no endpoints beyond JWKS + discovery (rest of spine slices 188-192 follow).

| Row | Transition                  | Evidence                                                                                                                                                  |
| --- | --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 141 | `in-progress` → `not-ready` | Spec retired in favor of OQ #21 Reading D. Gate: slice 192 (multi-tenant switch via OAuth token-exchange). Original spec preserved as historical context. |
| 187 | (new row) → `ready`         | Canonical row added. OAuth AS scaffolding (JWT signing + JWKS + discovery); 2-3d JUDGMENT; 20 ACs; 11 P0; spine foundation.                               |

## DRIFT REMNANT — 2026-05-20 (continuous-loop batch 84 claim-stake · 141 → in-progress · SOLO JUDGMENT)

## Drift detected — 2026-05-20 (continuous-loop batch 84 claim-stake · 141 → in-progress · SOLO JUDGMENT)

Continuous-loop batch 84 claims slice 141 as SOLO JUDGMENT iteration. Per `05-parallel-batch.md` line 214, JUDGMENT slices are not a batching constraint, but 141 is solo by virtue of its scope (multi-tenant SESSION rewrite touches schema + auth + frontend) which would conflict with 173 (spine touch on AI-assist schema invariant) or 155 (frontend shell overlap). Picking 141 SOLO matches the slice 084 precedent — high-risk pick + focused engineer attention.

| Row | Transition              | Evidence                                                                                                  |
| --- | ----------------------- | --------------------------------------------------------------------------------------------------------- |
| 141 | `ready` → `in-progress` | Engineer claim on `backend/141-multi-tenant-login-and-switcher` (cut from `4f0ded3`). 3-4d JUDGMENT SOLO. |

## DRIFT REMNANT — 2026-05-20 (batch 83 SOLO reconcile · 084 → merged · cosign v3 cascade resolved on first try)

## Drift detected — 2026-05-20 (batch 83 SOLO reconcile · 084 → merged)

Batch 83 SOLO — the slice the loop deferred 6+ ticks shipped CLEAN on first try.

Slice 080 iteration-5 confusion about `--new-bundle-format` was resolved by reading cosign v3.0.x release notes end-to-end: the flag is **disallowed in cosign v3.0.6** (sigstore/cosign#4762). No v2-compat opt-out exists; full `--bundle=${signature}` migration was the only forward path. Engineer also migrated consumer `verify-blob` docs at `RELEASE_READINESS.md §11.1` in the same PR (AC-3 allowed). Test-tag `v0.0.0-slice084-test` round-trip workflow `26200163638` green in 7m26s on first try.

Pre-slice-084 releases (v1.0-v1.12) keep their `.pem` + `.sig` assets and require cosign v2.x to verify — immutable per slice 080 P0-A1; cutover note added to §11.1.

| Row | Transition               | Evidence                                                                                               |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------ |
| 084 | `in-progress` → `merged` | PR #425 at `f15839a` (CLEAN first-try; ~18 min engineer wall-clock). All 5 ACs PASS; all 3 P0 honored. |

**Counts delta:** in-progress −1 · merged +1. Remaining ready queue: 141 + 155 + 173 (all 3-5d JUDGMENT solos). Natural wind-down point.

## Drift detected — 2026-05-20 (continuous-loop batch 83 claim-stake · 084 → in-progress · SOLO)

Continuous-loop batch 83 claims slice 084 as a SOLO iteration. After batches 78-82 drained all easy AFK pairs, the remaining ready queue contained 084 (high-escalation-risk infra) + three 3-5d JUDGMENT solos. Maintainer's repeated /loop ticks after the wind-down signal communicated "keep going" — picking 084 (the only AFK-pickable) is the natural next move. Engineer escalates cleanly if the cosign v3 cascade hits the same wall slice 080 documented.

| Row | Transition              | Evidence                                                                                                                                           |
| --- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| 084 | `ready` → `in-progress` | Engineer claim on `infra/084-cosign-v3-goreleaser-v7` (cut from `1b1c152`). cosign v3 + goreleaser-action v7 coordinated migration; 0.5d AFK SOLO. |

## Drift detected — 2026-05-20 (batch 82 reconcile · 183 + 186 → merged · ALL UI-honesty findings from slice 178 closed)

Continuous-loop batch 82 — closes the last UI-honesty findings from slice 178's first-pass audit.

- **Slice 186** (sidebar Admin role-gate) shipped CLEAN via PR #421 at `471f779`. ~8 min engineer wall-clock. `shouldShowAdminEntry` predicate follows slice-130 `canReachAuditLog` pattern. F-178-6 closed.
- **Slice 183** (calendar dead-links + dashboard mockup refresh) shipped via PR #422 at `f40fa6f` after rebase. ~14 min engineer wall-clock. `linkFor` refactored to tagged-union return with compile-time `assertNever` exhaustiveness — F-178-1/2/3 closed structurally. Exception/policy events render `<span title>` (no dead links). F-178-7/8 dashboard mockup refresh removed Vendors + Admin sidebar placeholders.

**Slice 178 first-pass audit FULLY CLOSED:** all 8 findings (F-178-1 through F-178-8) resolved across slices 184 / 185 (batch 81) + 183 / 186 (batch 82).

| Row | Transition               | Evidence                                                                                                                                               |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 183 | `in-progress` → `merged` | PR #422 at `f40fa6f` (CLEAN after rebase against 186-merged main). 9 files; tagged-union `linkFor` + dashboard mockup refresh; F-178-1/2/3/7/8 closed. |
| 186 | `in-progress` → `merged` | PR #421 at `471f779` (CLEAN first-try). 5 files; `shouldShowAdminEntry` predicate + async-server-component Sidebar; F-178-6 closed.                    |

**Counts delta:** in-progress −2 · merged +2.

## Drift detected — 2026-05-20 (continuous-loop batch 82 claim-stake · 183 + 186 → in-progress)

Continuous-loop batch 82 claimed slices 183 + 186 (remaining 2 UI-honesty spillovers from slice 178's first-pass audit). Both AFK; conflict-safe except CHANGELOG.

| Row | Transition              | Evidence                                                                                                                                                   |
| --- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 183 | `ready` → `in-progress` | Engineer claim on `frontend/183-calendar-deadlinks-and-dashboard-mockup` (cut from `dbef2f6`). Calendar dead-links + dashboard mockup refresh; 0.5-1d AFK. |
| 186 | `ready` → `in-progress` | Engineer claim on `frontend/186-sidebar-admin-role-gate` (cut from `dbef2f6`). Sidebar Admin role-gate via slice 035 OPA pattern; 0.25d AFK.               |

## Drift detected — 2026-05-20 (batch 81 reconcile · 184 + 185 → merged)

Continuous-loop batch 81 (parallel pair) — fastest shipout yet.

- **Slice 184** (UI honesty: /audits row-click 404) shipped CLEAN via PR #417 at `e058f25`. Option A picked: disable click + honesty banner. Engineer ~6 min wall-clock.
- **Slice 185** (UI honesty: /risks row-click → hierarchy) shipped CLEAN after rebase via PR #418 at `c509ca8`. Option A picked: disable click + actions column with "View in hierarchy →" link preserving `?focus=<id>` workflow + banner. Engineer ~6 min. Rebase against 184-merged main was conflict-free (no CHANGELOG overlap this time — both engineers used `[Unreleased] / Fixed` but at different line positions).

| Row | Transition               | Evidence                                                                                                                 |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| 184 | `in-progress` → `merged` | PR #417 squash-merged at `e058f25` (CLEAN first-try). Option A; F-178-4 closed.                                          |
| 185 | `in-progress` → `merged` | PR #418 squash-merged at `c509ca8` (CLEAN after rebase). Option A with workflow-preservation refinement; F-178-5 closed. |

**Counts delta:** in-progress −2 · merged +2.

## Drift detected — 2026-05-20 (continuous-loop batch 81 claim-stake · 184 + 185 → in-progress)

Continuous-loop batch 81 claimed slices 184 + 185 (both small AFK row-click fixes from slice 178's first-pass audit). Adds canonical Status table rows for the four spillover slices that filed in PR #414's commit but whose canonical rows were missed in the batch 80 reconcile.

| Row | Transition                | Evidence                                                                                                                                                                                                           |
| --- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 183 | (new row) → `ready`       | Canonical row registered (slice doc was filed in PR #414 but reconcile missed the row addition). Calendar dead-link family + dashboard mockup refresh; spillover from slice 178 F-178-1/2/3 + F-178-7/8 (batched). |
| 184 | (new row) → `in-progress` | Canonical row registered AND claimed by continuous-loop engineer. `/audits` row-click 404; spillover from slice 178 F-178-4.                                                                                       |
| 185 | (new row) → `in-progress` | Canonical row registered AND claimed by continuous-loop engineer. `/risks` row-click goes to hierarchy not detail; spillover from slice 178 F-178-5.                                                               |
| 186 | (new row) → `ready`       | Canonical row registered. Sidebar Admin entry shown to non-admin; spillover from slice 178 F-178-6.                                                                                                                |

## Drift detected — 2026-05-20 (batch 80 reconcile · 178 + 180 → merged)

Continuous-loop batch 80 (parallel pair) — shipped with one CodeQL fix needed.

- **Slice 180** (privacy-module foundation) shipped via PR #413 squash-merged at `125205b`. 9 audit-log tables modified; sqlc regen + 8+1 INSERT call-site updates. Slice 124 was already merged at gh#267 — engineer took the "124-before-180" path (per spec AC-8) and updated 124's UNION query to project `subject_module`. ~21 min engineer wall-clock. codecov/patch failed informationally; ignored per branch-protection policy.
- **Slice 178** (UI honesty audit harness) shipped via PR #414 squash-merged at `69ce7df`. Required CHANGELOG.md rebase against 180-merged main + a CodeQL fix on `web/e2e-audit/lib/heuristics.ts` (`js/incomplete-url-scheme-check` — orchestrator replaced enumerated `startsWith` chain with RFC 3986 §3.1 case-insensitive scheme regex). 23 files / +3222 / -1. First-pass audit produced 8 substantive findings (6 HONESTY-GAP + 2 MOCKUP-STALE) filed as spillover slices #183 / #184 / #185 / #186.

| Row | Transition               | Evidence                                                                                                                                                                                                                    |
| --- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 178 | `in-progress` → `merged` | PR #414 squash-merged at `69ce7df` (CLEAN after CHANGELOG rebase + CodeQL fix). 23 files. Reusable Playwright harness; 31 unit tests; first-pass audit filed 4 spillover slices (#183 / #184 / #185 / #186) for 8 findings. |
| 180 | `in-progress` → `merged` | PR #413 squash-merged at `125205b`. 9 audit-log tables get `subject_module` column. Slice 124-before-180 path: 124 already merged at gh#267; engineer updated 124's UNION + wire shape. All 13 ACs PASS.                    |

**Counts delta:** in-progress −2 · merged +2 · spillover +4 (slices 183, 184, 185, 186 filed as `ready` riding along in PR #414's commit).

## Drift detected — 2026-05-20 (continuous-loop batch 80 claim-stake · 178 + 180 → in-progress)

Continuous-loop batch 80 claimed slices 178 + 180 as a parallel pair. Both AFK; conflict-safe (only CHANGELOG.md overlaps, per the batch 78/79 precedent).

| Row | Transition              | Evidence                                                                                                                                          |
| --- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 178 | `ready` → `in-progress` | Engineer claim on branch `quality/178-ui-honesty-audit-harness` (cut from `a1e2286`). Playwright UI honesty harness + first-pass audit; 1.5d AFK. |
| 180 | `ready` → `in-progress` | Engineer claim on branch `infra/180-privacy-module-foundation` (cut from `a1e2286`). OQ #7 foundation pre-commitments; 1d AFK.                    |

## Drift detected — 2026-05-20 (batch 79 reconcile · 174 + 181 → merged)

Continuous-loop batch 79 (parallel pair) — clean shipout.

- **Slice 181** (open-governance pre-commitments) shipped CLEAN via PR #409 squash-merged at `67092db`. 6 files. GOVERNANCE.md 316 lines / ~1980 words. ~15 min engineer wall-clock.
- **Slice 174** (UCF anchor catalog export) shipped via PR #410 squash-merged at `79eb829`. Required CHANGELOG.md rebase against 181-merged main (predicted from claim-stake conflict analysis). 20 files / new migration `20260520010000_anchors_export_meta_audit.sql` for `anchors_export` action. Engineer ~28 min wall-clock + orchestrator rebase.

Both AFK budgeted (0.5d + 1-2d); both engineers came in well under budget.

| Row | Transition               | Evidence                                                                                                                                                                                          |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 174 | `in-progress` → `merged` | PR #410 squash-merged at `79eb829` (CLEAN after CHANGELOG rebase). 20 files. 3-format anchor export shipped per D1; reuses slice 135 export library; 50K-anchor streaming-memory budget asserted. |
| 181 | `in-progress` → `merged` | PR #409 squash-merged at `67092db` (CLEAN first-try). 6 files. GOVERNANCE.md 316 lines; re-evaluation locked at 2028-05-20 OR 100 self-hosts.                                                     |

**Counts delta:** in-progress −2 · merged +2.

## Drift detected — 2026-05-20 (continuous-loop batch 79 claim-stake · 174 + 181 → in-progress)

Continuous-loop batch 79 claimed slices 174 + 181 as a parallel pair. Both AFK; conflict analysis clean (only CHANGELOG.md overlaps, manageable in reconcile per batch-78 precedent).

| Row | Transition              | Evidence                                                                                                                              |
| --- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| 174 | `ready` → `in-progress` | Engineer claim on branch `backend/174-ucf-anchor-catalog-export` (cut from `54b2f63`). UCF anchor export all-three-formats; 1-2d AFK. |
| 181 | `ready` → `in-progress` | Engineer claim on branch `docs/181-open-governance-pre-commitments` (cut from `54b2f63`). OQ #5 foundation; 0.5d AFK.                 |

## Drift detected — 2026-05-20 (batch 78 reconcile · 179 + 182 → merged)

Continuous-loop batch 78 (parallel pair) — clean shipout.

- **Slice 182** (board-narrative AI-assist foundation) shipped CLEAN via PR #405 squash-merged at `6622e8e`. 5 files / +490 / -0. 42 tone-anti-pattern entries (well above 15-entry AC-2 floor). ADR slot incremented from spec's `0002` to `0006` per "ADR slot computation" guidance (slots 0002-0005 were occupied). Engineer ~10 min wall-clock.
- **Slice 179** (`schema-removal-age` CI check) shipped via PR #406 squash-merged at `f06c323`. Required a CHANGELOG.md rebase against 182-merged main (predicted CHANGELOG overlap from the claim-stake conflict analysis). 8 files. Script + companion test harness (27 assertions / 8 fixture cases). New CI job `Schema · removal-age (90-day floor)` informational only. Live AC-11 smoke test (PR's own filter match) fired the new job and exited count=0 cleanly. Engineer ~11 min wall-clock + orchestrator rebase.

Both 0.5d AFK budgeted; both engineers came in well under.

| Row | Transition               | Evidence                                                                                                                                                                                                              |
| --- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 179 | `in-progress` → `merged` | PR #406 squash-merged at `f06c323` (CLEAN after CHANGELOG.md rebase against 182-merged main). 8 files. Script + 27 fixture-driven test assertions. New CI job is informational (NOT in required-checks per P0-179-4). |
| 182 | `in-progress` → `merged` | PR #405 squash-merged at `6622e8e` (CLEAN first-try). 5 files / 490+ / 0-. 42 tone-anti-pattern entries. ADR slot 0006 (incremented per spec's guidance). 4 model references kept in sync per P0-182-6.               |

**Counts delta:** in-progress −2 · merged +2.

## Drift detected — 2026-05-20 (continuous-loop batch 78 claim-stake · 179 + 182 → in-progress)

Continuous-loop batch 78 claimed slices 179 + 182 as a parallel pair. Both are 0.5d AFK; conflict analysis clean (only CHANGELOG.md overlaps, manageable in reconcile).

| Row | Transition              | Evidence                                                                                                                        |
| --- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 179 | `ready` → `in-progress` | Engineer subagent claim on branch `infra/179-schema-removal-age-ci-check` (cut from `8b267dd`). OQ #9+#17 implementation half.  |
| 182 | `ready` → `in-progress` | Engineer subagent claim on branch `docs/182-board-narrative-ai-assist-foundation` (cut from `8b267dd`). OQ #14 foundation work. |

## Drift detected — 2026-05-20 (tier A+B maintainer decisions · 084 + 155 + 174 → ready · 141 pre-confirm)

Maintainer worked through the Tier A + B pending design decisions surfaced post-batch-77. Four slices touched in one design-session:

- **174** (UCF anchor catalog export) — demand gate cleared + D1 locked: ship all three formats (`format=json` nested, `format=xlsx` two-sheet, `format=csv` flat-nested fallback). Status `not-ready` → `ready`. Type AFK (D1 lock removed the JUDGMENT call). Estimate unchanged at 1-2d.
- **155** (Questionnaire feature) — scope locked to TRACER-BULLET: Excel-only import + manual authoring + AnswerLibrary skeleton + PDF export. CSV / JSON / Word / HECVAT / vendor portal / AI-assist all deferred as spillovers. Load-bearing decision pre-locked: AnswerLibrary keys on SCF anchor IDs (not free-text). Status `not-ready` → `ready`. Estimate 3-5d → 3-4d.
- **084** (cosign v3 + goreleaser-action v7) — maintainer green-lit; no real users yet → no release-window blast-radius concern → proceed as solo iteration. Status `not-ready` → `ready`. Estimate unchanged at 0.5d.
- **141** (Multi-tenant login) — was already `ready`; maintainer pre-confirms the `atlas_auth` + non-RLS `user_tenants` relaxation as sanctioned. Engineer no longer needs to escalate this constitutional question at design-grill.

**Counts delta:** not-ready −3 · ready +3 (141 unchanged).

| Row | Transition                    | Evidence                                                                                                                                                                                                                                                      |
| --- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 084 | `not-ready` → `ready`         | Maintainer green-lit migration; no real-user release-window concern. AFK · 0.5d. Solo iteration; round-trip via `v0.0.0-slice084-test` throwaway tag.                                                                                                         |
| 155 | `not-ready` → `ready`         | Scope locked tracer-bullet: Excel-only + manual + AnswerLibrary skeleton + PDF. AnswerLibrary keys on SCF anchors (load-bearing design call pre-resolved). Engineer's first step: deliver `questionnaire.html` mockup against locked scope. AFK · 3-4d.       |
| 174 | `not-ready` → `ready`         | Demand gate cleared + D1 locked: ship all three formats (json nested, xlsx two-sheet, csv flat-nested fallback). Type flipped JUDGMENT → AFK. AFK · 1-2d.                                                                                                     |
| 141 | `ready` → `ready` (annotated) | Maintainer pre-confirm: `atlas_auth` role + non-RLS `user_tenants` relaxation sanctioned. Engineer cleared to proceed without escalating at design-grill. CLAUDE.md "RLS everywhere" wording will be amended post-merge to note this single global exception. |

## Drift detected — 2026-05-20 (batch 77 reconcile · 176 + 177 → merged)

Continuous-loop batch 77 (parallel pair) — clean shipout.

- **Slice 177** (exceptions list-page UI) shipped CLEAN via PR #393 squash-merged at `ee0163a`. 12 files / +1781 / -179. 588/588 vitest pass (29 new tests). List endpoint pre-existed (slice 021); UI + BFF proxy + Export buttons consume slice-138's exceptions backend cleanly. Engineer ~55 min wall-clock.
- **Slice 176** (logo theme coupling + README docs refresh) shipped CLEAN via PR #394 squash-merged at `6eb0701`. Engineer ~35 min impl + ~10 min orchestrator rebase against 177-merged main. 11 files / +518 / -61. D1 = hybrid of A+B: `<ThemeAwareLogo>` reads `<html data-theme>` via `MutationObserver` + matchMedia fallback (slice 170 writes `data-theme`, not Tailwind `dark:` class). docs/images/logo-\*.png regenerated from slice-167 SVGs (4638B / 4746B gzipped — under 16 KB cap). All 10 P0 anti-criteria honored.

| Row | Transition               | Evidence                                                                                                                                                                                                                  |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 176 | `in-progress` → `merged` | PR #394 squash-merged at `6eb0701` (CLEAN after orchestrator rebase). 11 files / 518+ / 61-. D1 = hybrid A+B (data-theme MutationObserver + matchMedia fallback). docs/images/logo-\*.png regenerated. All 10 P0 honored. |
| 177 | `in-progress` → `merged` | PR #393 squash-merged at `ee0163a` (CLEAN first try). 12 files / 1781+ / 179-. List endpoint pre-existed (slice 021); UI + BFF proxy + Export buttons. 588/588 vitest. ~55 min engineer wall-clock.                       |

**Counts delta:** in-progress −2 · merged +2.

## Drift detected — 2026-05-20 (post-batch-76 row-flips · 176 + 177 → ready)

Maintainer-merged three PRs in quick succession (133 fix, 134 re-scope, 176 doc) but two `ready` flips weren't covered:

- **Slice 176** (logo theme coupling) — doc landed via PR #386 squash-merged at `5e33027`, but no canonical row was added to the Status table at the time. Adding the row here at `ready` (deps merged: #167 + #170).
- **Slice 177** (exceptions list-page UI) — gate cleared when slice 138 merged at `a171a0a`, but the canonical row stayed at `not-ready`. Flipping to `ready`.

| Row | Transition            | Evidence                                                                                                                                                                       |
| --- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 176 | (new) → `ready`       | slice doc landed via maintainer-merge PR #386 at `5e33027`; logo theme coupling + README/docs refresh (0.5d AFK); deps merged (#167 logo design + #170 theme picker hydration) |
| 177 | `not-ready` → `ready` | gate cleared by slice 138 merge at `a171a0a` (PR #387); exceptions list-page UI surface; 0.5d AFK                                                                              |

**Counts delta:** new ready +1 (176) · not-ready −1 + ready +1 (177).

## Drift detected — 2026-05-20 (batch 76 reconcile · 133 → merged · 134 ESCALATED + re-scoped)

Continuous-loop batch 76 (parallel pair attempt) outcome:

- **Slice 133** (mkdocs user docs content refresh) shipped CLEAN via PR #389 squash-merged at `b64cb06`. 14 files / +2160 / -178. 10 new mkdocs pages (6 primitives + audit-log trio + CI hardening + connector authoring + section nav). `mkdocs build --strict` green: 0 warnings, 0 broken links, 0 unresolved references. Engineer ~60 min vs 2-3d budget.
- **Slice 134** (originally "in-app walkthrough refresh") — engineer ESCALATED cleanly with full diagnostic: the original slice doc described browser-driven product tours which **don't exist in the codebase**. The only walkthrough-named concepts are slice 027 (auditor-evidence recorder — different) + slice 070 (showboat onboarding markdown docs). Slice-070 walkthroughs HAVE drifted (24 stale path occurrences across 5 files; captured 2026-05-16) but refresh is constitutionally HITL (slice 070 P0-A4 forbids hand-authored outputs). Maintainer chose option B (re-scope doc); slice 134 is now an HITL slice via PR #390 (open).

| Row | Transition                  | Evidence                                                                                                                                                                                                                                                                                       |
| --- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 133 | `in-progress` → `merged`    | PR #389 squash-merged at `b64cb06` (CLEAN). 14 files / 2160+ / 178-. 10 new mkdocs pages. mkdocs --strict green. ~60 min engineer wall-clock vs 2-3d budget.                                                                                                                                   |
| 134 | `in-progress` → `not-ready` | Engineer escalated cleanly (no commits); original slice doc described nonexistent feature. Re-scoped via PR #390 to HITL (slice-070 walkthroughs refresh via interactive `uvx showboat exec` replay). Loop cannot pick up unattended; gate: maintainer schedules interactive operator session. |

**Counts delta:** in-progress −2 · merged +1 · not-ready +1 (134 re-flipped after escalation).

## Drift detected — 2026-05-19 (batch 75 reconcile · 138 → merged · 4-entity export cluster closes)

Continuous-loop batch 75 (solo). Slice 138 (ledger entities export — evidence + policies + exceptions + samples) shipped via PR #387 squash-merged at `a171a0a`. UNSTABLE merge (only `codecov/patch` informational; required-checks all green).

**Slot collision resolved**: engineer initially filed spillover as slice 176, but slice 176 was already claimed by the logo theme-coupling slice (PR #386 filed earlier). Orchestrator renumbered engineer's spillover to **slice 177** via worktree rename + reference fix-up in slice doc + decisions log + CHANGELOG + \_STATUS row before push.

Engineer's design decisions (8 captured in decisions log):

- D1 — **Evidence export EXCLUDES `payload_json`** (load-bearing per slice 145 hardening pattern)
- D2-D8 — policies full set / exceptions computed `duration_days` / samples 250K cap / defensive DELETE / exceptions UI deferred to slice 177 / plural action-value spelling / inline SQL

4 endpoints + 4 BFFs + 3 UI Export buttons (exceptions UI deferred to slice 177). All 4 packages on coverage-gate excludes list. ~90 min engineer wall-clock.

| Row | Transition               | Evidence                                                                                                                                                                                                                                                                                                       |
| --- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 138 | `in-progress` → `merged` | PR #387 squash-merged at `a171a0a` (UNSTABLE — codecov/patch informational). 4 entity exports shipped. Migration `20260520000010_ledger_entities_export_meta_audit.{sql,down.sql}` with defensive DELETE + post-137 baseline restoration. Engineer ~90 min wall-clock; orchestrator ~5 min slot-collision fix. |
| 177 | (new) → `not-ready`      | filed by slice 138 engineer (originally slot 176; renumbered to 177 by orchestrator due to PR #386 collision); exceptions list-page UI surface; gate: 138 merged (now cleared — can flip to `ready` in next reconcile pass)                                                                                    |

**Counts delta:** in-progress −1 · merged +1 · new not-ready +1 (177 — should flip to ready since 138 just merged).

## Drift detected — 2026-05-19 (batch 74 reconcile · 137 → merged · controls UCF graph export)

Continuous-loop batch 74 (solo). Slice 137 (controls UCF graph data export) shipped via PR #384 squash-merged at `4300cde`. UNSTABLE merge — only `codecov/patch` failing (informational; NOT in required-checks per `.github/branch-protection.json`).

Engineer's design decisions:

- **D1 = flat projection** (one row per (anchor, requirement, framework) tuple). Rejected nested + two-sheet XLSX because CSV/JSON consumers don't need to handle non-flat shape; XLSX consumers can reconstruct topology client-side.
- D2-D8: column set + row cap (500K) + streaming-memory budget (≤200 MB) + in-package placement + defensive DELETE + plural `controls_export` action name + constructor surface + UI button placement (all in decisions log).

**Notable iteration**: first push failed `Go · build + test` (coverage gate: `internal/api/controls` at 22.1% < 26.0% floor). Per CLAUDE.md "raise the floor only by writing tests in same PR" — engineer added 12 new unit tests targeting handler dispatch + format validation + auth + role gate + degraded paths. Coverage rose to 27.3%. Required-checks all green; only `codecov/patch` (not required) stayed red.

Slice 137 ships **21 files / 2853+ / 190-**. Engineer ~75 min initial + ~15 min coverage iteration = ~90 min total.

**Spillovers filed by engineer**: slice 174 (UCF anchor catalog export — nested/two-sheet form factor) + slice 175 (control bundle history export — lineage incl. superseded). Both `not-ready`.

| Row | Transition               | Evidence                                                                                                                                                                                                                                                                                                                   |
| --- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 137 | `in-progress` → `merged` | PR #384 squash-merged at `4300cde` (UNSTABLE — codecov/patch informational). 21 files / 2853+ / 190-. D1 = flat projection. 500K row cap; ≤200 MB streaming-memory. Coverage gate cleared (22.1% → 27.3% after 12 unit-test additions). 2 spillovers filed (174 + 175). ~90 min total wall-clock incl. coverage iteration. |
| 174 | (new) → `not-ready`      | filed 2026-05-19 by slice 137 engineer; spillover for UCF anchor catalog export (nested / two-sheet form factor) — separate from slice 137's flat-projection control-row export                                                                                                                                            |
| 175 | (new) → `not-ready`      | filed 2026-05-19 by slice 137 engineer; spillover for control bundle history export (lineage incl. superseded bundles)                                                                                                                                                                                                     |

**Counts delta:** in-progress −1 · merged +1 · new not-ready +2 (174, 175).

## Drift detected — 2026-05-19 (batch 73 reconcile · 172 → merged · MCP server foundation)

Continuous-loop batch 73 (solo). Slice 172 (MCP server foundation + 6 read-only tools) shipped via PR #382 squash-merged at `95dd94d`. **CLEAN merge** — first-try clean, no orchestrator interventions. Zero CodeQL alerts on the new ~340 LoC hand-rolled JSON-RPC + ~6 tool handlers.

Notable engineer decision: **D2 rejected BOTH external MCP frameworks** (mark3labs/mcp-go AND modelcontextprotocol/go-sdk) in favor of a hand-rolled stdlib JSON-RPC 2.0 implementation. ~340 LoC pure stdlib. Honors P0-A11 ("do NOT bundle the MCP framework dependency into the platform binary") most strictly. Engineer's rationale documented in decisions log: for six tools + three message types the hand-rolled path is the cleanest dep-tree posture; libraries would propagate into every build target.

Other engineer decisions:

- **D1 transport**: stdio (HTTP+SSE deferred to v2)
- **D3 tool surface**: exactly the six listed (evaluated + declined `list_policies`, `list_exceptions`, `get_evidence`)
- **D4 audit strategy**: User-Agent header (`atlas-mcp/<version> (mcp; ai_assisted=read-only)`) — no per-call meta-audit row; that's slice 173's surface for writes

Slice ships **22 files / 3939+ / 175-**. All 13 P0 anti-criteria honored. Engineer ~95 min wall-clock (well under 3-4d budget). Per slice 116 advisory-vs-required pattern, `cmd/atlas-mcp` stays advisory for 30-day soak before any required-checks promotion.

| Row | Transition               | Evidence                                                                                                                                                                                                                                                                                                                                                                      |
| --- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 172 | `in-progress` → `merged` | PR #382 squash-merged at `95dd94d` (CLEAN). 22 files / 3939+ / 175-. D2 = hand-rolled JSON-RPC stdlib (rejected both external MCP frameworks). D1 = stdio. D3 = six tools exactly. D4 = User-Agent header (no per-call audit row for reads). All 13 P0 anti-criteria honored. Zero CodeQL alerts. 30-day soak before required-checks promotion. Slice 173 (writes) unblocked. |

**Counts delta:** in-progress −1 · merged +1. Slice 173 (write tools) dep gate now cleared → can move from `not-ready` to `ready` in a future reconcile pass.

## Drift detected — 2026-05-19 (batch 72 reconcile · 136 + 139 → merged · slices 172/173 rows added)

Batch 72 parallel pair shipped (136 + 139, both data-export entities). PR #380 (slices 172 + 173 docs) merged in parallel via maintainer. Combined reconcile here covers four transitions:

- **Slice 136** (risk register export): shipped via PR #378 at `c80bec3`. CLEAN after 3 orchestrator interventions during conflict resolution + chain-order bug: (a) rebase against post-139 main, (b) migration timestamp collision rename (`20260519000000` → `20260519000010`), (c) down-migration `vendor_export` → `vendors_export` plural-spelling fix, (d) round-trip fix: down migration needed `DELETE FROM me_audit_log WHERE action = 'risk_export'` because `internal/api/risks/` IS in CI's integration-test set (whereas slice 139's `internal/api/admin*` packages are NOT, so 139 didn't need the cleanup).

- **Slice 139** (audit periods + vendors export): shipped via PR #379 at `3522c64`. CLEAN merge first try. ~75 min engineer wall-clock.

- **Slice 172** (MCP server foundation + read tools): slice doc landed via PR #380 at `278ade9` (squash-merged before this reconcile's \_STATUS update could ride along). Canonical row added at `ready`.

- **Slice 173** (MCP server write tools — spillover): slice doc landed via PR #380. Canonical row added at `not-ready` (gated on 172).

| Row | Transition               | Evidence                                                                                                                                                                                                                                   |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 136 | `ready` → `merged`       | PR #378 squash-merged at `c80bec3` (CLEAN after 3 orchestrator fix-ups). 14 files / 2335+ / 176-. Risk register export with 18-column canonical set. Migration renamed to `20260519000010` to avoid collision with 139's `20260519000000`. |
| 139 | `in-progress` → `merged` | PR #379 squash-merged at `3522c64` (CLEAN first try). 20 files / 3176+ / 177-. Two endpoints (audit-periods + vendors); 14 maskEmail unit cases; vendor email masked to `*@domain.tld` at v1.                                              |
| 172 | (new) → `ready`          | slice doc landed via PR #380 at `278ade9`; MCP server foundation + 6 read tools; 18 ACs · 13 P0 anti-criteria; STRIDE has-mitigations; deps merged (#003 + #034 + #145)                                                                    |
| 173 | (new) → `not-ready`      | slice doc landed via PR #380 at `278ade9`; spillover from 172; MCP write tools + HITL approval flow per CLAUDE.md "AI-assist boundary"; preliminary STRIDE = HOLD-pending-review; gate: 172 merged                                         |

**Counts delta:** ready −1 + 1 (136 → merged; 172 added) = 0 net · in-progress −1 (139 → merged) · merged +2 · new not-ready +1 (173).

## Drift detected — 2026-05-19 (batch 71 reconcile · 145 → merged · export hardening)

Continuous-loop batch 71 (solo). Slice 145 (data-export hardening) shipped via PR #375 squash-merged at `637b8dc`. CLEAN merge after one orchestrator fix-up (golangci-lint `unused` rule caught a deprecated wrapper `jsonMarshalOrdered` the engineer left behind for "symmetry"; deleted the dead shim in a 1-line follow-up commit).

Engineer's slice 145 work:

- `?include_payload=<bool>` query param on `/v1/admin/audit-log/export` (default true; non-breaking)
- NEW `internal/export/concurrency.go` package-level semaphore keyed by `(tenant_id, user_id)`, default cap = 2, configurable via `ATLAS_EXPORT_MAX_CONCURRENT_PER_USER`
- 429 with `Retry-After: 30` header + JSON body on cap-exceeded
- Meta-audit records `include_payload` value
- 14 new unit tests + 8 new integration tests (incl. panic-recovery + double-release idempotency)
- CONTRIBUTING.md "Data exports" subsection + CHANGELOG entry

Slices 136-139 (per-entity exports) inherit both behaviors when they implement.

| Row | Transition               | Evidence                                                                                                                                                                                                                                                                                                        |
| --- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 145 | `in-progress` → `merged` | PR #375 squash-merged at `637b8dc`. 12 files / 2128+ / 209-. Engineer ~90 min wall-clock + ~5 min orchestrator lint-fix. D1 = include_payload default true; D2 = concurrency cap default 2. All 10 P0 anti-criteria honored. Zero CodeQL findings on the goroutine-leak surface (sync.Once-guarded release fn). |

**Counts delta:** in-progress −1 · merged +1.

## Drift detected — 2026-05-19 (slice 145 → in-progress · data-export hardening)

Continuous-loop pickup — slice 145 (data-export hardening: `?include_payload` flag + per-(tenant, user) concurrency cap, Backend / AFK / 0.5d, retro-STRIDE follow-on from slice 135). Branch `backend/145-data-export-hardening` claim-staked off main at `f1cae0d`.

Engineer plan: (a) `?include_payload=<bool>` query param on `GET /v1/admin/audit-log/export` (default `true` preserves slice 135 wire shape per P0-HARDEN-1) threaded through to all three encoders (CSV empty cell, JSON `null`, XLSX empty cell); (b) new `internal/export/concurrency.go` with a package-level semaphore keyed by `(tenant_id, user_id)`, size = `ATLAS_EXPORT_MAX_CONCURRENT_PER_USER` (default 2 — D2 JUDGMENT); 429 + `Retry-After: 30` + JSON body on overflow (P0-A10); (c) meta-audit blob gains `include_payload` key (legacy rows without it = default-true, P0-A5); (d) integration test fires 5 concurrent exports against cap=2 → 2 status=200, 3 status=429 (AC-6). BFF (`web/app/api/audit-log/export/route.ts`) needs no change — it's pure passthrough so `?include_payload` flows naturally.

| Row | Transition              | Evidence                                                                                                                                                                                                                                                                                     |
| --- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 145 | `ready` → `in-progress` | branch `backend/145-data-export-hardening` claim-stake off `f1cae0d`; D1 = `include_payload` default true (backwards-compat); D2 = concurrency cap default 2 (per-stream pgxpool pressure already meaningful at 2); decisions log at `docs/audit-log/145-data-export-hardening-decisions.md` |
...
| 137   | `in-progress` | Controls UCF graph export (claim-staked 2026-05-19)                          |
...
| 111 | Enable full assertions in `dashboard.spec.ts`                                                     | `merged`                      | quality/111-dashboard-spec-full-assertions              | gh#441 | 2026-05-21          | 2026-05-21 | batch 87 · parallel-pair w/ 188 · merged at `8c3b355` (UNSTABLE — Playwright failed by design; un-skip caught real bug → spillover slice 193 filed) · spillover from 082 · AFK · 0.25d · ~25 min engineer wall-clock · 11 dashboard test bodies un-skipped via slice-164 `authedPage` fixture pattern · 1/11 test bodies (AC-5 upcoming panel) caught a real fixture/wire bug — exactly the intended detection mechanism for the un-skip · soak gate satisfied 2026-05-21 (56 consecutive clean post-082 runs)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 116 | Promote `Frontend · Playwright e2e` to required-checks                                            | `merged`                      | infra/116-promote-playwright-required-check             | gh#494 | 2026-05-22          | 2026-05-22 | batch 102 · PARALLEL with 175 · merged CLEAN at `963e60e` · 6 files / +209/-12 · D1 engineer JUDGMENT: `ci.yml` INTENTIONALLY UNCHANGED (stub-twin uses same name as real job per slice-061 universal pattern; removing breaks docs-only PR mergeability) · D2 AC-1 "112-115 merged" gate interpreted as satisfied via maintainer-confirmed Playwright soak across 142/143/198/201/202                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 131 | Fix slice 029 integration tests' `SET LOCAL $1` syntax error                                      | `merged`                      | backend/131-set-local-parameterized-fix                 | gh#484 | 2026-05-22          | 2026-05-22 | batch 100 · PARALLEL with 143 · backend (test infra) · AFK · 0.25d · merged at `29ab44d` UNSTABLE (non-required `Self-host bundle · end-to-end (bundled)` failure pre-existing; spillover slice 202 filed) · 2-file diff (CHANGELOG + integration_slice029_test.go); 37+/12- · canonical `tenancy.ApplyTenant` helper replaces broken `SET LOCAL $1` parameterized call AND incidentally fixes a second latent GUC-name bug (`app.tenant_id` → `app.current_tenant`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 202 | Self-host bundled mode: atlas service starts before migrations finish                             | `merged`                      | infra/202-self-host-bundled-bootstrap-race-fix          | gh#490 | 2026-05-22          | 2026-05-22 | batch 101 · SOLO · merged CLEAN at `bd8c797` · 3 files / +289/-2 · approach (c) CI-time harness staged bring-up (postgres + bootstrap first; poll `to_regclass(public.evidence_kind_schemas)` sentinel up to 4 min; then bring up atlas + web) · (a) `service_completed_successfully` rejected (documented deadlock) · (b) atlas healthcheck rejected (Go change → P0-A1 violation) · production compose unchanged · Self-host bundle (bundled) PASSED — load-bearing CI verification · D2 CI-delta scan clean (engineer regression from batch 99 D8 false-positive CORRECTED)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 084 | cosign v3 + goreleaser-action v7 coordinated migration                                            | `merged`                      | infra/084-cosign-v3-goreleaser-v7                       | gh#425 | 2026-05-20          | 2026-05-20 | batch 83 SOLO · Infra · AFK · 0.5d · merged at `f15839a` (GREEN first try; engineer ~18 min) · test-tag `v0.0.0-slice084-test` workflow `26200163638` 7m26s · cosign-installer v3 → v4.1.2; cosign v2.4.1 → v3.0.6; goreleaser-action v6 → v7.2.2 · signs args → `--bundle=${signature}` · consumer verify-blob docs at RELEASE_READINESS.md §11.1 migrated · `--new-bundle-format` disallowed in cosign v3.0.6 (sigstore/cosign#4762) · pre-slice-084 releases keep .pem+.sig · 5/5 ACs · 3/3 P0                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 133 | mkdocs user docs content refresh (slice 058 follow-on)                                            | `merged`                      | docs/133-mkdocs-content-refresh                         | gh#389 | 2026-05-19          | 2026-05-20 | batch 76 · spillover from 132 · Docs · AFK · 2-3d · merged at `b64cb06` (CLEAN) · 14 files / 2160+ / 178- · 10 new mkdocs pages (6 primitives + audit-log trio + CI hardening + connector authoring + section nav) · `mkdocs build --strict` green (0 warnings, 0 broken links, 0 unresolved refs) · ~60 min engineer wall-clock                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 134 | Refresh slice-070 onboarding walkthroughs against current main (HITL)                             | `not-ready`                   | —                                                       | gh#390 | —                   | —          | RE-SCOPED 2026-05-20 from "in-app walkthrough refresh" (engineer escalated cleanly — original slice doc described nonexistent browser-driven product tours; closest legit interpretation is slice-070 walkthroughs refresh) · Docs · HITL · 0.5-1d interactive · gate: maintainer schedules interactive `uvx showboat exec` operator session (slice 070 P0-A4 forbids hand-authored outputs) · re-scope PR #390 open                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 136 | Risk register data export (CSV / JSON / XLSX)                                                     | `merged`                      | backend/136-risk-register-export                        | gh#378 | 2026-05-19          | 2026-05-19 | batch 72 · parallel-pair w/ 139 · backend/frontend · AFK · 0.5d · merged at `c80bec3` (CLEAN after 3 orchestrator fix-ups: rebase against post-139 main + migration timestamp rename `20260519000010` + plural-spelling fix `vendor_export` → `vendors_export` + down-migration DELETE of `risk_export` rows for round-trip cleanup) · 14 files · 2335+ / 176- · 18-column canonical set · `internal/api/risks/export.go` + `cmd/atlas-mcp/`-pattern absent (separate slice) · meta-audit action `risk_export` · slice 145 concurrency cap inherited                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 137 | Controls UCF graph data export (CSV / JSON / XLSX)                                                | `merged`                      | backend/137-controls-ucf-graph-export                   | gh#384 | 2026-05-19          | 2026-05-19 | batch 74 · SOLO · backend/frontend · JUDGMENT · 1d · merged at `4300cde` (UNSTABLE — codecov/patch informational) · D1 = flat projection (rejected nested + two-sheet XLSX) · 500K row cap · ≤200 MB streaming-memory budget · 21 files / 2853+ / 190- · coverage gate cleared via 12 new unit tests (22.1% → 27.3%) · 2 spillovers filed (174 + 175) · ~90 min total wall-clock                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 138 | Ledger entities export (evidence + policies + exceptions + samples)                               | `merged`                      | backend/138-ledger-entities-export                      | gh#387 | 2026-05-19          | 2026-05-19 | batch 75 · SOLO · backend/frontend · AFK · 1-2d · merged at `a171a0a` (UNSTABLE — codecov/patch informational) · 4 endpoints (evidence/policies/exceptions/samples) + 4 BFFs + 3 UI Export buttons (exceptions UI deferred to slice 177) · D1 = evidence excludes payload_json (load-bearing per slice 145 hardening) · samples 250K cap · migration 20260520000010 with defensive DELETE of 4 new action values · spillover slice 177 filed (orchestrator-renumbered from initial 176 collision) · ~90 min engineer wall-clock + ~5 min orchestrator slot-collision fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 139 | Audit periods + vendors data export (CSV / JSON / XLSX)                                           | `merged`                      | backend/139-audit-periods-vendors-export                | gh#379 | 2026-05-19          | 2026-05-19 | batch 72 · parallel-pair w/ 136 · backend/frontend · AFK · 0.5-1d · merged at `3522c64` (CLEAN first try) · 20 files · 3176+ / 177- · 2 endpoints (audit-periods + vendors) · 14 maskEmail unit cases · vendor email masked to `*@domain.tld` at v1 · audit-period freeze-metadata columns included; cosigned bundle bytes excluded (slice 030's surface) · ~75 min engineer wall-clock                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 141 | Multi-tenant login + tenant picker + persistent header switcher                                   | `merged-via-spine-completion` | (closed by slice 192 backend/192-multi-tenant-switch)   | gh#458 | 2026-05-20 (PARKED) | 2026-05-21 | **CLOSED via auth-substrate-v2 spine completion (slice 192 merged at `b0b5280`)** — PARKED 2026-05-20 by OQ #21 Reading D resolution. The vCISO success criterion ("switch tenants mid-session without re-auth") is fulfilled by slice 192's frontend wiring of slice 188's token-exchange primitive. Original 141 spec preserved as historical context at `docs/issues/141-multi-tenant-login-and-switcher.md`; the implementation moved entirely to the auth-substrate-v2 spine (slices 187-192). This is a non-standard transition status ("merged-via-spine-completion" rather than plain "merged") because no PR named "slice 141" merged — instead, the work happened across the spine. Documented in slice 192's decisions log D4.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 142 | super_admin role: full schema + management surface (slice 141 follow-on)                          | `merged`                      | backend/142-super-admin-role-surface                    | gh#480 | 2026-05-21          | 2026-05-21 | batch 98 · SOLO · merged at `ea674f6` (UNSTABLE codecov-only) · 12/12 ACs · 8/8 Go integration · 2/2 Playwright · 4/4 OPA matrix · 689/689 vitest · 6 JUDGMENT decisions (D1 advisory lock; D2 dual-write audit log; D5 schema reconciled to slice 198) · 3 CI-deltas discovered + addressed proactively · 0 spillovers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 143 | Create-tenant flow (super_admin-gated)                                                            | `merged`                      | backend/143-create-tenant-flow                          | gh#485 | 2026-05-22          | 2026-05-22 | batch 99 · SOLO · gates 141 ✅ + 142 ✅ · merged at `7332695` UNSTABLE-codecov-only · 17 files / 3393+/24- · 10/10 ACs · 9 Go integration + 12 vitest + 3 Playwright + 8 JUDGMENT decisions (D1-D8) · D6 fixes latent slice-034-vs-slice-192 schema contradiction (relaxes users(idp_issuer, idp_subject) GLOBAL UNIQUE to per-tenant) · engineer-as-collaborator with one D8 false-positive (CI-delta scan claimed clean; orchestrator fix-forward removed unused defaultScopeCellEnv const at `ab61dbf`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 144 | Rename-tenant flow (per-tenant admin or super_admin)                                              | `merged`                      | backend/144-rename-tenant                               | gh#462 | 2026-05-21          | 2026-05-21 | batch 92 · parallel-pair w/ 199 · merged at `dd2e876` (UNSTABLE — codecov/patch informational only; orchestrator pushed go-mod-tidy fix at 1caf400 after initial CI caught it) · AFK · 0.5d · ~50 min engineer wall-clock · 8/9 ACs (AC-8 Playwright deferred) · 18 files / +1648 lines · **filled `tenants` table schema gap** discovered mid-build (slice 192 referenced `SELECT * FROM tenants` with no underlying migration) · LOWER(name) UNIQUE + four-policy RLS + me_audit_log CHECK extension · NFC normalization + byte cap + control-char rejection                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 145 | Data-export hardening (payload_json redaction + concurrency cap)                                  | `merged`                      | backend/145-data-export-hardening                       | gh#375 | 2026-05-19          | 2026-05-19 | batch 71 · SOLO · backend · AFK · 0.5d · merged at `637b8dc` (CLEAN after orchestrator lint-fix removing dead `jsonMarshalOrdered` shim) · D1 = include_payload default true (backwards-compat); D2 = concurrency cap default 2 · 12 files / 2128+ / 209- · `internal/export/concurrency.go` semaphore + `?include_payload` flag + 429 with `Retry-After: 30` + meta-audit `include_payload` key · 14 unit + 8 integration tests · slices 136-139 inherit both behaviors                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 155 | Questionnaire feature design + build (CAIQ/SIG/HECVAT response)                                   | `merged`                      | feature/155-questionnaire-tracer                        | gh#433 | 2026-05-20          | 2026-05-21 | batch 85 · parallel-pair w/ 187 · merged at `12da637` (UNSTABLE — codecov/patch informational; Self-host bundle unrelated infra flake) · **scope locked 2026-05-20 by maintainer to TRACER-BULLET.** Shipped: 4-table migration · questionnaires/excel/library/pdf/store/api packages · 7 routes · 14 tests · 68/68 PRD ISCs · D1 excelize/v2 5MB cap · D2 AnswerLibrary updated_at DESC · D3 header-row heuristic · D4 chromedp PDF reuse · D5 unmappable rows `needs_mapping`. Orchestrator post-push fix: dropped scf_anchors FK (engineer keyed TEXT scf_id against UUID id target; tracer relaxes to free-form TEXT — v2 spillover documented in-migration) + regen openapi spec (7 RouteSpecs) + `go mod tidy`. CSV/JSON/Word import + HECVAT + vendor portal + AI-assist DEFERRED (7 spillover candidates in decisions log). · filed 2026-05-18 · backend/frontend · JUDGMENT · canvas §4.6 commits to v1 ship                                                                                                                                                                                                                                                                   |
...
| 172 | MCP server foundation + read-only tools                                                           | `merged`                      | backend/172-mcp-server-foundation                       | gh#382 | 2026-05-19          | 2026-05-19 | batch 73 · SOLO · backend/infra · JUDGMENT · 3-4d · merged at `95dd94d` (CLEAN first try) · 22 files / 3939+ / 175- · D1 = stdio · D2 = HAND-ROLLED JSON-RPC stdlib (rejected mark3labs/mcp-go + modelcontextprotocol/go-sdk; ~340 LoC pure stdlib) · D3 = six tools exactly · D4 = User-Agent header for read-audit-trail · all 13 P0 anti-criteria honored · zero CodeQL alerts · 30-day soak before required-checks · slice 173 (writes) now unblocked · ~95 min engineer wall-clock                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 173 | MCP server write tools (create / update operations)                                               | `merged`                      | backend/173-mcp-server-write-tools                      | gh#437 | 2026-05-21          | 2026-05-21 | batch 86 · SOLO JUDGMENT · backend/infra · 3-5d (engineer ~75min wall-clock) · merged at `34508fa` (CLEAN first-try post-rebase) · 16/16 ACs PASS · 8/8 P0 anti-criteria honored · 76/76 PRD ISC criteria PASS · 20 files / 3480+ / 27- · D1 Pattern A draft-then-confirm (pre-locked) · D2 single mcp*write_proposals table · D3 CHECK constraint for AI-assist invariant `(ai_assisted=true AND human_approved=true) → human_approver IS NOT NULL` · D4 applier runs INSIDE the confirm transaction (all-or-nothing) · D5 TEXT created_by/human_approver (`key*...` + IdP IDs) · D6 pending-cap default 5 per (tenant, operator) · D7 proposal table IS the audit trail (defer me_audit_log extension to v2) · 5 MCP write tools: create_risk · update_control_state · push_evidence · update_risk_treatment · confirm_write (approver-only) · reject_write · final STRIDE has-mitigations (HOLD-pending-review CLEARED) · engineer SHIPPED CLEANLY (no stall — ship-now sentinel worked) · no spillover slices filed                                                                                                                                                                 |
...
| 174 | UCF anchor catalog export (all three formats)                                                     | `merged`                      | backend/174-ucf-anchor-catalog-export                   | gh#410 | 2026-05-20          | 2026-05-20 | batch 79 · parallel pair · Backend/Frontend · AFK · 1-2d budget (engineer ~28 min wall-clock) · merged at `79eb829` (CLEAN after CHANGELOG rebase against 181-merged main) · 20 files · D1 pre-locked: 3 formats ship (json nested · xlsx two-sheet · csv flat-nested) · slice 135 export library reused · new meta-audit action `anchors_export` via migration `20260520010000_anchors_export_meta_audit.sql` · 50K-anchor streaming-memory budget asserted < 200 MB across all 3 encoders · XLSX writer emits exactly 6 zip members (P0-A-174-2 satisfied by construction) · cross-tenant byte-identical (catalog is public; D7) · OPA admit-set parity with `/v1/anchors` read endpoint · BFF + Export CSV/JSON/XLSX buttons on `/catalog/scf`                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 175 | Control bundle history export (lineage including superseded versions)                             | `merged`                      | backend/175-control-bundle-history-export               | gh#495 | 2026-05-22          | 2026-05-22 | batch 102 · PARALLEL with 116 · merged UNSTABLE-codecov-only at `fac1450` · 18 files / +2725/-1 · 7/7 ACs · 8 JUDGMENT decisions (D1 dropdown link group · D2 duplicate sqlc query · D3 synthesised superseded_at from updated_at · D4 50K-row streaming-memory test · D5 cross-tenant FK-chain · D6 plural action name · D7 chi literal route · D8 explicit CI-delta scan) · NEW `GET /v1/controls/history/export` 17-column endpoint · meta_audit_log.action CHECK extended with `controls_history_export`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 203 | Dark-mode stylesheet wiring (theme selection actually themes the UI)                              | `merged`                      | frontend/203-dark-mode-stylesheet-wiring                | gh#499 | 2026-05-22          | 2026-05-22 | batch 103 · PARALLEL with 205 · merged CLEAN at `d7a0b10` · 8 files / +897/-13 · D1 inline early-paint script in `<head>` (no `next-themes` dep) · `suppressHydrationWarning` on `<html>` for deliberate class mismatch · NEW `ThemeClassSync` component for runtime reactivity · 17 vitest tests + Playwright AC-12 · D2 flagged board-pack + exceptions + control coverage as future spillover candidates                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 205 | Comprehensive demo seed dataset (showcase tenant + evidence across all primitives)                | `merged`                      | backend/205-demo-seed-dataset                           | gh#500 | 2026-05-22          | 2026-05-22 | batch 103 · PARALLEL with 203 · merged UNSTABLE-codecov-only at `072f9a7` · 14 files / +3753 · 23/23 ACs · D1-D9 decisions logged · live smoke 50 controls / 20 risks / 200 evidence / 12 distinct evidence_kinds / 1 frozen audit period · shipped `atlas-cli demo seed` + `atlas-cli demo teardown` (D6) · D7 CI-delta scan FALSE-POSITIVE (claimed clean; 7 lint findings); orchestrator fix-forwards `eec3333` + `5d7c9e7` corrected; GitGuardian false-positive resolved on 3rd push                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 204 | Comprehensive page-by-page UI parity audit (per-page agent fleet)                                 | `ready`                       | —                                                       | —      | —                   | —          | filed 2026-05-22 via `/idea-to-slice` from maintainer-requested audit · Quality · AFK · 2d · 15 ACs · operationally blocked on resolving v1.14.0 500-error class (separate diagnostic slice the maintainer files) · dispatches per-page audit Agent fleet (11 mockup pages × 4-finding-axis comparison) · each finding → individual spillover slice · aggregate at `docs/audit-log/204-aggregate.md` · slice ships only docs + spillover slices (no production code change)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 206 | BFF auth cookie migration from `sa_session_token` to `atlas_jwt` (slice 197 follow-on)            | `merged`                      | frontend/206-bff-cookie-migration                       | gh#503 | 2026-05-22          | 2026-05-22 | batch 104 · SOLO · merged CLEAN at `be6a5cc` (deployment-blocking fix) · 23 files / +459/-141 · D1 Authorization Bearer (existing bff.ts already builds it; zero call-site refactor) · D2 exemptions `/v1/*` startsWith + `/metrics` exact · D3 honest CI-delta scan (74 files / 738 tests pass; pre-commit + lint + build all clean) — pattern correction after slice 143/205 false-positives held this time                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 207 | Edge deploy channel: per-commit images + Watchtower-driven `atlas-edge` instance                  | `merged`                      | infra/207-edge-deploy-channel                           | gh#506 | 2026-05-22          | 2026-05-22 | batch 105 · SOLO · merged CLEAN at `bf0aa19` · 9 files / +1702/-3 · 14/14 ACs · 7 P0s · D1-D7 logged · D1 single workflow (extends container-publish.yml) · D2 hybrid retention with safety invariant (jq filter only deletes versions whose tag list is composed ENTIRELY of `main-<sha7>`; preserves :edge + release tags) · D3 full compose isolation · D4 5-min Watchtower interval · D5 ldflag parity verified (no parallel goreleaser needed) · D6 doc-only access control (Tailscale recommended) · D7 honest scan (132 SHA-pinned uses) · benign pre-existing `:latest` bug surfaced + documented · release-please cut v1.15.0 at `7f2d140` packaging slice 206's BFF cookie fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 208 | Next.js rewrites for /v1/, /health, /metrics (deployment-portable backend routing)                | `merged`                      | frontend/208-nextjs-rewrites                            | gh#510 | 2026-05-22          | 2026-05-22 | batch 106 · SOLO · frontend/deploy · AFK · 0.25d · merged CLEAN at `3967ff9` · 5 files / +231/-3 · 8/8 ACs PASS · 6 P0s honored · D1 local-dev fallback to `http://localhost:8080` with `console.warn` when `ATLAS_HTTP_URL` unset · D2 scope discipline (3 exact prefixes; zero `/api/*` or `/oauth/*` coverage) · D3 CI-delta scan exposed two adjacent latent gaps via Playwright: `web/proxy.ts` missed `/health` in slice-206 exemption + `internal/auth/jwtmw/middleware.go` `DefaultCookieName` still `atlas_session` (slice-189 legacy) while slice-198 OIDC callback writes JWT to `atlas_jwt`. Orchestrator fix-forward `d714eb5` corrected both — the engineer's Playwright regression specs were the canary catching auth-substrate-v2 cookie-name drift before merge · unblocks atlas-edge.home.gmoney.sh dashboard end-to-end                                                                                                                                                                                                                                                                                                                                             |
| 176 | Logo follows app theme + README/docs refresh                                                      | `merged`                      | frontend/176-logo-theme-coupling                        | gh#394 | 2026-05-20          | 2026-05-20 | batch 77 · parallel pair · Frontend + Docs · AFK · 0.5d · merged at `6eb0701` (CLEAN after orchestrator rebase against 177-merged main) · 11 files / 518+ / 61- · D1 = hybrid A+B: `<ThemeAwareLogo>` reads `<html data-theme>` via MutationObserver + matchMedia fallback for system theme (slice 170 writes `data-theme`, not Tailwind `dark:` class) · docs/images/logo-\*.png regenerated from slice-167 SVGs (light 4638B / dark 4746B gzipped; under 16 KB cap) · all 10 P0 anti-criteria honored · ~45 min cumulative (35 impl + 10 rebase)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 177 | Exceptions list-page UI surface (with slice 138 Export buttons)                                   | `merged`                      | frontend/177-exceptions-list-page                       | gh#393 | 2026-05-20          | 2026-05-20 | batch 77 · parallel pair · Frontend · AFK · 0.5d · merged at `ee0163a` (CLEAN first-try) · 12 files / 1781+ / 179- · 588/588 vitest pass (29 new tests) · List endpoint pre-existed (slice 021); UI + BFF proxy at `/api/exceptions` + Export buttons consume slice-138's exceptions backend cleanly · 8-column table derived from exceptionWire (id, control_id, justification, status, requested_by, approved_by, expires_at, created_at) · 2 filter pills (status + control_id) · ~55 min engineer wall-clock                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 178 | UI honesty audit harness + first-pass audit against v1 mockup spec                                | `merged`                      | quality/178-ui-honesty-audit-harness                    | gh#414 | 2026-05-20          | 2026-05-20 | batch 80 · parallel pair · Quality/Testing · AFK · 1.5d budget (engineer ~24 min impl + ~5 min orchestrator CodeQL fix + CHANGELOG rebase) · merged at `69ce7df` · 23 files / +3222 / -1 · reusable Playwright harness at `web/e2e-audit/` (separate Playwright project from `web/e2e/`); 31 vitest unit tests; read-only enforcement via `makeReadOnly(page)` decorator · new CI job `Frontend · UI honesty (advisory)` informational only (NOT in required-checks per P0-178-6) · first-pass audit produced 8 substantive findings (6 HONESTY-GAP + 2 MOCKUP-STALE) filed as spillover slices #183 / #184 / #185 / #186 · orchestrator fix: CodeQL `js/incomplete-url-scheme-check` on `web/e2e-audit/lib/heuristics.ts` resolved by replacing enumerated-scheme `startsWith` chain with RFC 3986 §3.1 case-insensitive regex                                                                                                                                                                                                                                                                                                                                                         |
| 179 | `schema-removal-age` CI check (enforce 90-day deprecation window)                                 | `merged`                      | infra/179-schema-removal-age-ci-check                   | gh#406 | 2026-05-20          | 2026-05-20 | batch 78 · parallel pair · Infra/Quality · AFK · 0.5d · merged at `f06c323` (CLEAN after CHANGELOG.md rebase against 182-merged main) · 8 files · script at `scripts/check-schema-removal-age.sh` + companion `_test.sh` (27 assertions / 8 fixture cases) · new CI job `Schema · removal-age (90-day floor)` informational only (NOT in required-checks per P0-179-4) · `[deprecation-override]` PR label as escape-hatch · live AC-11 smoke test (the PR's own filter match) fired the new job and exited count=0 cleanly · OQ #9+#17 implementation half                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 180 | Privacy-module foundation (audit-log `subject_module` + sibling discipline docs)                  | `merged`                      | infra/180-privacy-module-foundation                     | gh#413 | 2026-05-20          | 2026-05-20 | batch 80 · parallel pair · Infra/Multi-tenancy · AFK · 1d budget (engineer ~21 min wall-clock) · merged at `125205b` (CLEAN; codecov/patch informational warning ignored per branch-protection policy) · 9 audit-log tables modified · migration `20260520020000_audit_log_subject_module.sql` (idempotent + reversible) · sqlc regen across 8 sqlc-managed audit-log INSERT call sites + 1 hand-written (`internal/authz/audit.go`) · slice 124 coordination: **124-before-180 path taken** (slice 124 was already merged at gh#267 / `2406725`) — 180 updates 124's UNION query + `UnifiedEntry` wire shape to project `subject_module` through · CONTRIBUTING.md "Module isolation discipline" section codifies OQ #7 B1-B4 sub-decisions · canvas §8.2 note added: privacy exports use W3C DPV JSON-LD, NOT OSCAL · all 13 ACs PASS; all 9 P0 honored                                                                                                                                                                                                                                                                                                                               |
| 181 | Open-governance pre-commitments (GOVERNANCE.md + funding signals + bus-factor plan)               | `merged`                      | docs/181-open-governance-pre-commitments                | gh#409 | 2026-05-20          | 2026-05-20 | batch 79 · parallel pair · Docs/Governance · AFK · 0.5d · merged at `67092db` (CLEAN first-try; ~15 min engineer wall-clock) · 6 files · GOVERNANCE.md (316 lines / ~1980 words; 7 sections) + .github/FUNDING.yml + README "Project status" section + CONTRIBUTING.md cross-link + first quarterly checkin baseline (`docs/audit-log/governance-checkin-2026-Q2.md`) · re-evaluation locked: 2028-05-20 OR 100 self-hosts (whichever first) · no telemetry; no CLA; DCO-only · all 15 ACs satisfied; all 9 P0 honored                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 182 | Board-narrative AI-assist foundation (tone reference + ADR-0006 + canvas update)                  | `merged`                      | docs/182-board-narrative-ai-assist-foundation           | gh#405 | 2026-05-20          | 2026-05-20 | batch 78 · parallel pair · Docs/AI-assist governance · AFK · 0.5d · merged at `6622e8e` (CLEAN first-try) · 5 files / 490+ / 0- · 42 tone-anti-pattern entries (20 banned phrases + 12 banned framings + 10 carve-outs + Section 4 living-document discipline; well above 15-entry AC-2 floor) · ADR slot 0006 (incremented from spec's 0002 per "ADR slot computation" guidance — slots 0002-0005 were occupied) · 4 model references kept in sync per P0-182-6 · OQ #14 foundation work                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 183 | UI honesty: calendar `linkFor` dead-link family + dashboard mockup refresh                        | `merged`                      | frontend/183-calendar-deadlinks-and-dashboard-mockup    | gh#422 | 2026-05-20          | 2026-05-20 | batch 82 · parallel pair · Frontend · AFK · 0.5-1d (engineer ~14 min) · merged at `f40fa6f` (CLEAN after rebase against 186-merged main) · 9 files; 642 vitest pass (7 new) · `linkFor` tagged-union return + `assertNever` (compile-time exhaustiveness) — F-178-1/2/3 closed structurally · exception/policy events render `<span title>` (no dead links) · dashboard.html mockup refreshed · F-178-1/2/3/7/8 closed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 184 | UI honesty: `/audits` row-click 404                                                               | `merged`                      | frontend/184-audits-row-click-404                       | gh#417 | 2026-05-20          | 2026-05-20 | batch 81 · parallel pair · Frontend · AFK · 0.25d (engineer ~6 min) · merged at `e058f25` (CLEAN first-try) · Option A: disable click + honesty banner; Option B deferred to future slice · 4 files · F-178-4 closed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 185 | UI honesty: `/risks` row-click → hierarchy not detail                                             | `merged`                      | frontend/185-risks-row-click-detail                     | gh#418 | 2026-05-20          | 2026-05-20 | batch 81 · parallel pair · Frontend · AFK · 0.25d (engineer ~6 min) · merged at `c509ca8` (CLEAN after rebase against 184-merged main; no CHANGELOG conflict) · Option A: disable click + actions column "View in hierarchy →" preserving `?focus=<id>` + honesty banner · 3 files · F-178-5 closed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 186 | UI honesty: sidebar Admin entry shown to non-admin                                                | `merged`                      | frontend/186-sidebar-admin-role-gate                    | gh#421 | 2026-05-20          | 2026-05-20 | batch 82 · parallel pair · Frontend · AFK · 0.25d (engineer ~8 min) · merged at `471f779` (CLEAN first-try) · 5 files / 18-case vitest matrix pass · `shouldShowAdminEntry` predicate at `web/lib/admin-nav.ts` follows slice-130 `canReachAuditLog` pattern · Sidebar refactored to async server component fetching `/api/admin/me`; fail-closed on fetch error · server-side authz at `/admin/layout.tsx` untouched (P0-186-1) · F-178-6 closed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 187 | OAuth AS scaffolding — JWT signing + JWKS + OIDC discovery                                        | `merged`                      | backend/187-oauth-as-scaffolding                        | gh#432 | 2026-05-20          | 2026-05-21 | batch 85 · parallel-pair w/ 155 · merged at `ac42517` (UNSTABLE — Self-host bundle unrelated `evidence_kind_schemas` race + atlas_app password-auth race + metrics_catalog NOT NULL; all pre-existing failures unrelated to OAuth AS scope) · auth-substrate-v2 spine foundation (OQ #21 Reading D resolution) · 20/20 ACs · 11/11 P0 anti-criteria honored · ships keystore + tokensign + jwt + oauth packages + ADR-0003 · ES256 + go-jose/v4 + filesystem keystore @ mode 0600 + multi-key JWKS · 4 untouched 501-stub `/oauth/*` routes · D1 ES256 · D2 go-jose/v4 · D3 ATLAS_KEYSTORE_PATH override · D4 90d rotation cadence + 24h overlap (design only) · D5 direct `root.Get` chi routing · spine continues: 188 (`/oauth/token` + token-exchange) · 189 (PKCE + frontend OAuth) · 190 (JWT validation middleware + R2 + revocation) · 191 (SDK migration ×4 + CLI device-code) · 192 (multi-tenant switch via token-exchange; closes slice 141 success criterion)                                                                                                                                                                                                              |
| 188 | OAuth `/oauth/token` endpoint + RFC 8693 token exchange                                           | `merged`                      | backend/188-oauth-token-endpoint                        | gh#442 | 2026-05-21          | 2026-05-21 | batch 87 · parallel-pair w/ 111 · merged at `c0e7531` (UNSTABLE — Frontend·Playwright e2e fails from slice 111's un-skip surface, addressed by spillover 193; all 11 required-checks PASS) · filed 2026-05-21 · auth-substrate-v2 spine slot 2 · Backend/Auth · JUDGMENT · 2-3d budget (engineer ~70 min wall-clock) · 25/25 ACs · 11/11 P0 anti-criteria honored · 18 files / +2589 lines · D1 argon2id reuses `password.Hash` (m=64MiB/t=1/p=4 RFC 9106 first-rec) · D2 custom `atlas:target_tenant_id` form param (capability swap vs delegation) · D3 audit log best-effort post-sign (safety enforced before write) · D4 60/min/client_id rate limit · threat-model has-mitigations (5 STRIDE categories covered) · post-rebase GitGuardian-webhook-lag required empty-commit re-trigger (process learning documented in drift block) · ships `oauth_clients` + `oauth_token_exchanges` migrations + `atlas oauth issue-client` CLI · unblocks slice 189                                                                                                                                                                                                                           |
| 189 | OAuth `/oauth/authorize` + PKCE + frontend OAuth client integration                               | `merged`                      | backend/189-oauth-authorize-pkce                        | gh#447 | 2026-05-21          | 2026-05-21 | batch 88 · parallel-pair w/ 193 · merged at `140b0d1` (UNSTABLE — codecov/patch informational only) · 26/26 ACs · 10/10 P0 anti-criteria honored · ~70 min initial + ~30 min CodeQL fix iteration · D1 NEW `atlas_jwt` cookie · D2 reuses oauth_token_exchanges audit · D3 PKCE S256 mandatory at 3 layers · D4 CLI-only redirect-URI registration · **D5 CodeQL fixes (post-CI)**: jsonForScriptTag helper for safe JSON-in-script · textContent replaces innerHTML · Store.LookupRedirectURI returns DB-sourced URI (taint-safe redirect) · both CodeQL review threads resolved via graphql · threat-model has-mitigations (6 STRIDE) · auth-substrate-v2 spine slot 3 closed · **unblocks slice 190** (JWT validation middleware + R2 + revoke + introspect)                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 193 | Diagnose + fix dashboard.spec.ts AC-5 upcoming-row Playwright failure                             | `merged`                      | quality/193-dashboard-upcoming-fixture                  | gh#446 | 2026-05-21          | 2026-05-21 | batch 88 · parallel-pair w/ 189 · merged at `de1b20a` (CLEAN) · spillover from slice 111 · Quality · JUDGMENT · 0.5d · **verdict H1 (fixture gap)** · fix: status `'approved'` → `'active'` + `ON CONFLICT (id) DO UPDATE` (slice 168 precedent) + activated_by/at for SoD · local DB-contract test PASSES · GitGuardian webhook lag handled via empty-commit nudge pattern · spillover slice 194 filed for CI path-filter gap (fixtures-only PRs short-circuit Playwright)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 190 | JWT validation middleware on `/v1/*` + R2 eviction + `/oauth/revoke` + `/oauth/introspect`        | `merged`                      | backend/190-jwt-middleware                              | gh#451 | 2026-05-21          | 2026-05-21 | batch 89 · parallel-pair w/ 194 · merged at `3df5df1` (UNSTABLE — codecov/patch informational only) · **CUTOVER SLICE** · 31/31 ACs · 11/11 P0 anti-criteria honored · ~95 min engineer wall-clock · 11 files / +2801 lines · auth-substrate-v2 spine slot 4 · JWT middleware coexists with slice 034 legacy bearer path (slice 191 retires) · ships RFC 7009 revoke + RFC 7662 introspect + oauth_revoked_tokens + sweeper · D1 header > cookie precedence · D2 separate oauth_revocation_events audit · D3 JWT-first / fail-closed coexistence · D4 5-min sweeper interval · threat-model has-mitigations · **AC-25 R2 eviction load-bearing test GREEN** (TestR2Eviction_EventualThenRevoke + TestR2Eviction_CrossTenantHeaderOverride) · **unblocks slice 191** (SDK migration + CLI device-code + legacy retirement)                                                                                                                                                                                                                                                                                                                                                               |
| 191 | SDK migration to `client_credentials` × 3 languages + CLI device-code + partial legacy retirement | `merged`                      | backend/191-sdk-migration-legacy-retirement             | gh#454 | 2026-05-21          | 2026-05-21 | batch 90 SOLO · merged at `8f0d265` (UNSTABLE — codecov/patch informational) · 35 files / +4519 / -60 · ~95 min engineer wall-clock · auth-substrate-v2 spine slot 5 closed · **34/36 ACs** ✓ + AC-27 PARTIAL (production-cutover via 410 responder; test-fixture cutover staged to slice 197) + AC-28 modified (responder narrowed to `atlas_` PROD prefix only; `atlas_test_` carved out for CI fixtures) · 11/11 P0 anti-criteria honored (P0-191-11 spirit honored — production legacy bearers → 410 + migration_url; only test-fixture path keeps fallthrough until slice 197 migrates ~60 integration tests from credstore.Issue() to JWT-bearer fixtures) · D1 Java spillover (#195) · D2 name-only API-key migration · D3 credstore v3 deferral · D4 410 + RFC 9745 + migration_url body · D5 5s device-code interval · threat-model has-mitigations · ships RFC 8628 device-code endpoint + Go/Python/TS OAuth clients + `atlas login` CLI + `atlas oauth migrate-api-key` · spillovers filed: **195** (Java SDK OAuth) · **196** (bootstrap OAuth migration) · **197** (complete slice-034 bearer retirement after test-fixture migration) · **unblocks slice 192**           |
| 192 | Multi-tenant switch via OAuth token-exchange + frontend tenant switcher (closes slice 141)        | `merged`                      | backend/192-multi-tenant-switch                         | gh#458 | 2026-05-21          | 2026-05-21 | batch 91 · parallel-pair w/ 195 · merged at `b0b5280` (UNSTABLE — codecov/patch informational only) · **🎉 FINAL auth-substrate-v2 spine slot — spine 6/6 COMPLETE** · 23/23 ACs · 11/11 P0 anti-criteria honored · ~95 min engineer wall-clock + ship-stall resumed via SendMessage · 21 files / +2519 lines · D1 /v1/me/tenants 60s cache · D2 inline membership-removed banner · D3 prompt=tenant only-when-≥2 (avoids slice 188/189 changes) · D4 merged-via-spine-completion status for slice 141 · threat-model has-mitigations (inherited from 188 + 190) · spillovers filed: 198 (OIDC-first-install bootstrap) · 199 (cross-tab BroadcastChannel sync) · downstream re-evaluation: slice 142 stays not-ready (re-scope), slice 143 gated on 142, slice 144 newly-ready · **CLOSES slice 141 vCISO criterion** (merged-via-spine-completion)                                                                                                                                                                                                                                                                                                                                    |
| 194 | CI path filter should include `fixtures/**` so fixture-only PRs trigger Playwright e2e            | `merged`                      | infra/194-ci-fixtures-pathfilter                        | gh#450 | 2026-05-21          | 2026-05-21 | batch 89 · parallel-pair w/ 190 · merged at `4159fb8` (CLEAN) · spillover from slice 193 · Infra/CI · AFK · 0.25d · ~2-line patch added `fixtures/**` to Playwright path-filter (also covers `walkthroughs/`/`readme-demo/` fixture subdirs by design)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 195 | Java SDK OAuth client_credentials helper                                                          | `merged`                      | sdk/195-java-oauth-client                               | gh#457 | 2026-05-21          | 2026-05-21 | batch 91 · parallel-pair w/ 192 · merged at `b89ee7a` (CLEAN) · spillover from slice 191 · SDK · AFK · ~30 min engineer wall-clock · 10 files / +963 / -7 · 7/7 ACs · 4/4 P0s · D1 Maven · D2 java.net.http.HttpClient · D3 hand-rolled JSON reader · D4 Java 17 LTS                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 196 | Bootstrap OAuth migration (atlas-bootstrap container → client_credentials)                        | `merged`                      | infra/196-bootstrap-oauth-migration                     | gh#465 | 2026-05-21          | 2026-05-21 | batch 93 · SOLO · spillover from slice 191 · merged at `3b8f0f1` (UNSTABLE — only `Self-host bundle · end-to-end (external)` failed; not in branch-protection required contexts; failure is a pre-existing latent issue surfaced by slice 196's path-filter touches, NOT a regression — slice 196 didn't modify postgres service or the external-mode pre-create logic; filed slice 200 as spillover for the external-mode fix) · 6/7 ACs + 1 partial · AC-6 partial: bundled mode green ✅, external mode deferred to slice 200 · Infra/Bootstrap · AFK · ~30 min engineer wall-clock · 16 files / +1138/-22 · 6 JUDGMENT decisions logged (machine-actor authz widening, OPA carve-out symmetric with evidence-push, atlas service OAuth issuer/keystore/data-dir wiring, token rate limit 60→600/min) · 3/3 P0s honored · slice 197 unblocked (now test-fixture migration is the only remaining slice-034 dependency)                                                                                                                                                                                                                                                                |
| 197 | Complete slice 034 bearer-middleware retirement (test-fixture migration + final removal)          | `merged`                      | backend/197-complete-bearer-retirement                  | gh#471 | 2026-05-21          | 2026-05-21 | batch 95 · SOLO · merged at `00a682c` (UNSTABLE — only `Frontend · Playwright e2e` failed; NOT in branch-protection required contexts per slice 127) · 31 Go integration test fixtures migrated to JWT-via-tokensign · NEW `internal/api/testjwt/` package + `Server.IssueTestJWT(t, claims)` helper · `httpAuthMiddlewareWithExemptions` mount removed + `atlas_test_` carve-out removed · Backend/Auth/Testing · AFK · ~50 min engineer wall-clock · 5/7 ACs PASS local + Go integration green (engineer's pre-existing-fails claim verified in CI) · 3/3 P0s honored · 6 JUDGMENT decisions logged · Self-host bundle bundled+external both green (slice 200 fix durable) · **Playwright regression discovered post-merge**: `web/e2e/seed.ts:53` defines `TEST_BEARER = "test-bearer-e2e"` (slice 034 bearer); spec was Go-focused, did NOT enumerate Playwright migration — spec scope gap, NOT engineer claim-inflation · slice 201 filed for Playwright JWT migration · engineer self-flagged honest disclosure on capability-invocation (selected 4 skills, only invoked `tdd` via Skill tool; others inline) — proves the slice-by-slice anti-pattern feedback loop is working |
| 198 | OIDC-first-install bootstrap (slice 192 follow-on)                                                | `merged`                      | backend/198-oidc-first-install-bootstrap                | gh#477 | 2026-05-21          | 2026-05-21 | batch 97 · SOLO · merged at `12a6219` (UNSTABLE on codecov/patch informational only) · 10/10 ACs · 5/5 P0s · NEW `super_admins` no-RLS table (D2: dedicated table over per-tenant column) · `BootstrapFirstInstallOrUpsert` 5-row atomic write · SQLSTATE 23505 retry via slice 144 partial UNIQUE (D1) · N=5 race verified · 5/5 integration tests PASS locally · closes slice 192's AC-11/AC-12 (batch 90 spillover thread closed) · self-host bundle + Playwright + UI honesty all green (slices 200 + 201 fixes durable across 3 batches now)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 199 | Cross-tab BroadcastChannel sync for tenant-switcher                                               | `merged`                      | frontend/199-tenant-broadcast-channel                   | gh#461 | 2026-05-21          | 2026-05-21 | batch 92 · parallel-pair w/ 144 · merged at `cf82f0d` (CLEAN) · spillover from slice 192 · Frontend · AFK · 0.5d · ~12 min engineer wall-clock · 10/10 ACs · 5/5 P0s · enhance `<TenantSwitcher>` so switching tenants in one tab updates others in real time via `BroadcastChannel` (with no-op fallback when API unavailable)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 200 | Fix self-host bundle external-mode postgres init regression                                       | `merged`                      | quality/200-self-host-external-mode-fix                 | gh#468 | 2026-05-21          | 2026-05-21 | batch 94 · SOLO · spillover from slice 196 · merged at `293a661` (CLEAN) · Quality/Infra · JUDGMENT · 0.5d · 6/6 ACs · 3/3 P0s · ~15 min engineer wall-clock · 1-line surgical fix to `deploy/docker/test-self-host-bundle.sh` · diagnosis was NEITHER H1 nor H2 from slice doc — actual root cause: race condition between `pg_isready -U postgres` (returns ready before docker_setup_db's CREATE DATABASE runs) and the next `psql -d security_atlas` call · fix: tighten readiness probe to `pg_isready -U postgres -d security_atlas` so it waits for the precondition the next step actually needs · engineer this time DID verify locally with full shell-output evidence in decisions log (contrast with slice 196's claim-inflation)                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 201 | Migrate Playwright e2e fixtures from slice 034 bearer to JWT-via-tokensign                        | `merged`                      | quality/201-playwright-jwt-migration                    | gh#474 | 2026-05-21          | 2026-05-21 | batch 96 · SOLO · spillover from slice 197 · merged at `d4bb38f` (CLEAN after 2-iteration fix) · 8/8 ACs · 4/4 P0s · NEW `internal/api/testissuejwt.go` + `web/e2e/global-setup.ts` · env-gated `POST /v1/test/issue-jwt` reuses slice 187 OAuth keystore · 159 Playwright specs PASS locally + CI · **2-iteration ship**: first push failed CI on (a) openapi-drift (endpoint without RouteSpec entry) + (b) Playwright (CI atlas EACCES /var/lib/security-atlas/keys — fix: ATLAS_KEYSTORE_PATH=/tmp/atlas-ci/keys) · iteration 2 (`3744260`) resolved both · Closes slice 197's Playwright regression                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
