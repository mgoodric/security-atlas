...
**Last reconciled:** 2026-05-22 (batch 106 claim-stake · 208 SOLO → in-progress + spec filed)

## Drift detected — 2026-05-22 (batch 106 claim-stake · 208 SOLO → in-progress + spec filed)

Continuous-loop batch 106 SOLO. Maintainer-authorized urgent pickup of slice 208 — Next.js rewrites for `/v1/*`, `/health`, `/metrics` so the dashboard's data layer actually works end-to-end across all deployment topologies.

Surfaced during atlas-edge.home.gmoney.sh provisioning (post-batch-105): live-debugged that **production atlas.home.gmoney.sh has never had functional `/v1/*` routing** — NPM forwards everything to web (Next.js) and Next.js has no route at `/v1/*`. Slice 206 fixed the redirect-loop symptom; slice 208 fixes the underlying routing gap. Maintainer chose Next.js-rewrites over per-deployment reverse-proxy path-routing because it ships portable across docker-compose / K8s / Cloudflare Tunnel / Caddy / etc. with zero operator config.

- **Slice 208** (Next.js rewrites for /v1/, /health, /metrics) — frontend/deploy · AFK · 0.25d · 8 ACs · 6 P0s. Adds `async rewrites()` to `web/next.config.ts`; rewrites three path prefixes to `${ATLAS_HTTP_URL}/...`; `ATLAS_HTTP_URL` env already wired in compose templates. Local-dev fallback to `http://localhost:8080`. Playwright regression verifies browser `/v1/me` reaches atlas (200 with auth) and unauth `/v1/anchors` returns 401 JSON (not 404).

| Row | Transition            | Evidence                                                                                    |
| --- | --------------------- | ------------------------------------------------------------------------------------------- |
| 208 | (new) → `in-progress` | spec file + canonical row added; worktree `frontend/208-nextjs-rewrites` cut from `7de0147` |
...
| 208 | Next.js rewrites for /v1/, /health, /metrics (deployment-portable backend routing)                | `in-progress`                 | frontend/208-nextjs-rewrites                            | —      | —                   | —          | batch 106 · SOLO · frontend/deploy · AFK · 0.25d · 8 ACs · 6 P0s · maintainer-surfaced 2026-05-22 during atlas-edge provisioning: live-debugged that production atlas.home.gmoney.sh has never had functional /v1/_ routing — NPM forwards everything to web, Next.js has no route at /v1/_. Slice 206 fixed the redirect-loop symptom; slice 208 fixes the underlying routing gap. Maintainer chose Next.js-rewrites over per-deployment reverse-proxy path-routing for portability across all deployment topologies                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
