...
**Last reconciled:** 2026-05-16 (slice 105 PR opened · `in-progress` → `in-review`)

## Drift detected — 2026-05-16 (slice 105 PR opened · `in-progress` → `in-review`)
...
| 082 | Playwright e2e seed-data harness (un-quarantine 079)                | `not-ready`   | —                                                       | —      | —          | —          | v2 backlog · AFK · ~2-3d · dep slice 079 merged BUT slice file frontmatter is `not-ready` with explicit "**Not-ready until staffed**" note — maintainer's staffing decision is the gate, not just the upstream dep · the batch-31 reconcile incorrectly flipped to `ready`; corrected in batch-33 claim-stake                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 099 | /evidence list view (per slice 093 mockup)                          | `in-review`   | frontend/099-evidence-list-view                         | gh#232 | 2026-05-16 | —          | batch 37 · AFK · 1-2d · 9/9 ACs PASS · 12 files / 1,467 insertions · reuses 098's shell · 8-char hash prefix with copy-on-click · 26 vitest cases + quarantined Playwright · spillover slice 106 filed for backend `GET /v1/evidence` extension (make `control_id` optional + `?kind=`/`?result=` filters) · PR gh#232                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 101 | /policies list view (per slice 093 mockup)                          | `in-review`   | frontend/101-policies-list-view                         | gh#233 | 2026-05-16 | —          | batch 37 · AFK · 1-2d · 10/10 ACs PASS · 13 files / 1,443 insertions · reuses 098's shell · new shadcn `<Progress>` primitive (Radix-free) · ack-rate renders `—` honestly (slice 098 D1 precedent) · spillover slice 107 filed for backend `?include=ack_rate` extension · 25 ack-rate + 14 filter + 3 BFF vitest cases · PR gh#233                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 105 | Risk-create UI for /risks empty-state CTA (follow-on to slice 100)  | `in-review`   | frontend/105-risk-create-ui                             | gh#231 | 2026-05-16 | 2026-05-16 | batch 37 · frontend · 1-2d · AFK · PR gh#231 · 9 files / 758 insertions · 8/8 ACs PASS · binds directly to `createReq` (no invented fields per P0-A4) · new `web/app/(authed)/risks/new/{page,risk-form,actions}.tsx` · BFF `web/app/api/risks/route.ts` extended with POST handler (GET unchanged) · `web/lib/api.ts` appends `RiskCreateInput` + `createRisk` · slice 100's empty-state CTA re-pointed `/admin` → `/risks/new` · 5x5 widget = two native `<select>` likelihood × impact serializing into `{likelihood, impact}` · 3 new vitest POST cases (6 total in file) · quarantined Playwright spec follows slice-079 pattern · 183/183 vitest green · pre-commit + npm-lint-web hooks green                                                                                                                                                                                                                                                                                                                                   |

## Ready set right now

| #   | Title                                     | Cluster  | Est (d) | Notes                                                                                                                          |
| --- | ----------------------------------------- | -------- | ------- | ------------------------------------------------------------------------------------------------------------------------------ |
| 099 | /evidence list view                       | Frontend | 1-2     | **Reuses 098's shell.** Consumes `GET /v1/evidence?...`. May need backend `?include=` extension.                               |
| 101 | /policies list view                       | Frontend | 1-2     | **Reuses 098's shell.** Consumes `GET /v1/policies` + ack rate. Backend extension preferred over client fan-out.               |
| 105 | Risk-create UI for /risks empty-state CTA | Frontend | 1-2     | Spillover from 100. /risks empty-state currently routes to /admin as a placeholder. Bind to `POST /v1/risks` `createReq` form. |
| 103 | /settings user-facing page                | Frontend | 2       | 5 sections; form-driven (distinct surface from list-views). Plaintext-token-shown-once is security-critical (P0-A2).           |

**4 slices ready** (099, 101, 105, 103 — ordered ascending by estimate). **Recommended next continuous-batch sweep:**

- **Batch 37**: 099 + 101 + 105 in parallel — three 1-2d slices, all frontend, all file-disjoint (web/app/(authed)/{evidence,policies,risks/new}/).
- **Then 103 solo** — form-driven, distinct surface, 2d.

After 099/101/105/103 land, the slice-093 UI fill-in suite is complete and the v2 backlog enters a new ready-set vacuum unless 082/084/095 gates clear or new slices file.

Total wall-clock estimate: ~2-3 calendar days at batch-of-three throughput.
...
- PR pending — this batch-36 final-reconcile sweep. Touches `_STATUS.md` only. Awaits CI + maintainer review.

Stale worktrees on disk: **0** (slice 096 cleanup remains in effect). The batch-36 worktrees (`-100`, `-102`, `-104`) were removed at merge-time.
