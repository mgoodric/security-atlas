...
> 4. **Control-bundle re-upload was never idempotent** (blocks AC-7 — the bootstrap idempotency re-run). `internal/control`'s `Upload` inserted the new active control row before superseding the predecessor, momentarily leaving two `superseded_by IS NULL` rows per bundle and tripping the `controls_one_active_version_per_bundle` unique index. The prescribed supersede-first order was impossible while the `controls_superseded_by_fk` self-FK was non-deferrable. Fixed by migration `20260511000032` (FK → `DEFERRABLE INITIALLY DEFERRED`) + reordering `Upload`. The slice-009 test that guards this (`TestUpload_ReuploadSupersedes`) existed but had never run — the CI integration job omitted `internal/control` from its package list; now wired in, with its bit-rotted schema fixtures repaired.
>
