...
> **Scope grew (PR #125).** The identifier alignment alone did **not** green slice 065's `test-self-host-bundle` e2e job — that job stayed red in both matrix modes. A CI-driven root-cause pass found several further independent defects that also block fresh-deploy control-bundle upload, each surfacing only once the prior one was fixed and the job advanced:
>
...
> 3. **Harness assertion 5 checked the wrong table.** It asserted `api_keys >= 1` to "prove phase 6 ran", but nothing in the bootstrap flow writes `api_keys`. Re-pointed at `decision_audit_log` — the table slice 065 bug #1 was actually about, populated by every authenticated control-bundle upload.
> 4. **Control-bundle re-upload was never idempotent** (blocks AC-7 — the bootstrap idempotency re-run). Two coupled defects: (a) `internal/control`'s `Upload` inserted the new active control row before superseding the predecessor, momentarily leaving two `superseded_by IS NULL` rows per bundle and tripping the `controls_one_active_version_per_bundle` unique index — the prescribed supersede-first order was impossible while the `controls_superseded_by_fk` self-FK was non-deferrable; and (b) even with that fixed, re-uploading byte-identical content version-bumped instead of being a no-op, so `controls` doubled (50 → 100) on the bootstrap re-run. Fixed by migration `20260511000033` (FK → `DEFERRABLE INITIALLY DEFERRED`), reordering `Upload` to supersede-then-insert, and making `Upload` a true no-op when the active version's `bundle_manifest_hash` matches the incoming bundle. The slice-009 test that guards (a) (`TestUpload_ReuploadSupersedes`) existed but had never run — the CI integration job omitted `internal/control` from its package list; now wired in, with its bit-rotted schema fixtures repaired, plus a new `TestUpload_ReuploadIdenticalIsNoop` for (b).
>
