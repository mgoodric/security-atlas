# 296 — Coverage lift — `internal/catalog/metrics` to 70%+
...
`docs/coverage-audit-2026-05.md` measured `internal/catalog/metrics` at **64.7% merged
...
**Notes:** catalog metric emitter; needs unit on the entry helpers

## What ships in this slice

1. **New unit tests** under `internal/catalog/metrics/*_test.go` covering the
...
- [ ] **AC-1.** New unit tests for `internal/catalog/metrics` move its merged coverage
      to ≥ 70%.
...
- [ ] **AC-3.** Each new test file's first comment block names the
      ...
      designed to cover.
- [ ] **AC-4.** `coverage-thresholds.json` ratchets the `internal/catalog/metrics` floor
...
- **P0-296-1.** Does NOT raise the `internal/catalog/metrics` floor without writing
...
go tool cover -func=merged.cov | grep 'internal/catalog/metrics'
```
