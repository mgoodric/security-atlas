# 294 — Coverage lift — `internal/metrics/eval` to 70%+
...
`docs/coverage-audit-2026-05.md` measured `internal/metrics/eval` at **33.0% merged
coverage** (unit-only: 33.0%), below the 70% aspirational target the
...
**Notes:** eval metric reducer; small

## What ships in this slice

1. **New unit tests** under `internal/metrics/eval/*_test.go` covering the
...
- [ ] **AC-1.** New unit tests for `internal/metrics/eval` move its merged coverage
      to ≥ 70%.
...
- [ ] **AC-3.** Each new test file's first comment block names the
      ...
      designed to cover.
- [ ] **AC-4.** `coverage-thresholds.json` ratchets the `internal/metrics/eval` floor
...
- **P0-294-1.** Does NOT raise the `internal/metrics/eval` floor without writing
  the unit tests that hit the new bar.
- **P0-294-2.** Does NOT lower any existing floor — every change to
  `thresholds` is monotonically ↑.
- **P0-294-3.** Does NOT modify `_STATUS.md` from inside this
...
go tool cover -func=merged.cov | grep 'internal/metrics/eval'
```
