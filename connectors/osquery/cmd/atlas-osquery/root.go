...
const ConnectorName = "osquery-connector"

// SupportedKinds is the canonical list of evidence kinds slice 047 emits.
// Reuses slice 014's bundled platform schema osquery.host_posture.v1
// unchanged — no new schemas this slice.
...
func newRootCmd() *cobra.Command {
	root := &cobra.Command{
		Use:           ConnectorName,
		Short:         "security-atlas osquery/Fleet endpoint connector",
...
func actorID(service string) string {
	return "connector:osquery:" + service + "@" + connectorVersion()
}
...
const longDescription = `security-atlas osquery / Fleet endpoint connector

Emits one evidence kind:
  - osquery.host_posture.v1 (run subcommand, pull)

Two upstream modes (single subcommand suite, --mode flag on run):
  - --mode=fleet (default): Fleet REST API; Bearer-token auth.
  - --mode=local          : local osqueryd extension Unix socket; no auth
                            (root-owned socket is the security boundary).

Required Fleet roles (least-privilege, read-only):
  - observer            (global)
  - observer_plus       (per-team)

Never grant admin or maintainer. Use the 'scopes' subcommand to print
the canonical list.

Auth (Fleet mode): set the FLEET_API_TOKEN environment variable in this
process. The --token flag is hidden so tokens never appear in shell
history or process listings; env-var is the supported path.
...
References:
  ...
  - canvas §1.6 (anti-pattern: never ship a proprietary endpoint agent)`
