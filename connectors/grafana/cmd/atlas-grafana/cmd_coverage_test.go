// Unit tests for the atlas-grafana cmd glue. Mirrors the datadog-connector
// coverage suite. No real Grafana tokens or vendor-prefixed JWTs in fixtures —
...
func TestNewPermissionsCmd_RendersRole(t *testing.T) {
...
func TestActorID_Shape(t *testing.T) {
	id := actorID("alerts")
	if !strings.HasPrefix(id, "connector:grafana:alerts@") {
