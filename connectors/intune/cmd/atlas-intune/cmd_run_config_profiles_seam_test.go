// Seam tests for doRunConfigProfiles. The Graph config-profile read + the sdk
// client constructor are swapped for fakes so doRunConfigProfiles is exercised
// end-to-end without touching live Graph or a real platform. Seams are restored
// via t.Cleanup. No real Graph credentials — neutral "test-*" / "fake-*" only.
package main
...
func twoDevicesConfigProfiles() []cfgprofile.RawDeviceProfiles {
	return []cfgprofile.RawDeviceProfiles{
		{DeviceID: "1", Profiles: []cfgprofile.RawProfile{{Name: "Compliance", Settings: []cfgprofile.RawSetting{{Key: "disk_encryption_enforced", Value: "true"}}}}},
		{DeviceID: "2", Profiles: []cfgprofile.RawProfile{{Name: "BitLocker"}}},
	}
}
...
func TestDoRunConfigProfiles_CollectError(t *testing.T) {
	...
	err := doRunConfigProfiles(context.Background(), okConfigProfileFlags())
	if !errors.Is(err, sentinel) || !strings.HasPrefix(err.Error(), "intune config-profile collect: ") {
...
func TestNewConfigProfileAPI_Constructor(t *testing.T) {
	if newConfigProfileAPI(devices.ClientConfig{GraphBaseURL: "https://graph.microsoft.com/v1.0"}) == nil {
