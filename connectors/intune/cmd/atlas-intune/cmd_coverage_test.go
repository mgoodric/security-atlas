// Unit tests for the atlas-intune cmd glue. Mirrors the slice-488 datadog-connector
...
//
// No real Intune credentials or vendor-prefixed JWTs appear in fixtures —
...
func TestNewRootCmd_HasSubcommands(t *testing.T) {
	...
	for _, want := range []string{"register", "run", "webhook", "permissions"} {
		if !names[want] {
...
func TestNewPermissionsCmd_RendersScope(t *testing.T) {
	...
	cmd.Run(cmd, nil)
	out := buf.String()
	for _, want := range []string{"CREDENTIAL", "DeviceManagementManagedDevices.Read.All", "Entra app"} {
...
func TestActorID_Shape(t *testing.T) {
	id := actorID("devices")
	if !strings.HasPrefix(id, "connector:intune:devices@") {
