...
// fakeDevicesAPI is a faked Intune managed-devices surface (NO live Graph).
type fakeDevicesAPI struct{ devices []devices.RawDevice }

func (f *fakeDevicesAPI) ListManagedDevices(_ context.Context) ([]devices.RawDevice, error) {
...
func TestRegister_ListsConnector(t *testing.T) {
	...
	resp, err := registry.Register(ctx, &connectorsv1.RegisterRequest{
		...
		InstanceId:        "test-instance-intune",
	...
	if !found {
		t.Fatal("intune-connector not present in List — AC-1 fail")
	}
}

// TestRun_PushesDeviceRecords verifies AC-3/AC-5/AC-6/AC-9.
func TestRun_PushesDeviceRecords(t *testing.T) {
	...
	api := &fakeDevicesAPI{devices: []devices.RawDevice{
		{ID: "d-1", Name: "ENG-PC-014", OSVersion: "10.0.22631", OS: "Windows", Encrypted: true, PasscodeCompliant: true,
			ComplianceState: "compliant", ManagementState: "mdm", Enrolled: true,
			OwnerAssignmentID: "u-1@tenant.example", OwnerDisplayName: "A. Engineer"},
	}}
	raw, err := devices.Collect(context.Background(), api)
	if err != nil {
		t.Fatalf("Collect: %v", err)
	}
	devs := devposture.Normalize(devposture.MDMIntune, raw, fixed)
	rec, err := devrecord.Build(devs[0], "scf:END-04", actorID("devices"), "intune", "prod")
	if err != nil {
	...
	if !strings.HasPrefix(rec.GetSourceAttribution().GetActorId(), "connector:intune:devices@") {
...
func TestRun_DedupesWithinHour(t *testing.T) {
	...
	api := &fakeDevicesAPI{devices: []devices.RawDevice{{ID: "1", Enrolled: true}}}
	raw, _ := devices.Collect(context.Background(), api)
	devs := devposture.Normalize(devposture.MDMIntune, raw, fixed)
	r1, _ := devrecord.Build(devs[0], "scf:END-04", actorID("devices"), "intune", "prod")
	r2, _ := devrecord.Build(devs[0], "scf:END-04", actorID("devices"), "intune", "prod")
...
// TestEmittedRecords_NoGeoOrAppsOrPII verifies AC-10 + P0-490-3.
func TestEmittedRecords_NoGeoOrAppsOrPII(t *testing.T) {
	...
	raw, _ := devices.Collect(context.Background(), api)
	devs := devposture.Normalize(devposture.MDMIntune, raw, fixed)
	rec, _ := devrecord.Build(devs[0], "scf:END-04", actorID("devices"), "intune", "prod")
	...
	banned := []string{"latitude", "longitude", "geolocation", "detectedapps", "installed_apps", "phonenumber", "phone_number"}
...
func TestCredential_NeverLogged(t *testing.T) {
	...
	cred, err := intuneauth.Resolve(intuneauth.ResolveOpts{TenantID: "t", ClientID: "c", ClientSecret: secret})
	if err != nil {
