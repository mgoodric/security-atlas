...
//   - pull:      scheduled read-only Microsoft Graph GETs (the `run` /
//     `run-software` / `run-config-profiles` subcommands).
//   - subscribe: event-driven receipt of Microsoft Graph change notifications
//     (the `webhook` subcommand) — a fresh device-posture record is emitted when a
//     managed device's compliance state changes. This is NOT "continuous
//     monitoring" and NOT a relabeled poll.
//
// Both values describe how the connector retrieves data FROM Microsoft Graph. The
