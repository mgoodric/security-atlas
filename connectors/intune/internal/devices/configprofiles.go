...
// concrete implementation issues read-only Graph API calls (the
// deviceConfigurationStates expansion); tests pass a fake. v0 reads the first
...
// RawConfigSetting is the narrow, secret-bounded view the Graph client returns
...
// RawConfigProfile is the narrow, secret-bounded view the Graph client returns
// for one configuration / compliance profile assigned to a device. There is no
...
// RawDeviceConfigProfiles is one Intune managed device's id + its assigned
...
// CollectConfigProfiles lists each managed device's assigned configuration /
// compliance profiles and returns secret-bounded cfgprofile.RawDeviceProfiles
// ready for cfgprofile.Normalize. Separated from normalization so the cmd layer
// owns the observed-at clock.
func CollectConfigProfiles(ctx context.Context, api ConfigProfileAPI) ([]cfgprofile.RawDeviceProfiles, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("list intune config profiles: %w", err)
	}
