// Unit tests for the atlas-rippling cmd glue. Mirrors the slice-490 jamf-connector
...
//
// No real Rippling credentials or vendor-prefixed JWTs appear in fixtures —
...
func TestNewPermissionsCmd_RendersScope(t *testing.T) {
	...
	cmd.Run(cmd, nil)
	out := buf.String()
	for _, want := range []string{"CREDENTIAL", "read-only", "API token"} {
	...
	if !strings.Contains(out, "NEVER grant") || !strings.Contains(strings.ToLower(out), "write scope") {
...
func TestActorID_Shape(t *testing.T) {
	id := actorID("workers")
	if !strings.HasPrefix(id, "connector:rippling:workers@") {
