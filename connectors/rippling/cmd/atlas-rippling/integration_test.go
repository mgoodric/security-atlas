...
// fakeWorkersAPI is a faked Rippling worker surface (NO live Rippling).
...
func TestRegister_ListsConnector(t *testing.T) {
	...
	resp, err := registry.Register(ctx, &connectorsv1.RegisterRequest{
		...
		InstanceId:        "test-instance-rippling",
	...
	if !found {
		t.Fatal("rippling-connector not present in List — AC-1 fail")
	}
}

// TestRun_PushesWorkerRecords verifies AC-2/AC-5/AC-6/AC-9: collect from a faked
// Rippling API, build the canonical record, push through the platform's single
// Push RPC, and assert the receipt (sha256 content hash).
func TestRun_PushesWorkerRecords(t *testing.T) {
	...
	api := &fakeWorkersAPI{workers: []workers.RawWorker{
		{ID: "emp-1", EmploymentStatus: "ACTIVE", StartDate: "2024-01-15", Title: "Software Engineer",
	...
	wks := worker.Normalize(worker.HRISRippling, raw, fixed)
	rec, err := workerrecord.Build(wks[0], "scf:IAC-22", actorID("workers"), "rippling", "prod")
	if err != nil {
	...
	if !strings.HasPrefix(rec.GetSourceAttribution().GetActorId(), "connector:rippling:workers@") {
...
func TestRun_DedupesWithinHour(t *testing.T) {
	...
	api := &fakeWorkersAPI{workers: []workers.RawWorker{{ID: "1", EmploymentStatus: "ACTIVE"}}}
	raw, _ := workers.Collect(context.Background(), api)
	wks := worker.Normalize(worker.HRISRippling, raw, fixed)
	r1, _ := workerrecord.Build(wks[0], "scf:IAC-22", actorID("workers"), "rippling", "prod")
	r2, _ := workerrecord.Build(wks[0], "scf:IAC-22", actorID("workers"), "rippling", "prod")
...
// (AC-10 + P0-491-3): even when the fake source carries banned-shaped values in
// every free-text lifecycle field, the emitted payload carries worker-lifecycle
// facts only — no SSN / compensation / address / bank / benefits / performance /
// DOB / personal-contact keys or substrings.
func TestEmittedRecords_NoSensitivePII(t *testing.T) {
	fixed := func() time.Time { return time.Date(2026, 6, 7, 12, 0, 0, 0, time.UTC) }
	api := &fakeWorkersAPI{workers: []workers.RawWorker{
		{ID: "1", EmploymentStatus: "TERMINATED", StartDate: "2024-01-15", EndDate: "2026-05-31",
			Title: "Software Engineer", Department: "Engineering", ManagerAssignmentID: "mgr-9",
			WorkEmail: "a.engineer@corp.example"},
	}}
	raw, _ := workers.Collect(context.Background(), api)
	wks := worker.Normalize(worker.HRISRippling, raw, fixed)
	rec, _ := workerrecord.Build(wks[0], "scf:IAC-22", actorID("workers"), "rippling", "prod")
	...
	// Keys/values that would indicate over-collection of the most sensitive HRIS
	// PII (threat-model I).
...
func TestCredential_NeverLogged(t *testing.T) {
	const secret = "test-rippling-key-no-log"
	cred, err := ripplingauth.Resolve(ripplingauth.ResolveOpts{APIToken: secret})
	if err != nil {
