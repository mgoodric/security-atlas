// Seam tests for doRunSoftware. The Jamf software read + the sdk client
// constructor are swapped for fakes so doRunSoftware is exercised end-to-end
// without touching live Jamf or a real platform. Seams are restored via
// t.Cleanup. No real Jamf credentials — neutral "test-*" / "fake-*" only.
package main
...
func twoDevicesSoftware() []swinventory.RawDeviceSoftware {
	return []swinventory.RawDeviceSoftware{
		...
		{DeviceID: "2", Software: []swinventory.RawSoftwareItem{{Name: "Firefox"}}},
	}
}
...
func TestDoRunSoftware_CollectError(t *testing.T) {
	...
	err := doRunSoftware(context.Background(), okSoftwareFlags())
	if !errors.Is(err, sentinel) || !strings.HasPrefix(err.Error(), "jamf software collect: ") {
...
func TestNewSoftwareAPI_Constructor(t *testing.T) {
	if newSoftwareAPI(http.DefaultClient, "https://org.jamfcloud.com", "test-jamf-client", "fake-jamf-secret") == nil {
