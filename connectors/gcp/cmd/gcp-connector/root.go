...
const ConnectorName = "gcp-connector"

// Evidence kinds slice 442 emits — one per GCP evidence surface (project IAM
// policy bindings / service-account inventory, and Cloud Storage bucket
// configuration). The `.v1` suffix is part of the stable identifier; the
// schema version is a separate semver (EVIDENCE_SDK §4.5).
const (
	KindIAMBinding    = "gcp.iam_policy_binding.v1"
	KindStorageBucket = "gcp.storage_bucket_config.v1"
)

// SupportedKinds is the set announced at registration.
var SupportedKinds = []string{KindIAMBinding, KindStorageBucket}

// PullIntervalNote is the honest interval label for the pull profile (slice
// 442 AC-7 / anti-pattern: no "continuous monitoring"). The connector is run
...
func newRootCmd() *cobra.Command {
	root := &cobra.Command{
		Use:           ConnectorName,
		Short:         "security-atlas GCP connector",
		SilenceUsage:  true,
		SilenceErrors: true,
	}
...
// convention `connector:<vendor>:<service>@<version>`. service is the GCP
// surface the record came from so the two kinds carry distinct actor ids
// (`connector:gcp:iam@…`, `connector:gcp:storage@…`).
func actorID(service string) string {
	return "connector:gcp:" + service + "@" + connectorVersion()
}
