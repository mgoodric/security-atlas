...
func newRunCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Read Datadog monitor inventory via the read-only Datadog API
(GET /api/v1/monitor, monitors_read scope), transform to monitoring.alert_config.v1
...
Auth: set DATADOG_API_KEY + DATADOG_APP_KEY (read-only monitors_read scope), and
optionally DATADOG_SITE. The keys never appear in a log line or an evidence
record. The connector emits monitor name/type/enabled + notification-target
HANDLES only — never the secret webhook URL, an integration token, a recipient
email, the query, dashboard JSON, or metric time-series.`,
		SilenceErrors: true,
		SilenceUsage:  true,
...
func doRun(ctx context.Context, f runFlags) error {
	...
	fmt.Printf("pushed %d records (vendor=datadog environment=%s)\n", pushed, f.environment)
	return nil
}
