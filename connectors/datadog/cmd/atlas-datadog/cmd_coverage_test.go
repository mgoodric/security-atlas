// Unit tests for the atlas-datadog cmd glue. Mirrors the slice-487 k8s-connector
...
//
// No real Datadog keys or vendor-prefixed JWTs appear in fixtures — neutral
...
func TestNewPermissionsCmd_RendersScope(t *testing.T) {
	...
	// P0-488-2: the rendered output must carry the read-only warning that bans
	// write/admin scopes (the warning is the only place "write" is mentioned —
	// as a prohibition, not a granted scope).
	if !strings.Contains(out, "NEVER grant a write/admin") {
...
func TestActorID_Shape(t *testing.T) {
	id := actorID("monitors")
	if !strings.HasPrefix(id, "connector:datadog:monitors@") {
