...
//   - monitoring.alert_config.v1 (Datadog monitor inventory, pull)
...
Least-privilege Datadog access (read-only):
  ...
  - NEVER grant a write/admin scope (no monitors_write, no admin).

The connector reads monitor CONFIGURATION only — monitor name, type, enabled
state, and the notification-target HANDLES parsed from the monitor message
(e.g. @slack-sec-oncall). It NEVER collects the secret webhook URL behind an
integration, an integration token, a recipient email address (PII), the monitor
query, dashboard JSON, or metric time-series. Email-recipient mentions in a
monitor message are dropped.
